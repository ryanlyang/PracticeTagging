#!/usr/bin/env bash
# Two-stage, self-contained run:
# 1) train an offline 7D-kin teacher (JetClassTransformer),
# 2) run confgen reconstructor with Stage-A teacher-logit alignment.
# No external teacher checkpoint dependency.
#
# Usage:
#   sbatch run_train_jetclass_joint_dualview_confgen_v2attr_250k50k250k_v1hltplus25_stagea_teacherlogit_autoteacher_recoonlydual.sh
#
# Optional overrides:
#   RUN_NAME=... TEACHER_RUN_NAME=... N_TRAIN_JETS=... N_VAL_JETS=... N_TEST_JETS=... sbatch ...

#SBATCH --job-name=jcOffTA2
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=12
#SBATCH --mem=96G
#SBATCH --time=72:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_confgen_v2attr_250k50k250k_v1hltplus25_stagea_teacherlogit_autoteacher_recoonlydual_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_confgen_v2attr_250k50k250k_v1hltplus25_stagea_teacherlogit_autoteacher_recoonlydual_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_confgen_v2attr_250k50k250k_v1hltplus25_stagea_teacherlogit_autoteacher_recoonlydual}"
TEACHER_RUN_NAME="${TEACHER_RUN_NAME:-${RUN_NAME}_teacherkin}"
...
echo "============================================================"
echo "Step 0/1: Train offline 7D-kin teacher for Stage-A logit alignment"
echo "Job ID: ${SLURM_JOB_ID:-N/A}"
echo "Node: ${SLURMD_NODENAME:-N/A}"
echo "Teacher run: ${SAVE_DIR}/${TEACHER_RUN_NAME}"
echo "Split: train=${N_TRAIN_JETS}, val=${N_VAL_JETS}, test=${N_TEST_JETS}"
echo "HLT profile for baseline in this pretrain: v1 +25%"
echo "============================================================"

TEACHER_CMD=(
  python -u evaluate_jetclass_hlt_teacher_baseline.py
  --data_dir "${DATA_DIR}"
  --save_dir "${SAVE_DIR}"
...
printf ' %q' "${TEACHER_CMD[@]}"
echo
"${TEACHER_CMD[@]}"

TEACHER_CKPT="${SAVE_DIR}/${TEACHER_RUN_NAME}/teacher_offline_best.pt"
if [[ ! -f "${TEACHER_CKPT}" ]]; then
  echo "Missing auto-trained Stage-A teacher checkpoint: ${TEACHER_CKPT}" >&2
  exit 1
fi

echo "============================================================"
echo "Step 1/1: Confgen with Stage-A teacher-logit alignment"
...
# Teacher-dominant Stage-A, target = teacher(offline), student = teacher(reconstructed).
export JETCLASS_STAGEA_OFFLINE_TEACHER_CKPT="${TEACHER_CKPT}"
