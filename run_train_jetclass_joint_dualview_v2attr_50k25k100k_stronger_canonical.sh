#!/usr/bin/env bash
#SBATCH --job-name=jcJv2SmC
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --time=1-12:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_50k25k100k_stronger_canonical_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_50k25k100k_stronger_canonical_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_v2attr_50k25k100k_stronger_canonical}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
...
CMD=(
  python -u train_jetclass_joint_dualview_stage2_unmergeonly_v2_attr.py
  --data_dir "${DATA_DIR}"
  --save_dir "${SAVE_DIR}"
  --run_name "${RUN_NAME}"
  --seed "${SEED}"
...
echo "============================================================"
echo "JetClass Joint Dual-View V2 (attr-head constrained unmerge)"
