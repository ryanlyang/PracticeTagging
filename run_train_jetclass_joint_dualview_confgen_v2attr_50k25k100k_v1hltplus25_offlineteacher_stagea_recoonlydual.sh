#!/usr/bin/env bash
#SBATCH --job-name=jcOffT
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=12
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_confgen_v2attr_50k25k100k_v1hltplus25_offlineteacher_stagea_recoonlydual_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_confgen_v2attr_50k25k100k_v1hltplus25_offlineteacher_stagea_recoonlydual_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_confgen_v2attr_50k25k100k_v1hltplus25_offlineteacher_stagea_recoonlydual}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
...
# IMPORTANT:
# Stage-A offline-teacher mode needs a 7D-kin teacher checkpoint compatible with the
# wrapper's corrected-view KD path. A fused-student offline teacher is typically used.
TEACHER_CKPT="${TEACHER_CKPT:-checkpoints/jetclass_joint_dualview/jetclass_offline_fused_student_50k25k100k_v1hltplus25_pair/offline_fused_student.pt}"
...
if [[ ! -f "${TEACHER_CKPT}" ]]; then
  echo "Missing Stage-A teacher checkpoint: ${TEACHER_CKPT}" >&2
  echo "Set TEACHER_CKPT=/path/to/compatible_offline_teacher.pt" >&2
  exit 1
fi
...
echo "============================================================"
echo "JetClass confgen run: Stage-A offline-teacher objective"
echo "Job ID: ${SLURM_JOB_ID:-N/A}"
echo "Node: ${SLURMD_NODENAME:-N/A}"
echo "Run: ${SAVE_DIR}/${RUN_NAME}"
echo "Teacher ckpt: ${TEACHER_CKPT}"
echo "HLT profile: v1 +25% stronger corruption"
echo "StageC epochs: ${STAGEC_EPOCHS}"
echo "============================================================"
printf ' %q' "${CMD[@]}"
echo
"${CMD[@]}"
