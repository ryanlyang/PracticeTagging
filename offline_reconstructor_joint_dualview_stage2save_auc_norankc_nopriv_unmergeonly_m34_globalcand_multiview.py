...
def _chunked_search_candidates_split(
    ...
    for cls_val in list(search_cls_values):
        ...
        round_found_out = np.full((n, target_k), float(max_rounds + 1), dtype=np.float32)
            ...
            for i in range(b):
                ...
                if not entries:
                    ...
                    entries = [{
                        ...
                        "round_found": float(max_rounds + 1),
                        "feasible": False,
                    }]
                ...
                for k, z in enumerate(selected):
                    ...
                    round_found_out[g, k] = float(z.get("round_found", float(max_rounds + 1)))  # type: ignore[index]
                    feasible_out[g, k] = bool(z["feasible"])  # type: ignore[index]

        resid_total_pre = resid_total_out.copy()
        resid_set_pre = resid_set_out.copy()
        resid_count_pre = resid_count_out.copy()
        resid_pt_pre = resid_pt_out.copy()
        resid_mass_pre = resid_mass_out.copy()
        ...
        out[f"class{cls_val}"] = {
            ...
            "res_total_pre": resid_total_pre,
            "res_set": resid_set_out,
            "res_set_pre": resid_set_pre,
            "res_count": resid_count_out,
            "res_count_pre": resid_count_pre,
            "res_pt": resid_pt_out,
            "res_pt_pre": resid_pt_pre,
            "res_mass": resid_mass_out,
            "res_mass_pre": resid_mass_pre,
            "round_found": round_found_out,
