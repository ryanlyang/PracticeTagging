...
def build_soft_corrected_view(
    ...
    Output feature dims:
      - 10 when include_flags=False: 7 base + [tok_weight, parent_added_weight, eff_share]
      - 12 when include_flags=True:  10 + [merge_flag_soft, eff_flag_soft]
    """
    eps = 1e-8
...
def build_corrected_view_numpy(
    ...
    out_dim = 12 if bool(corrected_use_flags) else 10
    feat_b = np.zeros((n_jets, seq_len, out_dim), dtype=np.float32)
...
def summarize_soft_corrected_view(
    ...
    # Extra channels:
    #   dim=10 -> [tok_weight, parent_added_weight, eff_share]
    #   dim=12 -> [tok_weight, parent_added_weight, eff_share, merge_flag_soft, eff_flag_soft]
    tok_w = feat_b[..., 7]
...
def metrics_from_preds(y_true: np.ndarray, y_score: np.ndarray) -> Dict[str, float]:
    if y_true.size == 0:
        return {"auc": float("nan"), "fpr30": float("nan"), "fpr50": float("nan")}
...
def _allocation_losses_from_out(
    out: Dict[str, torch.Tensor],
    mask_hlt: torch.Tensor,
    mask_off: torch.Tensor,
    merge_target_share: float,
    quota_strength: float,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Train/infer allocation consistency terms built from non-privileged count supervision.
    """
    child_w = out["child_weight"].clamp(0.0, 1.0)
    gen_w = out["gen_weight"].clamp(0.0, 1.0)
    pred_added_merge = child_w.sum(dim=1)
    ...
    return loss_alloc_hard, loss_alloc_quota


def train_reconstructor_priv_alloc(
    ...
    """
    Privileged Stage-A trainer (uses merge/eff budgets), with optional allocation constraints.
    """
    ...
    w_alloc_hard = float(loss_cfg.get("w_alloc_hard", 0.0))
    w_alloc_quota = float(loss_cfg.get("w_alloc_quota", 0.0))

    for ep in tqdm(range(int(train_cfg["epochs"])), desc="ReconstructorPrivAlloc"):
        model.train()
        ...
        for batch in train_loader:
            ...
            if w_alloc_hard > 0.0 or w_alloc_quota > 0.0:
                loss_alloc_hard, loss_alloc_quota = _allocation_losses_from_out(
                    out,
                    mask_hlt,
                    mask_off,
                ...
                total = losses["total"] + w_alloc_hard * loss_alloc_hard + w_alloc_quota * loss_alloc_quota
            else:
                loss_alloc_hard = torch.zeros((), device=device)
                loss_alloc_quota = torch.zeros((), device=device)
                total = losses["total"]
            ...
            tr_alloc_hard += loss_alloc_hard.item() * bs
            tr_alloc_quota += loss_alloc_quota.item() * bs
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                va_alloc_hard += loss_alloc_hard.item() * bs
                va_alloc_quota += loss_alloc_quota.item() * bs
...
def train_joint_dual(
    ...
    corrected_use_flags: bool,
    reco_loss_mode: str,
    enable_allocation_constraints: bool,
    alloc_merge_share: float,
    alloc_quota_strength: float,
    w_alloc_hard: float,
    w_alloc_quota: float,
    min_epochs: int,
    ...
    for ep in tqdm(range(int(epochs)), desc=stage_name):
        ...
        for batch in train_loader:
            ...
            if float(lambda_reco) > 0.0:
                if str(reco_loss_mode).lower() == "priv":
                    ...
                    reco_losses = compute_reconstruction_losses_nopriv(
                        ...
                        b_total,
                        BASE_CONFIG["loss"],
                        merge_target_share=float(alloc_merge_share),
                        quota_strength=float(alloc_quota_strength),
                    )

                loss_reco = reco_losses["total"]
                if bool(enable_allocation_constraints):
                    loss_alloc_hard, loss_alloc_quota = _allocation_losses_from_out(
                        reco_out,
                        mask_hlt,
                        mask_off,
                    ...
                    loss_reco = (
                        loss_reco
                        + float(w_alloc_hard) * loss_alloc_hard
                        + float(w_alloc_quota) * loss_alloc_quota
                    )
            else:
...
def main() -> None:
    ...
    parser.add_argument("--fresh_stagec_split", action="store_true")
    parser.add_argument("--use_nopriv_total_budget", action="store_true")
    parser.add_argument("--enable_allocation_constraints", action="store_true")
    ...
    parser.add_argument(
        "--max_split_children",
        type=int,
        default=int(BASE_CONFIG["reconstructor_model"]["max_split_children"]),
    )
    ...
    if bool(args.enable_allocation_constraints):
        cfg["loss"]["w_alloc_hard"] = float(args.w_alloc_hard)
        cfg["loss"]["w_alloc_quota"] = float(args.w_alloc_quota)
    else:
        cfg["loss"]["w_alloc_hard"] = 0.0
        cfg["loss"]["w_alloc_quota"] = 0.0
    ...
    use_fresh_stagec = bool(args.fresh_stagec_split)
    n_slices = 2 if use_fresh_stagec else 1
    max_jets_needed = args.offset_jets + n_slices * args.n_train_jets
    ...
    # Slice A is always used for stages A/B.
    # Slice C is either a fresh disjoint pool (when --fresh_stagec_split) or aliases slice A.
    ...
    if use_fresh_stagec:
        ...
        masks_off_c = masks_off_a
        const_off_c = const_off_a
    ...
    hlt_const_a, hlt_mask_a, hlt_stats_a, budget_truth_a, origin_counts_a = apply_hlt_effects_realistic_with_tracking(
        const_off_a, masks_off_a, cfg, seed=int(args.seed)
    )
    if use_fresh_stagec:
        hlt_const_c, hlt_mask_c, hlt_stats_c, budget_truth_c, origin_counts_c = apply_hlt_effects_realistic_with_tracking(
            const_off_c, masks_off_c, cfg, seed=int(args.seed) + 1337
        )
    else:
        hlt_const_c = hlt_const_a
        hlt_mask_c = hlt_mask_a
        hlt_stats_c = hlt_stats_a
        budget_truth_c = budget_truth_a
        origin_counts_c = origin_counts_a
    ...
    train_idx_a, temp_idx_a = train_test_split(
        idx_a, test_size=0.30, random_state=int(args.seed), stratify=labels_a
    )
    val_idx_a, test_idx_a = train_test_split(
        temp_idx_a, test_size=0.50, random_state=int(args.seed), stratify=labels_a[temp_idx_a]
    )
    if use_fresh_stagec:
        idx_c = np.arange(len(labels_c))
        train_idx_c, temp_idx_c = train_test_split(
            idx_c, test_size=0.30, random_state=int(args.seed) + 1, stratify=labels_c
        )
        val_idx_c, test_idx_c = train_test_split(
            temp_idx_c, test_size=0.50, random_state=int(args.seed) + 1, stratify=labels_c[temp_idx_c]
        )
        print(
        ...
        train_idx_c, val_idx_c, test_idx_c = train_idx_a, val_idx_a, test_idx_a
        print(
            f"Slice A sizes: Train={len(train_idx_a)}, Val={len(val_idx_a)}, Test={len(test_idx_a)} | "
            f"Slice C: aliased to Slice A (fresh split disabled)"
        )
    ...
    # Test pools: A and C individually; combined if fresh split is enabled.
    labels_test_a = labels_a[test_idx_a]
    feat_off_test_std_a = feat_off_std_a[test_idx_a]
    mask_off_test_a = masks_off_a[test_idx_a]
    feat_hlt_test_std_a = feat_hlt_std_a[test_idx_a]
    hlt_mask_test_a = hlt_mask_a[test_idx_a]
    const_off_test_a = const_off_a[test_idx_a]
    const_hlt_test_a = hlt_const_a[test_idx_a]
    budget_merge_test_a = budget_merge_true_a[test_idx_a]
    budget_eff_test_a = budget_eff_true_a[test_idx_a]
    budget_total_test_a = budget_total_true_a[test_idx_a]
    origin_counts_test_a = origin_counts_a[test_idx_a]

    labels_test_c = labels_c[test_idx_c]
    feat_off_test_std_c = feat_off_std_c[test_idx_c]
    mask_off_test_c = masks_off_c[test_idx_c]
    feat_hlt_test_std_c = feat_hlt_std_c[test_idx_c]
    hlt_mask_test_c = hlt_mask_c[test_idx_c]
    const_off_test_c = const_off_c[test_idx_c]
    const_hlt_test_c = hlt_const_c[test_idx_c]
    budget_merge_test_c = budget_merge_true_c[test_idx_c]
    budget_eff_test_c = budget_eff_true_c[test_idx_c]
    budget_total_test_c = budget_total_true_c[test_idx_c]
    origin_counts_test_c = origin_counts_c[test_idx_c]

    if use_fresh_stagec:
        labels_test = np.concatenate([labels_test_a, labels_test_c], axis=0)
        feat_off_test_std = np.concatenate([feat_off_test_std_a, feat_off_test_std_c], axis=0)
        mask_off_test = np.concatenate([mask_off_test_a, mask_off_test_c], axis=0)
        feat_hlt_test_std = np.concatenate([feat_hlt_test_std_a, feat_hlt_test_std_c], axis=0)
        hlt_mask_test = np.concatenate([hlt_mask_test_a, hlt_mask_test_c], axis=0)
        const_off_test = np.concatenate([const_off_test_a, const_off_test_c], axis=0)
        const_hlt_test = np.concatenate([const_hlt_test_a, const_hlt_test_c], axis=0)
        budget_merge_test = np.concatenate([budget_merge_test_a, budget_merge_test_c], axis=0)
        budget_eff_test = np.concatenate([budget_eff_test_a, budget_eff_test_c], axis=0)
        budget_total_test = np.concatenate([budget_total_test_a, budget_total_test_c], axis=0)
        origin_counts_test = np.concatenate([origin_counts_test_a, origin_counts_test_c], axis=0)
        ...
        labels_test = labels_test_a
        feat_off_test_std = feat_off_test_std_a
        mask_off_test = mask_off_test_a
        feat_hlt_test_std = feat_hlt_test_std_a
        hlt_mask_test = hlt_mask_test_a
        const_off_test = const_off_test_a
        const_hlt_test = const_hlt_test_a
        budget_merge_test = budget_merge_test_a
        budget_eff_test = budget_eff_test_a
        budget_total_test = budget_total_test_a
        origin_counts_test = origin_counts_test_a
        hlt_stats = hlt_stats_a
    ...
    if use_fresh_stagec:
        ...
        feat_hlt_dual_test = feat_hlt_dual_a[test_idx_a]
    if bool(args.enable_jet_regressor):
        ...
        if use_fresh_stagec:
            ...
            jet_reg_test_pred = pred_log_a[test_idx_a]
        ...
        if use_fresh_stagec:
            ...
            jet_reg_test_true = target_off_a[test_idx_a]
    ...
    print("=" * 70)
    if bool(args.use_nopriv_total_budget):
        ...
        ds_train_reco = ReconstructionDataset(
            feat_hlt_std_a[train_idx_a], hlt_mask_a[train_idx_a], hlt_const_a[train_idx_a],
            const_off_a[train_idx_a], masks_off_a[train_idx_a],
            budget_merge_true_a[train_idx_a], budget_eff_true_a[train_idx_a],
        )
        ds_val_reco = ReconstructionDataset(
            feat_hlt_std_a[val_idx_a], hlt_mask_a[val_idx_a], hlt_const_a[val_idx_a],
            const_off_a[val_idx_a], masks_off_a[val_idx_a],
            budget_merge_true_a[val_idx_a], budget_eff_true_a[val_idx_a],
        )
    ...
    elif bool(args.enable_allocation_constraints):
        reconstructor, reco_val_metrics = train_reconstructor_priv_alloc(
            reconstructor,
            dl_train_reco,
            dl_val_reco,
            device,
    ...
    ds_train_joint_b = JointDualDataset(
        ...
        const_off_a[train_idx_a], masks_off_a[train_idx_a],
        budget_total_true_a[train_idx_a],
        budget_merge_true_a[train_idx_a],
        budget_eff_true_a[train_idx_a],
    ...
    ds_val_joint_b = JointDualDataset(
        ...
        const_off_a[val_idx_a], masks_off_a[val_idx_a],
        budget_total_true_a[val_idx_a],
        budget_merge_true_a[val_idx_a],
        budget_eff_true_a[val_idx_a],
        labels_a[val_idx_a],
    )
    ...
    ds_train_joint_c = JointDualDataset(
        ...
        budget_merge_true_c[train_idx_c],
        budget_eff_true_c[train_idx_c],
    ...
    ds_val_joint_c = JointDualDataset(
        ...
        budget_merge_true_c[val_idx_c],
        budget_eff_true_c[val_idx_c],
        labels_c[val_idx_c],
    )
    ...
    dual_joint = DualViewCrossAttnClassifier(
        ...
        input_dim_b=(12 if bool(args.use_corrected_flags) else 10),
        **cfg["model"],
    ).to(device)
    reconstructor, dual_joint, stageB_metrics = train_joint_dual(
        ...
        reco_loss_mode=("nopriv" if bool(args.use_nopriv_total_budget) else "priv"),
        enable_allocation_constraints=bool(args.enable_allocation_constraints),
        alloc_merge_share=0.5,
        alloc_quota_strength=1.0 if bool(args.enable_allocation_constraints) else 0.0,
        w_alloc_hard=float(cfg["loss"].get("w_alloc_hard", 0.0)),
        w_alloc_quota=float(cfg["loss"].get("w_alloc_quota", 0.0)),
    ...
    reconstructor, dual_joint, stageC_metrics = train_joint_dual(
        ...
        reco_loss_mode=("nopriv" if bool(args.use_nopriv_total_budget) else "priv"),
        enable_allocation_constraints=bool(args.enable_allocation_constraints),
        alloc_merge_share=float(cfg["reconstructor_training"].get("merge_share_final", 0.5)),
        alloc_quota_strength=1.0 if bool(args.enable_allocation_constraints) else 0.0,
        w_alloc_hard=float(cfg["loss"].get("w_alloc_hard", 0.0)),
        w_alloc_quota=float(cfg["loss"].get("w_alloc_quota", 0.0)),
    ...
    if use_fresh_stagec:
        ...
        feat_b_test = feat_b_a[test_idx_a]
        mask_b_test = mask_b_a[test_idx_a]
    ...
    test_split_metrics: Dict[str, Dict[str, Dict[str, float]]] = {}
    if use_fresh_stagec:
        n_test_a = int(len(test_idx_a))
        y_a = labels_test[:n_test_a].astype(np.float32)
        y_c = labels_test[n_test_a:].astype(np.float32)

        split_metrics = {
            "slice_a_test": {
                "teacher": metrics_from_preds(y_a, preds_teacher[:n_test_a]),
                "baseline": metrics_from_preds(y_a, preds_baseline[:n_test_a]),
                "joint": metrics_from_preds(y_a, preds_joint[:n_test_a]),
            },
            "slice_c_test": {
                "teacher": metrics_from_preds(y_c, preds_teacher[n_test_a:]),
                "baseline": metrics_from_preds(y_c, preds_baseline[n_test_a:]),
                "joint": metrics_from_preds(y_c, preds_joint[n_test_a:]),
            },
            "combined_test": {
                "teacher": {"auc": float(auc_teacher), "fpr30": float(fpr30_teacher), "fpr50": float(fpr50_teacher)},
                "baseline": {"auc": float(auc_baseline), "fpr30": float(fpr30_baseline), "fpr50": float(fpr50_baseline)},
                "joint": {"auc": float(auc_joint), "fpr30": float(fpr30_joint), "fpr50": float(fpr50_joint)},
            },
        }
        if preds_joint_kd is not None:
            split_metrics["slice_a_test"]["joint_kd"] = metrics_from_preds(y_a, preds_joint_kd[:n_test_a])
            split_metrics["slice_c_test"]["joint_kd"] = metrics_from_preds(y_c, preds_joint_kd[n_test_a:])
            split_metrics["combined_test"]["joint_kd"] = {
                "auc": float(auc_joint_kd),
                "fpr30": float(fpr30_joint_kd),
                "fpr50": float(fpr50_joint_kd),
            }
        test_split_metrics = split_metrics
    ...
    if use_fresh_stagec:
        print("\nPer-slice test metrics (fresh split mode):")
        for split_name in ("slice_a_test", "slice_c_test", "combined_test"):
            m = test_split_metrics[split_name]
            line = (
                f"  {split_name}: "
                f"Tchr AUC/FPR50={m['teacher']['auc']:.4f}/{m['teacher']['fpr50']:.6f}, "
                f"Base AUC/FPR50={m['baseline']['auc']:.4f}/{m['baseline']['fpr50']:.6f}, "
                f"Joint AUC/FPR50={m['joint']['auc']:.4f}/{m['joint']['fpr50']:.6f}"
            )
            if "joint_kd" in m:
                line += f", Joint+KD AUC/FPR50={m['joint_kd']['auc']:.4f}/{m['joint_kd']['fpr50']:.6f}"
            print(line)
    ...
    with open(save_root / "test_split_metrics.json", "w", encoding="utf-8") as f:
        json.dump(test_split_metrics, f, indent=2)
        ...
        json.dump(
            {
                ...
                "test_split_metrics": test_split_metrics,
                "test": {
