...
"""
Slow Offline -> HLT Transition Training.

Train a single classifier while gradually replacing offline jets with their
HLT counterparts. Start with 0% HLT (pure offline) and ramp to 100% HLT.
Validation and test are always on HLT.
"""
...
def train_epoch(model, loader, opt, device, hlt_frac):
    ...
    for batch in loader:
        ...
        use_hlt = (torch.rand(len(y), device=device) < hlt_frac).view(-1, 1, 1)
        x = torch.where(use_hlt, x_hlt, x_off)
...
def get_hlt_fraction(epoch, transition_epochs, start_frac, mode="linear", power=1.0):
    if transition_epochs <= 1:
        return 1.0
    progress = min(1.0, max(0.0, epoch / max(transition_epochs - 1, 1)))
    if mode == "cosine":
        frac = 0.5 * (1 - np.cos(np.pi * progress))
    elif mode == "power":
        frac = progress ** power
    elif mode == "sigmoid":
        k = max(1.0, power)
        frac = 1.0 / (1.0 + np.exp(-k * (progress * 2 - 1)))
    else:
        frac = progress
    return start_frac + (1.0 - start_frac) * frac
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default=str(Path().cwd() / "checkpoints" / "slow_transition"))
    ...
    print("\n" + "=" * 70)
    print("SLOW TRANSITION TRAINING (offline -> HLT)")
    print("=" * 70)

    for ep in tqdm(range(CONFIG["training"]["epochs"]), desc="Slow-Transition"):
        hlt_frac = get_hlt_fraction(
        ...
        history.append((ep + 1, hlt_frac, train_loss, train_auc, val_auc))
        ...
        if (ep + 1) % 5 == 0:
            print(f"Ep {ep+1}: hlt_frac={hlt_frac:.2f}, train_auc={train_auc:.4f}, val_auc={val_auc:.4f}, best={best_auc:.4f}")
    ...
    model_path = save_dir / "slow_transition.pt"
    if not args.skip_save_models:
        torch.save({"model": model.state_dict(), "auc": best_auc, "history": history}, model_path)

    auc_test, preds_test, labs = evaluate(model, test_loader, device, "hlt", "mask_hlt")
    print(f"\nTest AUC (HLT): {auc_test:.4f}")

    fpr, tpr, _ = roc_curve(labs, preds_test)
    plt.figure(figsize=(8, 6))
    plt.plot(tpr, fpr, "-", label=f"Slow Transition (AUC={auc_test:.3f})", color="steelblue", linewidth=2)
