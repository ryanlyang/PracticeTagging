...
"""
Offline reconstructor variant:
- Raises max split children to K=3.
- Uses soft gating so split mass is distributed across children, instead of
  independent per-child on/off gating that can over-activate all children.

Design:
- split_total_gate in [0, 1] controls total split activation per token.
- softmax(split_exist_logits) allocates that gate across K children.
- split_total_frac controls total child pT fraction per token (cap kept at 2/3).
- softmax(split_delta[...,0]) allocates that total split pT across K children.
"""
...
class OfflineReconstructorSplitK3SoftGate(reco_base.OfflineReconstructor):
    def __init__(
        self,
        input_dim: int = 7,
        embed_dim: int = 256,
        ...
        ff_dim: int = 1024,
        dropout: float = 0.1,
        max_split_children: int = 3,
        max_generated_tokens: int = 48,
    ):
        # Force K=3 for this ablation (ignore caller-provided K).
        super().__init__(
            input_dim=input_dim,
            embed_dim=embed_dim,
            num_heads=num_heads,
        ...
        self.split_total_gate_head = nn.Linear(embed_dim, 1)
        self.split_total_frac_head = nn.Linear(embed_dim, 1)
    ...
    def forward(
        ...
        # Split children from each HLT token (K=3 with soft gating).
        K = self.max_split_children
        split_exist_logits = self.split_exist_head(x)  # [B, L, K]
        split_alloc = torch.softmax(split_exist_logits, dim=-1)
        split_total_gate = torch.sigmoid(self.split_total_gate_head(x)).squeeze(-1)
        split_total_gate = split_total_gate * (p_split * stage_scale) * mask_hlt.float()
        split_exist = split_total_gate.unsqueeze(-1) * split_alloc

        split_delta = self.split_delta_head(x).view(B, L, K, 3)
        # Keep baseline total split-fraction cap at 2/3 to isolate K+gating effect.
        split_total_frac = (2.0 / 3.0) * torch.sigmoid(self.split_total_frac_head(x)).squeeze(-1)
        split_frac_alloc = torch.softmax(split_delta[..., 0], dim=-1)
        split_frac = split_total_frac.unsqueeze(-1) * split_frac_alloc
