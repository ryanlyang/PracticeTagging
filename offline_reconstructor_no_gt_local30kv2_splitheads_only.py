...
class OfflineReconstructorSplitHeadsOnly(reco_base.OfflineReconstructor):
    """
    Split-head expressivity variant (no context conditioning):
    - Replace linear split heads with MLP split heads.
    - Keep action/unsmear/reassign paths unchanged from base model.
    - Keep generation and budget paths unchanged.
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.split_exist_mlp = nn.Sequential(
            nn.Linear(embed_dim, split_hidden),
            nn.GELU(),
    ...
    def load_state_dict(self, state_dict: Dict[str, torch.Tensor], strict: bool = True):
        ...
        if strict and (incompatible.missing_keys or incompatible.unexpected_keys):
            print(
                "[OfflineReconstructorSplitHeadsOnly] Loaded non-strict state dict "
    ...
    def forward(
        ...
        # Expressive split heads (MLPs), no context conditioning.
        K = self.max_split_children
        split_exist = torch.sigmoid(self.split_exist_mlp(x))
        split_exist = split_exist * (p_split.unsqueeze(-1) * stage_scale) * mask_hlt.float().unsqueeze(-1)

        split_delta = self.split_delta_mlp(x).view(B, L, K, 3)
        ...
        # Jet context and budget heads (unchanged).
