...
class OfflineReconstructorCtxFilmConcat(reco_base.OfflineReconstructor):
    """
    Context-conditioning variant:
    - Compute jet context from pooled encoder tokens.
    - FiLM-modulate each token: h' = gamma(ctx) * h + beta(ctx).
    - Concatenate context to each token and project: h'' = MLP([h', ctx]).
    - Feed h'' to action/unsmear/reassign/split heads.

    Keeps the original head types/sizes to isolate context-conditioning effects.
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.ctx_film_head = nn.Sequential(
            nn.Linear(embed_dim, embed_dim * 2),
            nn.GELU(),
            nn.Linear(embed_dim * 2, embed_dim * 2),
        )
        self.ctx_concat_proj = nn.Sequential(
            nn.Linear(embed_dim * 2, embed_dim * 2),
            nn.GELU(),
    ...
    def load_state_dict(self, state_dict: Dict[str, torch.Tensor], strict: bool = True):
        ...
        if strict and (incompatible.missing_keys or incompatible.unexpected_keys):
            print(
                "[OfflineReconstructorCtxFilmConcat] Loaded non-strict state dict "
                f"(missing={len(incompatible.missing_keys)}, "
                f"unexpected={len(incompatible.unexpected_keys)})."
            )
        return incompatible

    def _condition_tokens(self, x: torch.Tensor, ctx: torch.Tensor) -> torch.Tensor:
        # FiLM modulation
        gb = self.ctx_film_head(ctx)  # [B, 2E]
        gamma, beta = torch.chunk(gb, 2, dim=-1)
        gamma = torch.tanh(gamma).unsqueeze(1)
        beta = torch.tanh(beta).unsqueeze(1)
        s = float(self.ctx_mod_scale)
        x_film = x * (1.0 + s * gamma) + s * beta

        # Concatenate context to each token and project back to token dim.
        ctx_tiled = ctx.unsqueeze(1).expand(-1, x.shape[1], -1)
        x_cat = torch.cat([x_film, ctx_tiled], dim=-1)
        return self.ctx_concat_proj(x_cat)
    ...
    def forward(
        ...
        # Jet context from encoder tokens.
        q = self.pool_query.expand(B, -1, -1)
        pooled, _ = self.pool_attn(q, x, x, key_padding_mask=~mask_safe, need_weights=False)
        ctx = pooled.squeeze(1)

        # Context-conditioned token features for correction heads.
        x_corr = self._condition_tokens(x, ctx)
        ...
        split_exist = torch.sigmoid(self.split_exist_head(x_corr))
        split_exist = split_exist * (p_split.unsqueeze(-1) * stage_scale) * mask_hlt.float().unsqueeze(-1)

        split_delta = self.split_delta_head(x_corr).view(B, L, K, 3)
