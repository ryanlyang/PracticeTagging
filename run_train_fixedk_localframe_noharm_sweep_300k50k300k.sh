#!/usr/bin/env bash
# Local-frame K=4 predictor with no-harm sweep.
# Runs 3 no-harm settings sequentially in one allocation.
#
# Submit:
...
#SBATCH --output=offline_reconstructor_logs/fixk_localframe_noharm_sweep_%j.out
#SBATCH --error=offline_reconstructor_logs/fixk_localframe_noharm_sweep_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs

BASE_RUN_NAME="${BASE_RUN_NAME:-fixedk_localframe_300k50k300k_k4}"
TRAIN_PATH="${TRAIN_PATH:-$SLURM_SUBMIT_DIR/data}"
SAVE_DIR="${SAVE_DIR:-checkpoints/fixedk_localframe_noharm_predictor}"
...
NOHARM_MARGIN="${NOHARM_MARGIN:-0.0}"
NOHARM_SWEEP="${NOHARM_SWEEP:-0.0,0.05,0.1}"
...
echo "=================================================="
echo "Fixed-K local-frame + no-harm sweep"
echo "=================================================="
...
IFS=',' read -r -a nh_vals <<< "$NOHARM_SWEEP"

for nh in "${nh_vals[@]}"; do
  nh_tag="$(tagify "$nh")"
  run_name="${BASE_RUN_NAME}_nh${nh_tag}"

  echo ""
  echo "--------------------------------------------------"
  echo "Running noharm_lambda=${nh} (run_name=${run_name})"
  echo "--------------------------------------------------"

  python train_fixedk_localframe_noharm_added_predictor.py \
...
echo ""
echo "Sweep completed. Outputs under: ${SAVE_DIR}"
