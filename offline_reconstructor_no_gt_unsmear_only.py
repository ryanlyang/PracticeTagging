...
"""
Unsmear-only HLT -> Offline reconstruction pipeline without constituent-to-constituent mapping.

What this script does:
1) Load offline jets from HDF5 (jet-level pairing only).
2) Generate pseudo-HLT jets with smearing only (no merging, no efficiency loss, no reassignment).
3) Train an unsmear-only reconstructor that predicts corrected per-constituent four-vectors.
   No unmerge split head and no efficiency-generation head are active at runtime.
4) Build reconstructed jets from model predictions (same constituent count as HLT).
...
def apply_hlt_effects_realistic_nomap(
    ...
    # 3) Efficiency model (disabled by default in unsmear-only mode).
    ...
    if bool(hcfg.get("efficiency_enabled", True)):
...
class OfflineReconstructor(nn.Module):
    ...
    def forward(
        ...
        action_prob_raw = torch.softmax(action_logits, dim=-1)
        # Unsmear-only mode: keep + unsmear, disable split/generation/reassign behavior.
        p_keep_raw = action_prob_raw[..., 0]
        p_unsmear_raw = action_prob_raw[..., 1]
        norm_ku = (p_keep_raw + p_unsmear_raw + eps)
        p_keep = p_keep_raw / norm_ku
        p_unsmear = p_unsmear_raw / norm_ku
        p_split = torch.zeros_like(p_keep)
        ...
        reassign_delta = torch.zeros((B, L, 2), device=x.device, dtype=x.dtype)

        d_logpt = stage_scale * p_unsmear * unsmear_delta[..., 0]
        d_eta = stage_scale * p_unsmear * unsmear_delta[..., 1]
        d_phi = stage_scale * p_unsmear * unsmear_delta[..., 2]
        ...
        # Keep one output token per valid HLT token.
        tok_weight = mask_hlt.float()
        tok_merge_flag = torch.zeros_like(tok_weight)
        tok_eff_flag = torch.zeros_like(tok_weight)

        # Split/generation branches are disabled in unsmear-only mode.
        K = self.max_split_children
        split_delta = torch.zeros((B, L, K, 3), device=x.device, dtype=x.dtype)
        child_tokens = torch.zeros((B, L * K, 4), device=x.device, dtype=x.dtype)
        child_weight = torch.zeros((B, L * K), device=x.device, dtype=x.dtype)
        child_merge_flag = torch.ones_like(child_weight)
        child_eff_flag = torch.zeros_like(child_weight)

        # Use HLT count budget exactly (no add/remove constituents in unsmear-only mode).
        budget_total = mask_hlt.float().sum(dim=1)
        budget_merge = torch.zeros_like(budget_total)
        budget_eff = torch.zeros_like(budget_total)

        gen_tokens = torch.zeros((B, self.max_generated_tokens, 4), device=x.device, dtype=x.dtype)
        gen_exist = torch.zeros((B, self.max_generated_tokens), device=x.device, dtype=x.dtype)
...
def main():
    ...
    parser.add_argument("--run_name", type=str, default="unsmear_only")
