...
"""
JetClass V2-Attr + m2-style HLT + confidence-gated ops reconstructor.

Purpose:
- Keep the V2 constrained attribute-head training/eval pipeline.
- Swap the base reconstructor to a new confidence-gated ops model.
...
def _find_practice_root() -> Path:
    ...
    required = [
        ...
        "offline_reconstructor_joint_dualview_stage2save_auc_norankc_nopriv_unmergeonly_confgen_ops.py",
...
def main() -> None:
    ...
    jetlatent = importlib.import_module(
        "offline_reconstructor_joint_dualview_stage2save_auc_norankc_nopriv_unmergeonly_confgen_ops"
    )
    ...
    # Generation should not be over-penalized: split sparse > gen sparse by default.
    stagea_w_sparse_split = float(os.environ.get("JETCLASS_STAGEA_W_SPARSE_SPLIT", args.loss_w_sparse))
    stagea_w_sparse_gen = float(
        os.environ.get("JETCLASS_STAGEA_W_SPARSE_GEN", max(0.0, 0.25 * float(args.loss_w_sparse)))
    )
    ...
    def _compose_stagea_fullinfo_losses(
        ...
        *,
        ...
        loss_cfg_local["w_sparse_split"] = stagea_w_sparse_split
        loss_cfg_local["w_sparse_gen"] = stagea_w_sparse_gen
        loss_cfg_local["w_gen_fp"] = stagea_w_gen_fp
        losses_reco = jetlatent.compute_reconstruction_losses_weighted_confgen_ops(
            reco_out,
    ...
    v2.compute_reconstruction_losses_weighted = jetlatent.compute_reconstruction_losses_weighted_confgen_ops
    v2.build_soft_corrected_view = jetlatent.build_soft_corrected_view_confgen_ops
    ...
    reco_joint.compute_reconstruction_losses_weighted = jetlatent.compute_reconstruction_losses_weighted_confgen_ops
