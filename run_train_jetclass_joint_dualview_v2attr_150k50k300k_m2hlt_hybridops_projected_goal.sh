#!/usr/bin/env bash
#SBATCH --job-name=jcM2Proj
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=256G
#SBATCH --time=1-00:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_150k50k300k_m2hlt_hybridops_projected_goal_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_150k50k300k_m2hlt_hybridops_projected_goal_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_v2attr_150k50k300k_m2hlt_hybridops_projected_goal_sps001}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
...
# m2-style HLT corruption profile (merge-favoring).
...
# Hybrid-ops + adaptive split/gen budget knobs.
RECO_MAX_GENERATED_TOKENS="${RECO_MAX_GENERATED_TOKENS:-56}"
LOSS_W_SET="${LOSS_W_SET:-1.0}"
LOSS_W_BUDGET="${LOSS_W_BUDGET:-0.60}"
LOSS_W_SPARSE="${LOSS_W_SPARSE:-0.010}"     # small anti-overgeneration pressure
LOSS_W_LOCAL="${LOSS_W_LOCAL:-0.08}"
LOSS_GEN_LOCAL_RADIUS="${LOSS_GEN_LOCAL_RADIUS:-0.08}"
LOSS_W_PT_RATIO="${LOSS_W_PT_RATIO:-0.18}"
...
GOAL_APPLY_SCALE="${GOAL_APPLY_SCALE:-1.0}"
GOAL_LAMBDA_HEAD="${GOAL_LAMBDA_HEAD:-0.35}"
GOAL_LAMBDA_RESPONSE="${GOAL_LAMBDA_RESPONSE:-0.60}"
GOAL_LAMBDA_AXIS="${GOAL_LAMBDA_AXIS:-0.35}"
GOAL_MAX_DLOGPT="${GOAL_MAX_DLOGPT:-1.25}"
GOAL_MAX_DETA="${GOAL_MAX_DETA:-0.55}"
GOAL_MAX_DPHI="${GOAL_MAX_DPHI:-0.55}"
...
SCRIPT="${SCRIPT:-$(pwd)/train_jetclass_joint_dualview_stage2_unmergeonly_m2hlt_hybrid_ops_projected_goal_v2_attr.py}"
...
CMD=(
  ...
  --goal_apply_scale "${GOAL_APPLY_SCALE}"
  --goal_lambda_head "${GOAL_LAMBDA_HEAD}"
  --goal_lambda_response "${GOAL_LAMBDA_RESPONSE}"
  --goal_lambda_axis "${GOAL_LAMBDA_AXIS}"
  --goal_max_dlogpt "${GOAL_MAX_DLOGPT}"
  --goal_max_deta "${GOAL_MAX_DETA}"
  --goal_max_dphi "${GOAL_MAX_DPHI}"
...
echo "============================================================"
echo "JetClass m2-style projected-goal hybrid reconstructor"
...
echo "Projection goal: apply_scale=${GOAL_APPLY_SCALE}, lambdas(head/response/axis)=${GOAL_LAMBDA_HEAD}/${GOAL_LAMBDA_RESPONSE}/${GOAL_LAMBDA_AXIS}"
echo "============================================================"
printf ' %q' "${CMD[@]}"
echo
"${CMD[@]}"
