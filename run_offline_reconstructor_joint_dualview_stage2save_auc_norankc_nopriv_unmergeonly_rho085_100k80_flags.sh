...
# - Added target uses conservative scaled true_added (default scale=0.85).
# - Merge/eff flag channels disabled.
#
# Submit:
#   sbatch run_offline_reconstructor_joint_dualview_stage2save_auc_norankc_nopriv_unmergeonly_rho085_100k80_flags.sh

#SBATCH --job-name=nrivU85
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=32G
#SBATCH --time=03:00:00
#SBATCH --output=offline_reconstructor_logs/offline_reco_nopriv_uo_rho085_%j.out
#SBATCH --error=offline_reconstructor_logs/offline_reco_nopriv_uo_rho085_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs

RUN_NAME="${RUN_NAME:-joint_100k_80c_stage2save_auc_norankc_nopriv_unmergeonly_rho085_noflags_noconf}"
N_TRAIN_JETS="${N_TRAIN_JETS:-100000}"
OFFSET_JETS="${OFFSET_JETS:-0}"
MAX_CONSTITS="${MAX_CONSTITS:-80}"
NUM_WORKERS="${NUM_WORKERS:-6}"
SAVE_DIR="${SAVE_DIR:-checkpoints/offline_reconstructor_joint_060}"
ADDED_TARGET_SCALE="${ADDED_TARGET_SCALE:-0.50}"
