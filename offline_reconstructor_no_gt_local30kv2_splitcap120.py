...
- Single change: split child pT cap is raised so total child pT can reach
  1.2 * parent pT (for K=2 => each child up to 0.6 * parent).
"""
...
class OfflineReconstructorSplitCap120(reco_base.OfflineReconstructor):
    def forward(
        self,
        ...
        # Single ablation change: raise total split cap from (K/(K+1)) to 1.2.
        split_frac = 1.2 * torch.sigmoid(split_delta[..., 0]) / float(max(K, 1))
