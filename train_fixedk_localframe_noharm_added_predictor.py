...
"""
Fixed-K additive predictor (K=4 by default) with Hungarian matching in local jet frame.
...
def compute_jet_anchors_np(const: np.ndarray, mask: np.ndarray) -> np.ndarray:
    ...
    jet_px = (px * w).sum(axis=1)
    jet_py = (py * w).sum(axis=1)
    jet_pz = (pz * w).sum(axis=1)
    jet_E = (E * w).sum(axis=1)
    ...
    anchors = np.stack([jet_pt, jet_eta, jet_phi, np.maximum(jet_E, 1e-8)], axis=-1).astype(np.float32)
    return anchors


def const_to_local_feat_np(const: np.ndarray, anchors: np.ndarray, eps: float = 1e-8) -> np.ndarray:
    jet_pt = np.maximum(anchors[:, 0:1], eps)
    jet_eta = anchors[:, 1:2]
    jet_phi = anchors[:, 2:3]
    jet_E = np.maximum(anchors[:, 3:4], eps)
    ...
    log_pt_rel = np.log(np.clip(pt / jet_pt, eps, 1e8))
    d_eta = eta - jet_eta
    d_phi = wrap_phi_np(phi - jet_phi)
    log_E_rel = np.log(np.clip(E / jet_E, eps, 1e8))

    feat = np.stack([log_pt_rel, d_eta, d_phi, log_E_rel], axis=-1)
    feat = np.nan_to_num(feat, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32)
    return feat


def train_classifier_optional_noharm(
    model: nn.Module,
    loader: DataLoader,
    ...
    for batch in loader:
        ...
        opt.zero_grad()
        logits = _model_logits(model, x, mask)
        loss_bce = F.binary_cross_entropy_with_logits(logits, y)
        loss = loss_bce

        if float(noharm_lambda) > 0.0 and "base_logit" in batch:
            base_logit = batch["base_logit"].to(device).view_as(logits)
            sign = 2.0 * y - 1.0
            violation = F.relu((sign * base_logit - float(noharm_margin)) - sign * logits)
            loss_noharm = violation.mean()
            loss = loss_bce + float(noharm_lambda) * loss_noharm
    ...
    return total_loss / max(len(preds), 1), float(auc)


def predict_logits_array(
    model: nn.Module,
    feat: np.ndarray,
    mask: np.ndarray,
    device: torch.device,
    ...
    ds = JetDataset(feat, mask, np.zeros((feat.shape[0],), dtype=np.float32))
    ...
    with torch.no_grad():
        for batch in dl:
            ...
            logits = _model_logits(model, x, m)
...
def train_single_view_classifier_auc(
    ...
    for ep in tqdm(range(int(train_cfg["epochs"])), desc=name):
        _, tr_auc = train_classifier_optional_noharm(
            model,
            train_loader,
            opt,
            device,
            noharm_lambda=float(noharm_lambda),
            noharm_margin=float(noharm_margin),
        )
...
class PredictorDataset(Dataset):
    def __init__(self, feat_hlt: np.ndarray, mask_hlt: np.ndarray, target_feat: np.ndarray):
        self.feat_hlt = torch.tensor(feat_hlt, dtype=torch.float32)
        self.mask_hlt = torch.tensor(mask_hlt, dtype=torch.bool)
        self.target_feat = torch.tensor(target_feat, dtype=torch.float32)
    ...
    def __getitem__(self, i: int):
        return {
            ...
            "target_feat": self.target_feat[i],
        }
...
def pred_to_const_from_local(
    pred_feat: torch.Tensor,
    anchors: torch.Tensor,
    pt_max: float = 1e8,
    e_max: float = 1e8,
) -> torch.Tensor:
    log_pt_rel = pred_feat[..., 0]
    d_eta = pred_feat[..., 1]
    ...
    jet_eta = anchors[..., 1]
    jet_phi = anchors[..., 2]
    jet_E = anchors[..., 3].clamp(min=1e-6)

    pt = (torch.exp(log_pt_rel) * jet_pt).clamp(min=1e-6, max=pt_max)
    eta = (d_eta + jet_eta).clamp(min=-5.0, max=5.0)
    phi = wrap_phi_t(d_phi + jet_phi)

    E_raw = (torch.exp(log_E_rel) * jet_E).clamp(min=1e-6, max=e_max)
...
def hungarian_loss(
    ...
    """
    pred_feat: [B,K,4] in local feature space [log_pt_rel, d_eta, d_phi, log_E_rel]
    target_feat: [B,K,4] same local feature space
    """
    device = pred_feat.device
...
class JetDatasetWithBaseline(Dataset):
    def __init__(self, feat: np.ndarray, mask: np.ndarray, labels: np.ndarray, base_logits: np.ndarray):
        self.feat = torch.tensor(feat, dtype=torch.float32)
        self.mask = torch.tensor(mask, dtype=torch.bool)
        self.label = torch.tensor(labels, dtype=torch.float32)
        self.base_logit = torch.tensor(base_logits, dtype=torch.float32)
...
def train_eval_tagger(
    ...
    pin = torch.cuda.is_available()
    if baseline_ref_logits is not None and float(noharm_lambda) > 0.0:
        ds_tr = JetDatasetWithBaseline(feat[train_idx], mask[train_idx], labels[train_idx], baseline_ref_logits[train_idx])
        ds_va = JetDatasetWithBaseline(feat[val_idx], mask[val_idx], labels[val_idx], baseline_ref_logits[val_idx])
        ds_te = JetDatasetWithBaseline(feat[test_idx], mask[test_idx], labels[test_idx], baseline_ref_logits[test_idx])
        dl_tr = DataLoader(ds_tr, batch_size=bs, shuffle=True, drop_last=True, num_workers=num_workers, pin_memory=pin)
        dl_va = DataLoader(ds_va, batch_size=bs, shuffle=False, num_workers=num_workers, pin_memory=pin)
        dl_te = DataLoader(ds_te, batch_size=bs, shuffle=False, num_workers=num_workers, pin_memory=pin)
    else:
    ...
    model = train_single_view_classifier_auc(
        model,
        dl_tr,
        dl_va,
        device,
...
def predict_added_tokens(
    ...
    with torch.no_grad():
        for s in range(0, n, batch_size):
            ...
            ab = torch.tensor(anchors_hlt[s:e], dtype=torch.float32, device=device)
            pred_feat = model(xb, mb)
            pred_const = pred_to_const_from_local(pred_feat, ab)
...
def joint_finetune_predictor_and_tagger_alternating(
    ...
    ds_pred_tr = PredictorDataset(feat_hlt_std[train_idx], hlt_mask[train_idx], target_feat[train_idx])
    ds_pred_va = PredictorDataset(feat_hlt_std[val_idx], hlt_mask[val_idx], target_feat[val_idx])
    ...
    for ep in range(1, int(joint_epochs) + 1):
        ...
        if baseline_ref_logits is not None and float(noharm_lambda) > 0.0:
            ds_cls_tr = JetDatasetWithBaseline(
                feat_aug_tr,
                mask_aug_tr,
                labels[train_idx],
                baseline_ref_logits[train_idx],
            )
            ds_cls_va = JetDatasetWithBaseline(
                feat_aug_va,
                mask_aug_va,
                labels[val_idx],
                baseline_ref_logits[val_idx],
            )
        else:
        ...
        cls_train_loss, cls_train_auc = train_classifier_optional_noharm(
            tagger,
            dl_cls_tr,
            opt_tagger,
            device,
...
def main() -> None:
    ...
    parser.add_argument("--k_fixed", type=int, default=4)
    ...
    parser.add_argument("--noharm_lambda", type=float, default=0.0)
    parser.add_argument("--noharm_margin", type=float, default=0.0)
    ...
    anchors_hlt = compute_jet_anchors_np(hlt_const, hlt_mask)
    ...
    baseline_logits_hlt = predict_logits_array(
        model=baseline,
        feat=feat_hlt_std,
        mask=hlt_mask,
        device=device,
    ...
    target_feat = const_to_local_feat_np(target_const, anchors_hlt)
    ...
    ds_pred_val = PredictorDataset(feat_hlt_std[val_idx], hlt_mask[val_idx], target_feat[val_idx])
    ...
    # ----------------- Tagger evals ----------------- #
    print("\n" + "=" * 70)
    print("Training tagger on HLT + Predicted-K view (pre-joint)")
    print("=" * 70)
    pred_metrics_prejoint, pred_tagger_model = train_eval_tagger(
        feat=feat_aug_pred_std,
        mask=aug_pred_mask,
        labels=labels,
        train_idx=train_idx,
        val_idx=val_idx,
        ...
        device=device,
        num_workers=nw,
        name=f"Added-Predicted-K{k_fixed}-PreJoint",
        return_model=True,
    ...
    torch.save(predictor.state_dict(), save_root / "predictor_prejoint.pt")
    torch.save(pred_tagger_model.state_dict(), save_root / "tagger_prejoint.pt")

    pred_metrics_postjoint = pred_metrics_prejoint
    joint_history: List[Dict[str, float]] = []
    if bool(args.enable_joint_finetune):
        print("\n" + "=" * 70)
        print("Joint fine-tuning predictor + added-view tagger (alternating)")
        print("=" * 70)
        predictor, pred_tagger_model, joint_history = joint_finetune_predictor_and_tagger_alternating(
            predictor=predictor,
            tagger=pred_tagger_model,
            feat_hlt_std=feat_hlt_std,
            hlt_const=hlt_const,
            hlt_mask=hlt_mask,
            anchors_hlt=anchors_hlt,
            ...
            pred_weight_decay=float(args.pred_weight_decay),
            aug_max_constits=int(aug_max_constits),
            loss_w={
                "logpt": float(args.loss_w_logpt),
                "eta": float(args.loss_w_eta),
            ...
            joint_patience=int(args.joint_patience),
            joint_lr_pred=float(args.joint_lr_pred),
            joint_lr_tagger=float(args.joint_lr_tagger),
            baseline_ref_logits=baseline_logits_hlt,
            noharm_lambda=float(args.noharm_lambda),
            noharm_margin=float(args.noharm_margin),
            supervision_mode=str(args.supervision_mode),
        ...
        pred_metrics_postjoint = eval_tagger_model_on_arrays(
            model=pred_tagger_model,
            feat=feat_aug_post_te,
            mask=mask_aug_post_te,
        ...
        torch.save(predictor.state_dict(), save_root / "predictor_postjoint.pt")
        torch.save(pred_tagger_model.state_dict(), save_root / "tagger_postjoint.pt")

    print("\n" + "=" * 70)
    print("Training tagger on HLT + Oracle-K view (reference)")
    print("=" * 70)
    oracle_metrics = train_eval_tagger(
    ...
    summary = {
        ...
        "predicted_added_prejoint": {
            **pred_metrics_prejoint,
            "recovery_auc": rec_auc(pred_metrics_prejoint["auc"]),
            "recovery_fpr50": rec_fpr50(pred_metrics_prejoint["fpr50"]),
        },
        "predicted_added_postjoint": {
            **pred_metrics_postjoint,
            "recovery_auc": rec_auc(pred_metrics_postjoint["auc"]),
            "recovery_fpr50": rec_fpr50(pred_metrics_postjoint["fpr50"]),
        },
        "oracle_added": {
            **oracle_metrics,
        ...
        "joint_enabled": bool(args.enable_joint_finetune),
        "k_fixed": int(k_fixed),
        "mean_added_pred_tokens": float(np.mean(np.sum(pred_add_const[:, :, 0] > 0, axis=1))),
    ...
    with open(save_root / "joint_history.json", "w", encoding="utf-8") as f:
        json.dump(joint_history, f, indent=2)
    ...
    print(
        f"Pred-K{k_fixed} PreJoint: AUC={pred_metrics_prejoint['auc']:.4f} FPR50={pred_metrics_prejoint['fpr50']:.6f} "
        f"RecAUC={summary['predicted_added_prejoint']['recovery_auc']:.3f} "
        f"RecFPR50={summary['predicted_added_prejoint']['recovery_fpr50']:.3f}"
    )
    print(
        f"Pred-K{k_fixed} PostJoint: AUC={pred_metrics_postjoint['auc']:.4f} FPR50={pred_metrics_postjoint['fpr50']:.6f} "
        f"RecAUC={summary['predicted_added_postjoint']['recovery_auc']:.3f} "
        f"RecFPR50={summary['predicted_added_postjoint']['recovery_fpr50']:.3f}"
    )
    print(
        f"Oracle-K{k_fixed}: AUC={oracle_metrics['auc']:.4f} FPR50={oracle_metrics['fpr50']:.6f} "
        f"RecAUC={summary['oracle_added']['recovery_auc']:.3f} "
        f"RecFPR50={summary['oracle_added']['recovery_fpr50']:.3f}"
    )
