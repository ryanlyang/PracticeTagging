...
6) Select checkpoints by val AUC (with optional earliest-within-tolerance AUC policy).
...
def train_joint_dual(
    ...
    sel_epoch = -1
    sel_policy = str(select_metric).lower()
    no_improve = 0
    store_auc_records = str(select_metric).lower() == "auc" and float(auc_early_tol) > 0.0
    auc_improve_records = []  # records only epochs that improved best AUC
    ...
    for ep in tqdm(range(int(epochs)), desc=stage_name):
        ...
        if np.isfinite(va_auc) and float(va_auc) > best_val_auc:
            ...
            if store_auc_records:
                auc_improve_records.append(
                    {
                        "epoch": int(ep + 1),
                        "val_auc": float(va_auc),
                        "val_fpr50": float(va_fpr50),
                        "dual": best_state_dual_auc,
                        "reco": best_state_reco_auc,
                    }
                )
    ...
    # Optional policy: among AUC-improving checkpoints, choose the earliest checkpoint
    # within a small tolerance of the best observed AUC.
    if store_auc_records and len(auc_improve_records) > 0 and np.isfinite(best_val_auc):
        auc_threshold = float(best_val_auc) - float(auc_early_tol)
        chosen = None
        for rec in auc_improve_records:
            if float(rec["val_auc"]) >= auc_threshold:
                chosen = rec
                break
        if chosen is None:
            chosen = auc_improve_records[-1]
        best_state_dual_sel = chosen["dual"]
        best_state_reco_sel = chosen["reco"]
        sel_val_auc = float(chosen["val_auc"])
        sel_val_fpr50 = float(chosen["val_fpr50"])
        sel_epoch = int(chosen["epoch"])
        sel_policy = "auc_earlytol"
        print(
            f"{stage_name}: selecting earliest checkpoint within AUC tolerance "
            f"(tol={float(auc_early_tol):.6f}) of best AUC={float(best_val_auc):.6f}; "
            f"picked epoch {sel_epoch} (val_auc={sel_val_auc:.6f}, val_fpr50={sel_val_fpr50:.6f})."
        )
    ...
    metrics = {
        ...
        "selection_policy": str(sel_policy),
        "auc_early_tol": float(auc_early_tol),
        "selected_epoch": int(sel_epoch),
...
def main() -> None:
    ...
    parser.add_argument("--stageB_auc_early_tol", type=float, default=0.001)
    ...
    parser.add_argument("--stageC_auc_early_tol", type=float, default=0.0)
    ...
    reconstructor, dual_joint, stageB_metrics, stageB_states = train_joint_dual(
        ...
        auc_early_tol=float(args.stageB_auc_early_tol),
    )
    ...
    reconstructor, dual_joint, stageC_metrics, stageC_states = train_joint_dual(
        ...
        select_metric=selection_metric,
        auc_early_tol=float(args.stageC_auc_early_tol),
    )
