...
but with a hard per-token cap on unsmear-induced log-scale pT/E updates.

Key behavior:
- Reassign behavior remains ORIGINAL (including its existing coupling into d_logpt/d_logE).
- d_logpt and d_logE are softly hard-capped via: cap * tanh(raw / cap).
  This preserves small updates and bounds large drifts.

Cap controls (environment variables):
...
class OfflineReconstructorUnsmearCap(reco_base.OfflineReconstructor):
    """
    Same architecture/params/state_dict keys as base OfflineReconstructor.
    Behavior change: cap d_logpt/d_logE magnitudes to reduce unsmear drift.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        pt_cap = float(os.environ.get("UNSMEAR_LOGPT_CAP", "0.25"))
        e_cap_raw = os.environ.get("UNSMEAR_LOGE_CAP", str(pt_cap))
        e_cap = float(e_cap_raw)
        self.unsmear_logpt_cap = max(pt_cap, 0.0)
        self.unsmear_loge_cap = max(e_cap, 0.0)

    @staticmethod
    def _soft_cap(x: torch.Tensor, cap: float) -> torch.Tensor:
        if cap <= 0.0:
            return x
        return float(cap) * torch.tanh(x / float(cap))
    ...
    def forward(
        ...
        # Keep original behavior (reassign also contributes to log-scale pT/E updates),
        # then cap update magnitudes to prevent large drift.
        d_logpt_raw = stage_scale * (p_unsmear + 0.5 * p_reassign) * unsmear_delta[..., 0]
        d_logE_raw = stage_scale * (p_unsmear + 0.5 * p_reassign) * unsmear_delta[..., 3]

        d_logpt = self._soft_cap(d_logpt_raw, self.unsmear_logpt_cap)
        d_logE = self._soft_cap(d_logE_raw, self.unsmear_loge_cap)
