...
- Single change: budget calibration high clamp is raised from 4.0 -> 6.0
  for both child_scale and gen_scale.
"""
...
class OfflineReconstructorBudgetClamp6(reco_base.OfflineReconstructor):
    def forward(
        self,
        ...
        # Single ablation change: high clamp 4.0 -> 6.0
        child_scale = (budget_merge.unsqueeze(1) / child_sum).clamp(min=0.25, max=6.0)
        gen_scale = (budget_eff.unsqueeze(1) / gen_sum).clamp(min=0.25, max=6.0)
