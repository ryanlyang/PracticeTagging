...
"""
JetClass V2-Attr + m2-style HLT + projected-goal hybrid-ops reconstructor.
...
def main() -> None:
    ...
    wrapper_parser.add_argument("--goal_apply_scale", type=float, default=1.0)
    ...
    class OfflineReconstructorProjectedGoalHybridOps(jetlatent.OfflineReconstructorHybridOps):
        """
        HLT-only global target head plus hard vector-sum projection on the hybrid operation branches.

        The head predicts a jet-level correction and projects operation candidates so the weighted candidate cloud
        matches the predicted pT and axis by construction. Split/generate/reassign/unsmear all
        ...
        def _target_from_goal(
            ...
            scale = goal_apply_scale * max(0.0, min(float(stage_scale), 1.0))
    ...
    v2.OfflineReconstructor = OfflineReconstructorProjectedGoalHybridOps
    ...
    reco_joint.OfflineReconstructor = OfflineReconstructorProjectedGoalHybridOps
