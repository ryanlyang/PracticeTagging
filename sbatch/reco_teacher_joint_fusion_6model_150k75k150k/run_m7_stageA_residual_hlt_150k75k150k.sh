#!/usr/bin/env bash
#SBATCH --job-name=m7res
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=48G
#SBATCH --time=10:00:00
#SBATCH --output=offline_reconstructor_logs/reco_teacher_joint_fusion_6model_150k75k150k/m7_stageA_residual_%j.out
#SBATCH --error=offline_reconstructor_logs/reco_teacher_joint_fusion_6model_150k75k150k/m7_stageA_residual_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs/reco_teacher_joint_fusion_6model_150k75k150k

RUN_NAME="${RUN_NAME:-model7_stageA_residual_hlt_150k75k150k_seed0}"
SAVE_DIR="${SAVE_DIR:-checkpoints/reco_teacher_joint_fusion_6model_150k75k150k/model7_stageA_residual_hlt}"
SEED="${SEED:-0}"
DEVICE="${DEVICE:-cuda}"
...
RESIDUAL_JOINT_EPOCHS="${RESIDUAL_JOINT_EPOCHS:-12}"
...
echo "============================================================"
echo "Model-7 StageA RecoTeacher + Residual(HLT + alpha*r_hat)"
echo "Run: ${SAVE_DIR}/${RUN_NAME}"
echo "Split: train=${N_TRAIN_SPLIT}, val=${N_VAL_SPLIT}, test=${N_TEST_SPLIT}"
echo "Residual joint epochs: ${RESIDUAL_JOINT_EPOCHS}"
echo "============================================================"
printf ' %q' "${CMD[@]}"
echo
"${CMD[@]}"
