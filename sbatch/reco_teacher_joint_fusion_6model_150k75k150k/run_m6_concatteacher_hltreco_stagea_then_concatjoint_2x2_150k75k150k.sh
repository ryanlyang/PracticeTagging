#!/usr/bin/env bash
#SBATCH --job-name=m6c2x2
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=48G
#SBATCH --time=9:00:00
#SBATCH --output=offline_reconstructor_logs/reco_teacher_joint_fusion_6model_150k75k150k/m6_concat_hltreco_2x2_%j.out
#SBATCH --error=offline_reconstructor_logs/reco_teacher_joint_fusion_6model_150k75k150k/m6_concat_hltreco_2x2_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs/reco_teacher_joint_fusion_6model_150k75k150k

RUN_NAME="${RUN_NAME:-model6_concat_hltreco_stagea_concatjoint_2x2_150k75k150k_seed0}"
SAVE_DIR="${SAVE_DIR:-checkpoints/reco_teacher_joint_fusion_6model_150k75k150k/model6_concat_hltreco_stagea_concatjoint_2x2}"
SEED="${SEED:-0}"
DEVICE="${DEVICE:-cuda}"
...
STAGEA_PHASE035_EPOCHS="${STAGEA_PHASE035_EPOCHS:-2}"
STAGEA_PHASE070_EPOCHS="${STAGEA_PHASE070_EPOCHS:-2}"
...
CMD=(
  ...
  --stageA_epochs 115
  --stageA_patience 18
  --stageA_phase035_epochs "${STAGEA_PHASE035_EPOCHS}"
  --stageA_phase070_epochs "${STAGEA_PHASE070_EPOCHS}"
  --stageA_kd_temp 2.5
...
echo "============================================================"
echo "Model-6 variant: StageA teacher loss on concat(HLT+reco) vs concat(HLT+offline)"
echo "Then concat(HLT+reco) single-view joint finetune"
echo "Run: ${SAVE_DIR}/${RUN_NAME}"
echo "Split: train=${N_TRAIN_SPLIT}, val=${N_VAL_SPLIT}, test=${N_TEST_SPLIT}"
echo "StageA custom curriculum: phase_035=${STAGEA_PHASE035_EPOCHS}, phase_070=${STAGEA_PHASE070_EPOCHS}"
echo "============================================================"
printf ' %q' "${CMD[@]}"
echo
"${CMD[@]}"
