#!/usr/bin/env bash
#SBATCH --job-name=m21seedpj
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --time=1-12:00:00
#SBATCH --output=offline_reconstructor_logs/reco_teacher_joint_fusion_6model_150k75k150k/m21_dualview_seeded_postjoint_%j.out
#SBATCH --error=offline_reconstructor_logs/reco_teacher_joint_fusion_6model_150k75k150k/m21_dualview_seeded_postjoint_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs/reco_teacher_joint_fusion_6model_150k75k150k

RUN_NAME="${RUN_NAME:-model21_dualview_seeded_m6m2_postjoint_multijoint_150k75k150k_seed0}"
SAVE_DIR="${SAVE_DIR:-checkpoints/reco_teacher_joint_fusion_6model_150k75k150k/model21_dualview_seeded_m6m2_postjoint}"
SEED="${SEED:-0}"
DEVICE="${DEVICE:-cuda}"
NUM_WORKERS="${NUM_WORKERS:-6}"

# Source runs (load post-joint reco checkpoints from these run dirs)
...
M6_RECO_CKPT="${M6_RECO_CKPT:-offline_reconstructor_stageA.pt}"
...
echo "============================================================"
echo "Model-21 Seeded DualView POSTJOINT (m6 + m2 Hungarian)"
