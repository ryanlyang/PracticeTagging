...
if [[ -n "${DEP_JOB_IDS:-}" ]]; then
  ...
  dep_opt=(--export=ALL,STEP1_LOAD_DIR="${STEP1_LOAD_DIR}")
  if [[ -n "${STEP1_DEP_JOB_ID}" ]]; then
    dep_opt=(--dependency="afterok:${STEP1_DEP_JOB_ID}" --export=ALL,STEP1_LOAD_DIR="${STEP1_LOAD_DIR}")
  fi

  j2a=$(sbatch "${dep_opt[@]}" "${RUN_M2D005}" | awk '{print $4}')
  j2b=$(sbatch "${dep_opt[@]}" "${RUN_M2D020}" | awk '{print $4}')
  j4=$(sbatch "${dep_opt[@]}" "${RUN_M4}" | awk '{print $4}')
  j5=$(sbatch "${dep_opt[@]}" "${RUN_M5}" | awk '{print $4}')
  j6=$(sbatch "${dep_opt[@]}" "${RUN_M6}" | awk '{print $4}')
  j9m=$(sbatch "${dep_opt[@]}" "${RUN_M9MID}" | awk '{print $4}')
  j9h=$(sbatch "${dep_opt[@]}" "${RUN_M9HIGH}" | awk '{print $4}')
  j12=$(sbatch "${dep_opt[@]}" "${RUN_M12}" | awk '{print $4}')
  j15m=$(sbatch "${dep_opt[@]}" "${RUN_M15MID}" | awk '{print $4}')
  j15h=$(sbatch "${dep_opt[@]}" "${RUN_M15HIGH}" | awk '{print $4}')
  j16=$(sbatch "${dep_opt[@]}" "${RUN_M16}" | awk '{print $4}')
  j17=$(sbatch "${dep_opt[@]}" "${RUN_M17}" | awk '{print $4}')
...
if [[ -n "${STEP1_DEP_JOB_ID}" ]]; then
  echo "Training jobs were submitted with STEP1 dependency: ${STEP1_DEP_JOB_ID}"
fi
