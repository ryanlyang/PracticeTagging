...
SAVE_DIR="${SAVE_DIR:-checkpoints/reco_teacher_joint_fusion_6model_150k75k150k/model2_joint_delta005_fulltrain_prog_unfreeze_oracle_study}"
...
CMD=(
  python offline_reconstructor_joint_dualview_stage2save_auc_norankc_nopriv_unmergeonly_oracle_study.py
  --save_dir "${SAVE_DIR}"
  --run_name "${RUN_NAME}"
  ...
  --oracle_study_variant "${ORACLE_VARIANT}"
  --oracle_unsmear_keep_thresh 0.03
  --oracle_w_action 0.15
  --oracle_w_tok 0.20
  --oracle_w_split 0.20
  --save_fusion_scores
  --disable_final_kd
...
if [[ "${ORACLE_VARIANT}" == "A" ]]; then
  CMD+=(--oracle_eval_interventions_for_A)
fi

echo "============================================================"
echo "Model-2 oracle study A/B/C/D/E (ptclamp baseline, normal clamp)"
