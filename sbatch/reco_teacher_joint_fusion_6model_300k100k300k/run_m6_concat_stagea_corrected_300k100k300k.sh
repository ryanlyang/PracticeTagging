#!/usr/bin/env bash
#SBATCH --job-name=m6c300
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=96G
#SBATCH --time=1-12:00:00
#SBATCH --output=offline_reconstructor_logs/reco_teacher_joint_fusion_6model_300k100k300k/m6_concat_%j.out
#SBATCH --error=offline_reconstructor_logs/reco_teacher_joint_fusion_6model_300k100k300k/m6_concat_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs/reco_teacher_joint_fusion_6model_300k100k300k

RUN_NAME="${RUN_NAME:-model6_concat_stagea_corrected_300k100k300k_seed0}"
SAVE_DIR="${SAVE_DIR:-checkpoints/reco_teacher_joint_fusion_6model_300k100k300k/model6_concat_stagea_corrected}"
SEED="${SEED:-0}"
DEVICE="${DEVICE:-cuda}"
