#!/usr/bin/env bash
#SBATCH --job-name=ctA75c
#SBATCH --partition=debug
#SBATCH --gres=gpu:1
#SBATCH --mem=48G
#SBATCH --time=1-00:00:00
#SBATCH --output=offline_reconstructor_logs/reco_teacher_joint_fusion_debug/concat_stageA_corr_%j.out
#SBATCH --error=offline_reconstructor_logs/reco_teacher_joint_fusion_debug/concat_stageA_corr_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs/reco_teacher_joint_fusion_debug

RUN_NAME="${RUN_NAME:-concat_teacher_stageA_then_corrected_75k25k150k_seed0}"
SAVE_DIR="${SAVE_DIR:-checkpoints/reco_teacher_joint_fusion_debug/concat_stagea_corrected}"
SEED="${SEED:-0}"
DEVICE="${DEVICE:-cuda}"
...
echo "============================================================"
echo "ConcatTeacher StageA -> CorrectedOnly"
