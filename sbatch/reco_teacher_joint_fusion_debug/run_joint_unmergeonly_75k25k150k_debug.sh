#!/usr/bin/env bash
#SBATCH --job-name=ju75d
#SBATCH --partition=debug
#SBATCH --gres=gpu:1
#SBATCH --mem=48G
#SBATCH --time=1-00:00:00
#SBATCH --output=offline_reconstructor_logs/reco_teacher_joint_fusion_debug/joint_unmergeonly_%j.out
#SBATCH --error=offline_reconstructor_logs/reco_teacher_joint_fusion_debug/joint_unmergeonly_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs/reco_teacher_joint_fusion_debug

RUN_NAME="${RUN_NAME:-joint_unmergeonly_75k25k150k_seed0}"
SAVE_DIR="${SAVE_DIR:-checkpoints/reco_teacher_joint_fusion_debug/joint}"
SEED="${SEED:-0}"
DEVICE="${DEVICE:-cuda}"
...
echo "============================================================"
echo "Joint unmergeonly run (separate from RecoTeacher)"
