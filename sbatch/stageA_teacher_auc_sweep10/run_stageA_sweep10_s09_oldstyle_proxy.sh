#!/usr/bin/env bash
#SBATCH --job-name=sA09old
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=96G
#SBATCH --time=36:00:00
#SBATCH --output=offline_reconstructor_logs/stageA_teacher_auc_sweep10/s09_oldproxy_%j.out
#SBATCH --error=offline_reconstructor_logs/stageA_teacher_auc_sweep10/s09_oldproxy_%j.err

set -euo pipefail
mkdir -p offline_reconstructor_logs/stageA_teacher_auc_sweep10

export RUN_NAME="joint_stageA_sweep10_s09_oldstyle_proxy_seed0"
...
export STAGEA_LAMBDA_KD=5.0
export STAGEA_LAMBDA_EMB=0.0
export STAGEA_LAMBDA_TOK=0.0
export STAGEA_LAMBDA_PHYS=0.05
export STAGEA_LAMBDA_BUDGET_HINGE=1.0
