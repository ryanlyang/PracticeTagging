#!/usr/bin/env bash
#SBATCH --job-name=sA10lng
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=96G
#SBATCH --time=36:00:00
#SBATCH --output=offline_reconstructor_logs/stageA_teacher_auc_sweep10/s10_longA_%j.out
#SBATCH --error=offline_reconstructor_logs/stageA_teacher_auc_sweep10/s10_longA_%j.err

set -euo pipefail
mkdir -p offline_reconstructor_logs/stageA_teacher_auc_sweep10

export RUN_NAME="joint_stageA_sweep10_s10_longer_stageA_seed0"
...
export STAGEA_EPOCHS=120
export STAGEA_PATIENCE=24
export STAGEA_LOSS_NORM_EMA_DECAY=0.995
export STAGEA_KD_TEMP=2.5
export STAGEA_LAMBDA_KD=1.4
export STAGEA_LAMBDA_EMB=1.8
export STAGEA_LAMBDA_TOK=1.0
export STAGEA_LAMBDA_PHYS=0.15
