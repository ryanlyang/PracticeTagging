...
RUNNER_TRAIN="${RUNNER_TRAIN:-run_train_jetclass_joint_dualview_v2attr_500k250k1m_m2hlt_hybridops_projected_goal.sh}"
RUNNER_FUSION="${RUNNER_FUSION:-run_analyze_jetclass_six_model_stacked_fusion_500k250k1m_m2projected_goal_selected6.sh}"

for f in "${RUNNER_TRAIN}" "${RUNNER_FUSION}"; do
  [[ -f "${f}" ]] || { echo "Missing runner: ${f}" >&2; exit 1; }
done

CORE03="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_projected_goal_${TAG}_core03_budgetlite"
CORE07="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_projected_goal_${TAG}_core07_splitlight"
CORE08="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_projected_goal_${TAG}_core08_reassignstrong"
CORE10="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_projected_goal_${TAG}_core10_offdrophigh"
CORE11="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_projected_goal_${TAG}_core11_topk60ish"
CORE12="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_projected_goal_${TAG}_core12_antioverlap"

OUT_DIR="${OUT_DIR:-${SAVE_DIR}/fusion_reports/six_model_500k250k1m_m2projected_goal_selected6_${TAG}_stacked_acc}"
...
echo "Submitting selected-6 fixed-HLT JetClass m2-projected-goal 500k/250k/1M pipeline on ${PARTITION}"
