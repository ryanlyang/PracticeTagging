...
#SBATCH --mem=32G
#SBATCH --time=5:00:00
#SBATCH --output=offline_reconstructor_logs/offline_reco_uo_specbucket_50k20k100k100_%j.out
#SBATCH --error=offline_reconstructor_logs/offline_reco_uo_specbucket_50k20k100k100_%j.err
...
echo "python offline_reconstructor_joint_dualview_stage2save_auc_norankc_nopriv_unmergeonly_specialist_bucket.py --save_dir ${SAVE_DIR} --run_name ${RUN_NAME} --n_train_jets ${N_TRAIN_JETS} --n_train_split ${N_TRAIN_SPLIT} --n_val_split ${N_VAL_SPLIT} --n_test_split ${N_TEST_SPLIT} --offset_jets ${OFFSET_JETS} --max_constits ${MAX_CONSTITS} --num_workers ${NUM_WORKERS} --seed ${SEED} --selection_metric auc --stageB_lambda_rank 0.0 --stageB_lambda_cons 0.0 --stageC_lr_dual 1e-5 --stageC_lr_reco 5e-6 --lambda_reco 0.4 --lambda_cons 0.06 --added_target_scale ${ADDED_TARGET_SCALE} --spec_bucket_enable --spec_bucket_count_low ${SPEC_COUNT_LOW} --spec_bucket_count_high ${SPEC_COUNT_HIGH} --spec_bucket_p_hlt_threshold ${SPEC_PHLT_THR} --spec_bucket_w_neg ${SPEC_W_NEG} --spec_bucket_w_pos ${SPEC_W_POS} --spec_bucket_w_other ${SPEC_W_OTHER} --disable_final_kd --device cuda"
