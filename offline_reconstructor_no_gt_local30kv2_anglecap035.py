...
- Identical to base architecture/forward
- Single change: split child angle shift cap is raised from 0.25 -> 0.35
  for both eta and phi split deltas.
"""
...
class OfflineReconstructorAngleCap035(reco_base.OfflineReconstructor):
    def forward(
        self,
        ...
        # Single ablation change: 0.25 -> 0.35
        child_eta = (eta.unsqueeze(-1) + 0.35 * torch.tanh(split_delta[..., 1])).clamp(min=-5.0, max=5.0)
        child_phi = wrap_phi_t(phi.unsqueeze(-1) + 0.35 * torch.tanh(split_delta[..., 2]))
