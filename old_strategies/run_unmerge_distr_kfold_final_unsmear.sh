...
KFOLD_RANDOM_VALTEST=${KFOLD_RANDOM_VALTEST:-0}
...
if [ "$KFOLD_RANDOM_VALTEST" -eq 1 ]; then
  CMD="$CMD --kfold_random_valtest --kfold_random_mode $KFOLD_RANDOM_MODE --kfold_random_seed $KFOLD_RANDOM_SEED"
fi
...
if [ $EXIT_CODE -eq 0 ]; then
  ...
  for i in "${!TAGS[@]}"; do
    ...
    CMD_CLS="python unmerge_distr_model_unsmear.py \
      --save_dir $SAVE_DIR \
    ...
    if [ "$UNSMEAR_MERGE_FLAG" -eq 1 ]; then
      CMD_CLS="$CMD_CLS --unsmear_merge_flag --unsmear_merge_weight $UNSMEAR_MERGE_WEIGHT"
    fi
    if [ "$UNSMEAR_TWO_HEAD" -eq 1 ]; then
      CMD_CLS="$CMD_CLS --unsmear_two_head"
    fi
