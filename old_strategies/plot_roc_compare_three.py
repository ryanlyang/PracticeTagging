...
parser.add_argument('--checkpoint3', type=str, required=True, help='Path to third model checkpoint directory')
...
plt.plot(metrics3['tpr'], metrics3['fpr'], ":", label=args.label3, color='forestgreen', linewidth=2)
plt.ylabel(r"False Positive Rate", fontsize=12)
plt.xlabel(r"True Positive Rate (Signal efficiency)", fontsize=12)
plt.legend(fontsize=12, frameon=False)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(plots_dir / 'roc_compare_three.png', dpi=300)
print(f"Saved ROC plot to {plots_dir / 'roc_compare_three.png'}")


# Background rejection comparison
pt_bins1 = metrics1['pt_bins'] / 1000
pt_bins2 = metrics2['pt_bins'] / 1000
pt_bins3 = metrics3['pt_bins'] / 1000
...
plt.plot(pt_bins3, metrics3['binned_br'], ":", drawstyle='steps-post',
         label=args.label3, color='forestgreen', linewidth=2)
plt.ylabel(r"Background rejection", fontsize=12)
plt.xlabel(r"Jet $p_T$ [GeV]", fontsize=12)
plt.legend(fontsize=12, frameon=False)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(plots_dir / 'background_rejection_compare_three.png', dpi=300)
print(f"Saved BR plot to {plots_dir / 'background_rejection_compare_three.png'}")
...
print(f"\n{args.label3}:")
print(f"  Background rejection at {metrics3['wp']} signal efficiency: {metrics3['br']:.2f}")
