...
# Import tf_keras for compatibility with energyflow
import tf_keras
...
if tagger_type == 'efn':
    ...
    # REDUCED model size for laptop testing - original: Phi_sizes=(350,350,350,350,350), F_sizes=(300,300,300,300,300)
    model = EFN(
        input_dim=2,
        ...
        optimizer=tf_keras.optimizers.Adam(learning_rate=6.3e-5),
        output_dim=1,
        output_act='sigmoid',
        summary=False,
        compile_opts={'weighted_metrics': [tf_keras.metrics.BinaryAccuracy(name='accuracy')]}
    )
...
# DNN and PFN datasets can be constructed similarly
    ...
    if tagger_type == 'dnn':
        ...
        model.add(tf_keras.Input(shape=train_data.shape[1:]))
        ...
        for i in range(5):
            model.add(tf_keras.layers.Dense(
                ...
                kernel_regularizer=tf_keras.regularizers.l1(l1=2e-4)
            ))
            model.add(tf_keras.layers.BatchNormalization(axis=1))
            model.add(tf_keras.layers.ReLU())
        ...
        # Compile DNN
        model.compile(
            optimizer=tf_keras.optimizers.Adam(learning_rate=1.2e-5),
            ...
            weighted_metrics=[tf_keras.metrics.BinaryAccuracy(name='accuracy')]
        )
        ...
        model = PFN(
            ...
            F_dropouts=0.022,
            loss="binary_crossentropy",
            optimizer=tf_keras.optimizers.Adam(learning_rate=7.9e-5),
            output_dim=1,
            output_act='sigmoid',
            summary=False,
            compile_opts={'weighted_metrics': [tf_keras.metrics.BinaryAccuracy(name='accuracy')]}
