...
"""
Super-Resolution (SR) front-end for HLT jets with probabilistic generator.
...
Stages:
  ...
  3) Pretrain G to map HLT -> offline-like features (mu + sigma per token)
...
CONFIG = {
    ...
    "generator": {
        ...
        "lr": 5e-4,
        "epochs": 30,
        # Interpreted as NLL weight for probabilistic reconstruction
        "lambda_feat": 1.0,
        "lambda_mask": 0.5,
        "lambda_mult": 0.1,
        "lambda_stats": 0.1,
...
class SuperResGenerator(nn.Module):
    def __init__(self, input_dim=7, embed_dim=64, num_heads=4, num_layers=2, ff_dim=256,
        ...
        self.out_log_sigma = nn.Linear(embed_dim, input_dim)
    ...
    def forward(self, x, mask, z=None):
        ...
        mu = self.out_mu(h_dec)
        log_sigma = self.out_log_sigma(h_dec)
        mask_logits = self.out_mask(h_dec).squeeze(-1)
        return mu, log_sigma, mask_logits, z_mu, z_logvar

    def sample(self, x, mask, n_samples=1, mask_threshold=0.5, min_sigma=0.02):
        views = []
        masks = []
        probs = []
        for _ in range(n_samples):
            mu, log_sigma, mask_logits, _, _ = self(x, mask)
...
def nll_gaussian(mu, log_sigma, target, mask, min_sigma=0.02, eps=1e-8):
    sigma = F.softplus(log_sigma) + min_sigma
    var = sigma ** 2
    diff2 = (target - mu) ** 2
    nll = 0.5 * (diff2 / (var + eps) + 2.0 * torch.log(sigma + eps))
    mask_f = mask.float().unsqueeze(-1)
...
def train_generator_epoch(generator, loader, opt, device, cfg):
    ...
    for batch in loader:
        ...
        opt.zero_grad()
        mu, log_sigma, mask_logits, z_mu, z_logvar = generator(x_hlt, m_hlt)
        p_exist = torch.sigmoid(mask_logits)

        loss_feat = nll_gaussian(mu, log_sigma, x_off, m_off, min_sigma=cfg["generator"]["min_sigma"])
        ...
        loss_stats = generator_stats_loss(mu, x_off, m_off)
...
def train_superres_epoch(generator, classifier, teacher, loader, opt, device, cfg,
    ...
    for batch in loader:
        ...
        for _ in range(cfg["superres"]["n_samples"]):
            mu, log_sigma, mask_logits, z_mu, z_logvar = generator(x_hlt, m_hlt)
            sigma = F.softplus(log_sigma) + cfg["generator"]["min_sigma"]
            eps = torch.randn_like(mu)
            x_hat = mu + sigma * eps
            p_exist = torch.sigmoid(mask_logits)
            x_hat = x_hat * p_exist.unsqueeze(-1)
            ...
            loss_feat = loss_feat + nll_gaussian(mu, log_sigma, x_off, m_off, min_sigma=cfg["generator"]["min_sigma"])
            ...
            loss_stats = loss_stats + generator_stats_loss(mu, x_off, m_off)
            ...
            if use_kd and teacher is not None:
                ...
                if use_conf:
                    ...
                    loss_kd = loss_kd + kd_loss_basic(t_logits_hat, t_logits_true, temperature)
                if a_rep > 0:
...
def fit_superres(generator, classifier, teacher, train_loader, val_loader, device, cfg,
    ...
    for ep in tqdm(range(cfg["superres"]["epochs"]), desc=label):
        ...
        if (ep + 1) % 5 == 0:
            print(f"Ep {ep+1}: train_auc={tr_auc:.4f}, val_auc={val_auc:.4f}, best={best_auc:.4f}, T={temp:.2f}, alpha={alpha:.2f}")
...
def evaluate_superres(generator, classifier, loader, device, cfg):
    ...
    for batch in loader:
        ...
        views, masks, _ = generator.sample(
            x_hlt, m_hlt,
            n_samples=cfg["superres"]["n_samples_eval"],
