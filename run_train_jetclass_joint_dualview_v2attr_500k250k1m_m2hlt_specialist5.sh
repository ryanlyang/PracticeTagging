#!/usr/bin/env bash
#SBATCH --job-name=jcSp5
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=224G
#SBATCH --time=2-00:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_500k250k1m_m2hlt_specialist5_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_500k250k1m_m2hlt_specialist5_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_v2attr_500k250k1m_m2hlt_specialist5_generalist}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
...
# Fixed m2-style HLT profile. The submit script exports these explicitly so every
# specialist sees the same HLT view; defaults here match that fixed profile.
...
# General hybrid-op reconstruction knobs.
RECO_MAX_GENERATED_TOKENS="${RECO_MAX_GENERATED_TOKENS:-56}"
LOSS_W_SET="${LOSS_W_SET:-1.0}"
LOSS_W_BUDGET="${LOSS_W_BUDGET:-0.70}"
LOSS_W_SPARSE="${LOSS_W_SPARSE:-0.010}"
LOSS_W_LOCAL="${LOSS_W_LOCAL:-0.08}"
LOSS_GEN_LOCAL_RADIUS="${LOSS_GEN_LOCAL_RADIUS:-0.08}"
LOSS_W_PT_RATIO="${LOSS_W_PT_RATIO:-0.12}"
LOSS_W_M_RATIO="${LOSS_W_M_RATIO:-0.02}"
LOSS_W_E_RATIO="${LOSS_W_E_RATIO:-0.02}"
LOSS_W_PHYS="${LOSS_W_PHYS:-0.00}"
ADDED_TARGET_SCALE="${ADDED_TARGET_SCALE:-0.90}"

# Specialist-only knobs. Neutral defaults preserve the generalist behavior.
SPECIALIST_EDIT_DELTA_SCALE="${SPECIALIST_EDIT_DELTA_SCALE:-1.0}"
SPECIALIST_SPLIT_WEIGHT_SCALE="${SPECIALIST_SPLIT_WEIGHT_SCALE:-1.0}"
SPECIALIST_GEN_WEIGHT_SCALE="${SPECIALIST_GEN_WEIGHT_SCALE:-1.0}"
SPECIALIST_LOSS_W_AXIS="${SPECIALIST_LOSS_W_AXIS:-0.0}"
SPECIALIST_LOSS_W_SPLIT_SPARSE="${SPECIALIST_LOSS_W_SPLIT_SPARSE:-0.0}"
SPECIALIST_LOSS_W_GEN_SPARSE="${SPECIALIST_LOSS_W_GEN_SPARSE:-0.0}"
SPECIALIST_LOSS_W_RADIAL_PROFILE="${SPECIALIST_LOSS_W_RADIAL_PROFILE:-0.0}"
...
if missing:
    raise SystemExit("[preflight] Missing modules: " + ", ".join(missing))
PY
...
CMD=(
  ...
  --specialist_edit_delta_scale "${SPECIALIST_EDIT_DELTA_SCALE}"
  --specialist_split_weight_scale "${SPECIALIST_SPLIT_WEIGHT_SCALE}"
  --specialist_gen_weight_scale "${SPECIALIST_GEN_WEIGHT_SCALE}"
  --specialist_loss_w_axis "${SPECIALIST_LOSS_W_AXIS}"
  --specialist_loss_w_split_sparse "${SPECIALIST_LOSS_W_SPLIT_SPARSE}"
  --specialist_loss_w_gen_sparse "${SPECIALIST_LOSS_W_GEN_SPARSE}"
  --specialist_loss_w_radial_profile "${SPECIALIST_LOSS_W_RADIAL_PROFILE}"
...
echo "============================================================"
echo "JetClass m2-hybrid specialist reconstructor (500k/250k/1m)"
echo "Job ID: ${SLURM_JOB_ID:-N/A}"
echo "Node: ${SLURMD_NODENAME:-N/A}"
echo "Run: ${SAVE_DIR}/${RUN_NAME}"
echo "Split: train=${N_TRAIN_JETS}, val=${N_VAL_JETS}, test=${N_TEST_JETS}"
echo "Fixed HLT: pt=${HLT_PT_THRESHOLD} merge=${MERGE_PROB_SCALE} reassign=${REASSIGN_SCALE} smear=${SMEAR_SCALE} eff=${EFF_PLATEAU_BARREL}/${EFF_PLATEAU_ENDCAP} width=${EFF_WIDTH_PT}"
echo "Specialist scales: edit=${SPECIALIST_EDIT_DELTA_SCALE} split=${SPECIALIST_SPLIT_WEIGHT_SCALE} gen=${SPECIALIST_GEN_WEIGHT_SCALE}"
echo "Specialist losses: axis=${SPECIALIST_LOSS_W_AXIS} split_sparse=${SPECIALIST_LOSS_W_SPLIT_SPARSE} gen_sparse=${SPECIALIST_LOSS_W_GEN_SPARSE} radial=${SPECIALIST_LOSS_W_RADIAL_PROFILE}"
echo "============================================================"
printf ' %q' "${CMD[@]}"
echo
"${CMD[@]}"
