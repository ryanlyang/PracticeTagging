#!/usr/bin/env bash
#SBATCH --job-name=jcFuseTgt
#SBATCH --partition=debug
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=12
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_fused_targets_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_fused_targets_%j.err
...
OUT_DIR="${OUT_DIR:-checkpoints/jetclass_joint_dualview/fused_targets/$(basename "${RUN_A_DIR}")__AND__$(basename "${RUN_B_DIR}")}"
DEVICE="${DEVICE:-cuda}"
...
CMD=(
  python -u build_jetclass_fused_targets_two_model.py
  --run_a_dir "${RUN_A_DIR}"
  --run_b_dir "${RUN_B_DIR}"
  --data_dir "${DATA_DIR}"
  --out_dir "${OUT_DIR}"
...
echo "============================================================"
echo "Build JetClass Fused Targets"
