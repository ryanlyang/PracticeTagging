...
class OfflineReconstructorSplitCtx(reco_base.OfflineReconstructor):
    """
    Drop-in replacement for OfflineReconstructor with:
    1) More expressive split heads (MLPs instead of single linear layers).
    2) Jet-context conditioning via FiLM-style token modulation before correction heads.

    This class is intentionally backward-compatible with existing constructor args.
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.ctx_mod_scale = 0.25

        self.ctx_mod_head = nn.Sequential(
            nn.Linear(embed_dim, embed_dim * 2),
            nn.GELU(),
            nn.Linear(embed_dim * 2, embed_dim * 2),
        )

        split_in = embed_dim * 2
        split_hidden = embed_dim * 2
        self.split_exist_mlp = nn.Sequential(
            nn.Linear(split_in, split_hidden),
            nn.GELU(),
            nn.LayerNorm(split_hidden),
            nn.Linear(split_hidden, self.max_split_children),
        )
        self.split_delta_mlp = nn.Sequential(
            nn.Linear(split_in, split_hidden),
            nn.GELU(),
            nn.LayerNorm(split_hidden),
            nn.Linear(split_hidden, self.max_split_children * 3),
        )

    def load_state_dict(self, state_dict: Dict[str, torch.Tensor], strict: bool = True):
        # Allow warm-starting from old checkpoints that do not have the new split/context modules.
        incompatible = super().load_state_dict(state_dict, strict=False)
        if strict and (incompatible.missing_keys or incompatible.unexpected_keys):
            print(
                "[OfflineReconstructorSplitCtx] Loaded non-strict state dict "
                f"(missing={len(incompatible.missing_keys)}, "
                f"unexpected={len(incompatible.unexpected_keys)})."
            )
        return incompatible

    def _modulate_tokens_with_context(self, x: torch.Tensor, ctx: torch.Tensor) -> torch.Tensor:
        gb = self.ctx_mod_head(ctx)  # [B, 2E]
        gamma, beta = torch.chunk(gb, 2, dim=-1)
        gamma = torch.tanh(gamma).unsqueeze(1)
        beta = torch.tanh(beta).unsqueeze(1)
        s = float(self.ctx_mod_scale)
        return x * (1.0 + s * gamma) + s * beta
    ...
    def forward(
        ...
        # Jet context from encoder tokens (same mechanism as budget pooling).
        q = self.pool_query.expand(B, -1, -1)
        pooled, _ = self.pool_attn(q, x, x, key_padding_mask=~mask_safe, need_weights=False)
        ctx = pooled.squeeze(1)

        # Context-conditioned tokens for correction heads.
        x_corr = self._modulate_tokens_with_context(x, ctx)

        action_logits = self.action_head(x_corr)
        ...
        unsmear_delta = self.unsmear_head(x_corr)
        reassign_delta = 0.35 * torch.tanh(self.reassign_head(x_corr))
        ...
        # Expressive split heads with context-conditioned features.
        K = self.max_split_children
        ctx_tiled = ctx.unsqueeze(1).expand(-1, L, -1)
        split_in = torch.cat([x_corr, ctx_tiled], dim=-1)
        split_exist = torch.sigmoid(self.split_exist_mlp(split_in))
        split_exist = split_exist * (p_split.unsqueeze(-1) * stage_scale) * mask_hlt.float().unsqueeze(-1)

        split_delta = self.split_delta_mlp(split_in).view(B, L, K, 3)
        ...
        # Keep generation path unchanged for stability.
