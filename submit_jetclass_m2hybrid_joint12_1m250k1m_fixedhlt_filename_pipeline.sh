...
TAG="${TAG:-fixedhlt_filename}"
CORE01="jetclass_joint_v2attr_1m250k1m_m2hlt_hybridops_adaptivegen_${TAG}_core01_base"
CORE02="jetclass_joint_v2attr_1m250k1m_m2hlt_hybridops_adaptivegen_${TAG}_core02_consstrong"
CORE03="jetclass_joint_v2attr_1m250k1m_m2hlt_hybridops_adaptivegen_${TAG}_core03_budgetlite"
CORE04="jetclass_joint_v2attr_1m250k1m_m2hlt_hybridops_adaptivegen_${TAG}_core04_genlow"
CORE05="jetclass_joint_v2attr_1m250k1m_m2hlt_hybridops_adaptivegen_${TAG}_core05_genhigh"
...
OUT_DIR="${OUT_DIR:-${SAVE_DIR}/fusion_reports/twelve_model_1m250k1m_m2hybrid_${TAG}_stacked_acc}"
OPTIMIZE_FOR="${OPTIMIZE_FOR:-acc}"

# One fixed train/val/test split, one fixed HLT simulation profile, one seed.
COMMON_EXPORT="SAVE_DIR=${SAVE_DIR},SEED=52,N_TRAIN_JETS=1000000,N_VAL_JETS=250000,N_TEST_JETS=1000000,FEATURE_PREPROCESSING=canonical,CLASS_ASSIGNMENT=filename,TARGET_CLASS=Hbb,BACKGROUND_CLASS=QCD,HLT_PT_THRESHOLD=1.30,MERGE_PROB_SCALE=1.35,REASSIGN_SCALE=1.00,SMEAR_SCALE=1.00,EFF_PLATEAU_BARREL=0.99,EFF_PLATEAU_ENDCAP=0.97,EFF_TURNON_PT=1.40,EFF_WIDTH_PT=0.20"
...
j01=$(submit_train "jcFHA01" "RUN_NAME=${CORE01}")
echo "  CORE01 ${j01} ${CORE01}"

j02=$(submit_train "jcFHA02" "RUN_NAME=${CORE02},LAMBDA_CONS=0.06,LOSS_W_BUDGET=0.80,STAGEC_LR_DUAL=1e-4,STAGEC_LR_RECO=5e-5")
echo "  CORE02 ${j02} ${CORE02}"

j03=$(submit_train "jcFHA03" "RUN_NAME=${CORE03},LOSS_W_BUDGET=0.55,LOSS_W_PT_RATIO=0.18,LOSS_W_SPARSE=0.012")
echo "  CORE03 ${j03} ${CORE03}"

j04=$(submit_train "jcFHA04" "RUN_NAME=${CORE04},RECO_MAX_GENERATED_TOKENS=40,LOSS_W_SPARSE=0.012")
echo "  CORE04 ${j04} ${CORE04}"

j05=$(submit_train "jcFHA05" "RUN_NAME=${CORE05},RECO_MAX_GENERATED_TOKENS=72,LOSS_W_SPARSE=0.006")
echo "  CORE05 ${j05} ${CORE05}"

j06=$(submit_train "jcFHA06" "RUN_NAME=${CORE06},LOSS_W_LOCAL=0.12,LOSS_W_BUDGET=0.75")
echo "  CORE06 ${j06} ${CORE06}"

j07=$(submit_train "jcFHA07" "RUN_NAME=${CORE07},LOSS_W_LOCAL=0.05,LOSS_W_BUDGET=0.65")
...
j12=$(submit_train "jcFHA12" "RUN_NAME=${CORE12},LOSS_W_LOCAL=0.14,LOSS_GEN_LOCAL_RADIUS=0.06,LOSS_W_SPARSE=0.013")
