#!/usr/bin/env bash
#SBATCH --job-name=jcFS2R
#SBATCH --partition=debug
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=12
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_confgen_v2attr_50k25k100k_v1hltplus25_fusedstudent_stagea_teacher_recoonlydual_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_confgen_v2attr_50k25k100k_v1hltplus25_fusedstudent_stagea_teacher_recoonlydual_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_confgen_v2attr_50k25k100k_v1hltplus25_fusedstudent_stagea_teacher_recoonlydual}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
...
FUSED_STUDENT_CKPT="${FUSED_STUDENT_CKPT:-checkpoints/jetclass_joint_dualview/jetclass_offline_fused_student_50k25k100k_v1hltplus25_pair/offline_fused_student.pt}"
...
if [[ ! -f "${FUSED_STUDENT_CKPT}" ]]; then
  echo "Missing fused-student checkpoint: ${FUSED_STUDENT_CKPT}" >&2
  echo "Set FUSED_STUDENT_CKPT=/path/to/offline_fused_student.pt" >&2
  exit 1
fi
...
# Keep generator lightly penalized.
...
# Teacher-dominant Stage-A using frozen offline fused student.
export JETCLASS_STAGEA_OFFLINE_FUSED_STUDENT_CKPT="${FUSED_STUDENT_CKPT}"
...
echo "============================================================"
echo "JetClass confgen run with Stage-A fused-student teacher"
echo "Job ID: ${SLURM_JOB_ID:-N/A}"
echo "Node: ${SLURMD_NODENAME:-N/A}"
echo "Run: ${SAVE_DIR}/${RUN_NAME}"
echo "Teacher ckpt: ${FUSED_STUDENT_CKPT}"
