#!/usr/bin/env bash
#SBATCH --job-name=nrivUO9C08
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=128G
#SBATCH --time=3-12:00:00
#SBATCH --output=offline_reconstructor_logs/stagec_uo_rho090_chunk08_%j.out
#SBATCH --error=offline_reconstructor_logs/stagec_uo_rho090_chunk08_%j.err
...
manifest="offline_reconstructor_logs/stagec_unmergeonly_rho090_chunk08_runs_${SLURM_JOB_ID:-manual}.tsv"
...
run_one() {
  ...
  echo "[chunk08] (${idx}/4) Starting $run_name"
  set +e
  RUN_NAME="$run_name" \
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    echo "[chunk08] FAILED on $run_name (exit=$exit_code). Stopping chunk." >&2
    exit "$exit_code"
  fi
  echo "[chunk08] Completed $run_name"
}

echo "Running 4 sequential Stage-C runs for chunk08"
run_one 1 1.05 0.04 A 2e-5 1e-5 0.50
run_one 2 1.05 0.04 B 3e-5 1.5e-5 0.60
run_one 3 1.05 0.06 A 2e-5 1e-5 0.50
run_one 4 1.05 0.06 B 3e-5 1.5e-5 0.60


echo "Chunk 08 complete. Manifest: $manifest"
