...
"""
m29: Jet-latent set2set + K-hypothesis routing/selector experiments.

Built as a thin fork over the m2 jet-latent wrapper:
- keeps the same base m2 A/B/C pipeline,
- adds K-hypothesis reconstructor outputs (default K=6),
- adds a selector trained on reconstructed hypotheses,
- routes corrected-view features through selector picks,
- supports Stage-C strategy modes:
  - all_three: train reco+dual (selector fixed after refresh)
  - selector_dual: freeze reco, train dual (selector fixed after refresh)
  - selector_only: refresh selector only, skip Stage-C dual/reco updates
...
class HypothesisSelector(nn.Module):
    ...
    def forward(self, feat_bkf: torch.Tensor) -> torch.Tensor:
        b, k, f = feat_bkf.shape
        x = feat_bkf.reshape(b * k, f)
        y = self.net(x).reshape(b, k)
        return y
...
def _fit_selector_from_loaders(
    ...
    def collect(loader: DataLoader) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if len(feat_all) == 0:
            return np.zeros((0, int(M29_STATE.opts.num_hypotheses), 7), dtype=np.float32), np.zeros((0,), dtype=np.int64)
        return np.concatenate(feat_all, axis=0), np.concatenate(tgt_all, axis=0)

    feat_tr, y_tr = collect(train_loader)
    feat_va, y_va = collect(val_loader)
    if feat_tr.shape[0] == 0 or feat_va.shape[0] == 0:
        print("m29 selector: empty train/val feature set; skipping selector fit")
        return

    selector = HypothesisSelector(feat_dim=int(feat_tr.shape[-1]), hidden=64).to(device)
    ...
    tr_dl = DataLoader(TensorDataset(xtr, ytr), batch_size=512, shuffle=True)
    ...
    for ep in range(int(max(M29_STATE.opts.selector_epochs, 1))):
        ...
        for xb, yb in tr_dl:
            ...
            loss = loss_ce + float(M29_STATE.opts.selector_rank_weight) * loss_rank
            loss.backward()
    ...
    print(f"m29 selector trained: val_acc={best_acc:.4f}, Ntr={feat_tr.shape[0]}, Nva={feat_va.shape[0]}")
