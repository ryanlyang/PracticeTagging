...
"""
Super-Resolution (SR) front-end for HLT jets with a conditional flow generator.

Pipeline:
  x_hlt -> G (flow-based SR generator) -> x_hat_off -> C (offline-style classifier) -> y_hat

Stages:
  1) Train offline teacher T on true offline (optional but recommended)
  2) Train HLT baseline (reference)
  3) Pretrain G with conditional flow NLL + mask/shape losses
  4) Pretrain C on true offline (warm start)
  5) End-to-end SR training:
     - SR-only: supervised + flow regularization + consistency
...
CONFIG = {
    ...
    "generator": {
        ...
        "lr": 5e-4,
        "epochs": 30,
        "lambda_feat": 1.0,  # flow NLL weight
        "lambda_mask": 0.5,
        "lambda_mult": 0.1,
        "lambda_stats": 0.1,
    },
...
class ConditionalAffineCoupling(nn.Module):
    def __init__(self, in_dim, ctx_dim, hidden_dim, mask, scale=0.8):
        super().__init__()
        self.register_buffer("mask", mask)
        self.scale = scale
        self.net = nn.Sequential(
            nn.Linear(in_dim + ctx_dim, hidden_dim),
            nn.GELU(),
...
class ConditionalRealNVP(nn.Module):
    def __init__(self, in_dim, ctx_dim, hidden_dim, n_layers=6, scale=0.8):
        super().__init__()
        self.in_dim = in_dim
        self.ctx_dim = ctx_dim
        masks = []
        base = torch.arange(in_dim) % 2
        for i in range(n_layers):
            mask = base if i % 2 == 0 else 1 - base
            masks.append(mask.float())
        self.layers = nn.ModuleList(
            [ConditionalAffineCoupling(in_dim, ctx_dim, hidden_dim, m, scale=scale) for m in masks]
        )
...
class SuperResGenerator(nn.Module):
    ...
    def encode(self, x, mask):
        ...
        pooled = pooled.unsqueeze(1).expand(-1, seq_len, -1)
        ctx = torch.cat([h, pooled], dim=-1)
        ctx = self.ctx_proj(ctx)
        mask_logits = self.out_mask(h).squeeze(-1)
        return ctx, mask_logits

    def log_prob(self, x_off, ctx, mask):
        bsz, seq_len, _ = x_off.shape
        x_flat = x_off.reshape(-1, self.input_dim)
        ctx_flat = ctx.reshape(-1, ctx.size(-1))
        z, log_det = self.flow(x_flat, ctx_flat)
        log_p_z = -0.5 * (z ** 2 + np.log(2.0 * np.pi)).sum(dim=-1)
        log_p = log_p_z + log_det
        log_p = log_p.view(bsz, seq_len)
        mask_f = mask.float()
        denom = mask_f.sum().clamp(min=1.0)
        return (log_p * mask_f).sum() / denom

    def sample_from_context(self, ctx):
        bsz, seq_len, _ = ctx.shape
        z = torch.randn(bsz * seq_len, self.input_dim, device=ctx.device)
        x_hat = self.flow.inverse(z, ctx.reshape(-1, ctx.size(-1)))
        return x_hat.view(bsz, seq_len, self.input_dim)

    def sample(self, x, mask, n_samples=1, mask_threshold=0.5):
        views = []
        masks = []
        probs = []
        ctx, mask_logits = self.encode(x, mask)
        p_exist = torch.sigmoid(mask_logits)
        for _ in range(n_samples):
            x_hat = self.sample_from_context(ctx)
...
def train_generator_epoch(generator, loader, opt, device, cfg):
    ...
    for batch in loader:
        ...
        opt.zero_grad()
        ctx, mask_logits = generator.encode(x_hlt, m_hlt)
        p_exist = torch.sigmoid(mask_logits)
        x_hat = generator.sample_from_context(ctx)
        x_hat = x_hat * p_exist.unsqueeze(-1)

        loss_nll = -generator.log_prob(x_off, ctx, m_off)
        ...
        loss = (
            cfg["generator"]["lambda_feat"] * loss_nll
...
def main():
    ...
    parser.add_argument("--gen_flow_layers", type=int, default=CONFIG["generator"]["flow_layers"])
    parser.add_argument("--gen_flow_hidden", type=int, default=CONFIG["generator"]["flow_hidden"])
    parser.add_argument("--gen_flow_scale", type=float, default=CONFIG["generator"]["flow_scale"])
    ...
    CONFIG["generator"]["flow_layers"] = args.gen_flow_layers
    CONFIG["generator"]["flow_hidden"] = args.gen_flow_hidden
    CONFIG["generator"]["flow_scale"] = args.gen_flow_scale
    ...
    generator = SuperResGenerator(
        ...
        flow_layers=CONFIG["generator"]["flow_layers"],
        flow_hidden=CONFIG["generator"]["flow_hidden"],
        flow_scale=CONFIG["generator"]["flow_scale"],
    ).to(device)
