...
Multi-view unmerge pipeline:
  ...
  4) Build N unmerged datasets with stochastic variation.
  5) Train teacher (offline), baseline (HLT), unmerge model (multi-view),
     and unmerge+KD (multi-view).
  6) Evaluate with multi-view averaged predictions.
"""
        ...
        with torch.no_grad():
            for i in range(0, len(samples), batch_size):
                ...
                for k in range(len(chunk)):
                    ...
                    if noise_pt > 0:
                        pred[:, 0] *= rng.normal(1.0, noise_pt, size=c)
                    if noise_eta > 0:
                        pred[:, 1] += rng.normal(0.0, noise_eta, size=c)
                    if noise_phi > 0:
                        pred[:, 2] += rng.normal(0.0, noise_phi, size=c)
...
@torch.no_grad()
def evaluate_student_views(model, feats_list, masks_list, labels, idx, device, batch_size):
    preds_views = []
    for feat, mask in zip(feats_list, masks_list):
        ds = JetDataset(feat[idx], mask[idx], labels[idx])
        loader = DataLoader(ds, batch_size=batch_size, shuffle=False)
        _, preds, _ = eval_classifier(model, loader, device)
        preds_views.append(preds)
    preds_mean = np.mean(np.stack(preds_views, axis=0), axis=0)
    auc = roc_auc_score(labels[idx], preds_mean) if len(np.unique(labels[idx])) > 1 else 0.0
    return auc, preds_mean
...
def main():
    ...
    parser.add_argument("--n_unmerge_views", type=int, default=8)
    parser.add_argument("--unmerge_noise_pt", type=float, default=0.02)
    parser.add_argument("--unmerge_noise_eta", type=float, default=0.005)
    parser.add_argument("--unmerge_noise_phi", type=float, default=0.005)
    parser.add_argument("--save_dir", type=str, default=str(Path().cwd() / "checkpoints" / "unmerge_model_multi"))
    ...
    train_ds_un = UnmergeKDDataset(features_hlt_std[train_idx], hlt_mask[train_idx],
                                   features_off_std[train_idx], masks_off[train_idx], all_labels[train_idx])
    # Train the unmerger on merged samples only
    train_loader_un = DataLoader(
        UnmergeKDDataset(features_hlt_std[train_idx], hlt_mask[train_idx],
                         features_off_std[train_idx], masks_off[train_idx], all_labels[train_idx]),
        batch_size=BS_un, shuffle=True, drop_last=True
    )
    ...
    # Build datasets for unmerge model
    def build_unmerge_dataset(sample_list):
        return UnmergeKDDataset(features_hlt_std, hlt_mask, features_off_std, masks_off, all_labels), sample_list

    train_ds_u = UnmergeKDDataset(features_hlt_std, hlt_mask, features_off_std, masks_off, all_labels)
    train_loader_u = DataLoader(train_ds_u, batch_size=BS_un, shuffle=True, drop_last=True)

    val_ds_u = UnmergeKDDataset(features_hlt_std, hlt_mask, features_off_std, masks_off, all_labels)
    val_loader_u = DataLoader(val_ds_u, batch_size=BS_un, shuffle=False)

    # Use the merged-token list for the actual loss
    def unmerge_epoch(loader, train_mode):
        if train_mode:
            unmerge_model.train()
        else:
            unmerge_model.eval()
        total_loss = 0.0
        n_batches = 0
        sample_list = train_samples if train_mode else val_samples
        for i in range(0, len(sample_list), BS_un):
            ...
            x = torch.tensor(features_hlt_std[jet_idx], dtype=torch.float32, device=device)
            m = torch.tensor(hlt_mask[jet_idx], dtype=torch.bool, device=device)
            token_idx_t = torch.tensor(token_idx, dtype=torch.long, device=device)
            pred_count_t = torch.tensor(pred_counts_batch, dtype=torch.long, device=device)
            true_count_t = torch.tensor(true_counts, dtype=torch.long, device=device)
            target_t = torch.tensor(target, dtype=torch.float32, device=device)

            if train_mode:
                opt_u.zero_grad()
            preds = unmerge_model(x, m, token_idx_t, pred_count_t)
            loss = set_chamfer_loss(preds, target_t, true_count_t)
            if train_mode:
                loss.backward()
    ...
    for ep in tqdm(range(CONFIG["unmerge_training"]["epochs"]), desc="Unmerge"):
        train_loss = unmerge_epoch(train_loader_u, True)
        val_loss = unmerge_epoch(val_loader_u, False)
        sch_u.step()
    ...
    # ------------------- Build multi-view unmerged datasets ------------------- #
    print("\n" + "=" * 70)
    print(f"STEP 5: BUILD {args.n_unmerge_views} UNMERGED DATASETS")
    print("=" * 70)
    unmerged_const_list = []
    unmerged_mask_list = []
    for v in range(args.n_unmerge_views):
        seed = RANDOM_SEED + 1000 + v
        const_v, mask_v = build_unmerged_view(
            features_hlt_std,
            hlt_mask,
            hlt_const,
            pred_counts,
            ...
            device,
            BS_un,
            args.unmerge_noise_pt,
            args.unmerge_noise_eta,
            args.unmerge_noise_phi,
            seed,
        )
        unmerged_const_list.append(const_v)
        unmerged_mask_list.append(mask_v)

    feats_unmerged_list = []
    masks_unmerged_list = []
    for v in range(args.n_unmerge_views):
        feat_v = compute_features(unmerged_const_list[v], unmerged_mask_list[v])
        feat_v = standardize(feat_v, unmerged_mask_list[v], feat_means, feat_stds)
        feats_unmerged_list.append(feat_v)
        masks_unmerged_list.append(unmerged_mask_list[v])

    # ------------------- Train unmerge model classifier (multi-view) ------------------- #
    print("\n" + "=" * 70)
    print("STEP 6: UNMERGE MODEL CLASSIFIER (multi-view)")
    print("=" * 70)
    train_feat = np.concatenate([f[train_idx] for f in feats_unmerged_list], axis=0)
    train_mask = np.concatenate([m[train_idx] for m in masks_unmerged_list], axis=0)
    train_labels = np.concatenate([all_labels[train_idx] for _ in feats_unmerged_list], axis=0)
    val_feat = [f for f in feats_unmerged_list]
    val_mask = [m for m in masks_unmerged_list]

    train_ds_um = JetDataset(train_feat, train_mask, train_labels)
    train_loader_um = DataLoader(train_ds_um, batch_size=BS, shuffle=True, drop_last=True)
    ...
    for ep in tqdm(range(CONFIG["training"]["epochs"]), desc="UnmergeCls"):
        ...
        val_auc, _ = evaluate_student_views(unmerge_cls, val_feat, val_mask, all_labels, val_idx, device, BS)
        sch_ucls.step()
    ...
    auc_unmerge, preds_unmerge = evaluate_student_views(unmerge_cls, feats_unmerged_list, masks_unmerged_list, all_labels, test_idx, device, BS)

    # ------------------- Train unmerge model + KD (multi-view) ------------------- #
    print("\n" + "=" * 70)
    print("STEP 7: UNMERGE MODEL + KD (multi-view)")
    print("=" * 70)
    kd_train_feat = np.concatenate([f[train_idx] for f in feats_unmerged_list], axis=0)
    kd_train_mask = np.concatenate([m[train_idx] for m in masks_unmerged_list], axis=0)
    kd_train_off = np.concatenate([features_off_std[train_idx] for _ in feats_unmerged_list], axis=0)
    kd_train_off_mask = np.concatenate([masks_off[train_idx] for _ in feats_unmerged_list], axis=0)
    kd_train_labels = np.concatenate([all_labels[train_idx] for _ in feats_unmerged_list], axis=0)

    kd_val_feat = np.concatenate([f[val_idx] for f in feats_unmerged_list], axis=0)
    kd_val_mask = np.concatenate([m[val_idx] for m in masks_unmerged_list], axis=0)
    kd_val_off = np.concatenate([features_off_std[val_idx] for _ in feats_unmerged_list], axis=0)
    kd_val_off_mask = np.concatenate([masks_off[val_idx] for _ in feats_unmerged_list], axis=0)
    kd_val_labels = np.concatenate([all_labels[val_idx] for _ in feats_unmerged_list], axis=0)

    kd_train_ds = UnmergeKDDataset(kd_train_feat, kd_train_mask, kd_train_off, kd_train_off_mask, kd_train_labels)
    kd_val_ds = UnmergeKDDataset(kd_val_feat, kd_val_mask, kd_val_off, kd_val_off_mask, kd_val_labels)
    ...
    for ep in tqdm(range(CONFIG["training"]["epochs"]), desc="Unmerge+KD"):
        ...
        _, train_auc = train_kd_epoch(kd_student, teacher, kd_train_loader, opt_kd, device, kd_cfg_ep)
        val_auc, _ = evaluate_student_views(kd_student, feats_unmerged_list, masks_unmerged_list, all_labels, val_idx, device, BS)
        sch_kd.step()
    ...
    auc_unmerge_kd, preds_unmerge_kd = evaluate_student_views(kd_student, feats_unmerged_list, masks_unmerged_list, all_labels, test_idx, device, BS)
    ...
    np.savez(
        ...
        n_unmerge_views=args.n_unmerge_views,
        noise_pt=args.unmerge_noise_pt,
        noise_eta=args.unmerge_noise_eta,
        noise_phi=args.unmerge_noise_phi,
    )
