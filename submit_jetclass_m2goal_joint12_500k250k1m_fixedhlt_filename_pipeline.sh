...
# Queues 12 JetClass m2-goal hybrid adaptive-generation variants at 500k/250k/1M,
...
RUNNER_FUSION="${RUNNER_FUSION:-run_analyze_jetclass_twelve_model_stacked_fusion_500k250k1m_m2goal.sh}"
...
TAG="${TAG:-fixedhlt}"
CORE01="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_${TAG}_core01_base"
CORE02="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_${TAG}_core02_consstrong"
CORE03="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_${TAG}_core03_budgetlite"
CORE04="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_${TAG}_core04_genlow"
CORE05="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_${TAG}_core05_genhigh"
CORE06="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_${TAG}_core06_splitstrong"
CORE07="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_${TAG}_core07_splitlight"
CORE08="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_${TAG}_core08_physstrong"
CORE09="jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_${TAG}_core09_offdropmid"
...
OUT_DIR="${OUT_DIR:-${SAVE_DIR}/fusion_reports/twelve_model_500k250k1m_m2goal_${TAG}_stacked_acc}"
...
echo "Submitting fixed-HLT JetClass 12-runner m2-goal 500k/250k/1M pipeline on ${PARTITION}"
...
j01=$(submit_train "jcG01" "RUN_NAME=${CORE01}")
echo "  CORE01 ${j01} ${CORE01}"

j02=$(submit_train "jcG02" "RUN_NAME=${CORE02},LAMBDA_CONS=0.06,LOSS_W_BUDGET=0.80,STAGEC_LR_DUAL=1e-4,STAGEC_LR_RECO=5e-5")
...
j04=$(submit_train "jcG04" "RUN_NAME=${CORE04},RECO_MAX_GENERATED_TOKENS=40,LOSS_W_SPARSE=0.012")
echo "  CORE04 ${j04} ${CORE04}"

j05=$(submit_train "jcG05" "RUN_NAME=${CORE05},RECO_MAX_GENERATED_TOKENS=72,LOSS_W_SPARSE=0.006")
echo "  CORE05 ${j05} ${CORE05}"

j06=$(submit_train "jcG06" "RUN_NAME=${CORE06},LOSS_W_LOCAL=0.12,LOSS_W_BUDGET=0.75")
...
j09=$(submit_train "jcG09" "RUN_NAME=${CORE09},TARGET_DROP_PROB_MAX=0.50,TARGET_DROP_WARMUP_EPOCHS=20,TARGET_DROP_NUM_BANKS=3,TARGET_DROP_BANK_CYCLE_EPOCHS=1")
...
echo "============================================================"
echo "Queued fixed-HLT JetClass 12-runner m2-goal 500k/250k/1M pipeline"
