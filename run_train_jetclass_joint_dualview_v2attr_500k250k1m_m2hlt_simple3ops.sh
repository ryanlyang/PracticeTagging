#!/usr/bin/env bash
#SBATCH --job-name=jcS3Ops
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=128G
#SBATCH --time=2-00:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_500k250k1m_m2hlt_simple3ops_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_500k250k1m_m2hlt_simple3ops_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_v2attr_500k250k1m_m2hlt_simple3ops_sps001}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
...
LOSS_W_BUDGET="${LOSS_W_BUDGET:-0.45}"
LOSS_W_SPARSE="${LOSS_W_SPARSE:-0.010}"     # small anti-overgeneration pressure
LOSS_W_LOCAL="${LOSS_W_LOCAL:-0.04}"
LOSS_GEN_LOCAL_RADIUS="${LOSS_GEN_LOCAL_RADIUS:-0.08}"
LOSS_W_PT_RATIO="${LOSS_W_PT_RATIO:-0.22}"
LOSS_W_M_RATIO="${LOSS_W_M_RATIO:-0.04}"
LOSS_W_E_RATIO="${LOSS_W_E_RATIO:-0.04}"
LOSS_W_PHYS="${LOSS_W_PHYS:-0.00}"
ADDED_TARGET_SCALE="${ADDED_TARGET_SCALE:-0.90}"
SIMPLE3_LOSS_W_AXIS="${SIMPLE3_LOSS_W_AXIS:-0.08}"
...
SCRIPT="${SCRIPT:-$(pwd)/train_jetclass_joint_dualview_stage2_unmergeonly_m2hlt_simple3_ops_v2_attr.py}"
...
CMD=(
  ...
  --simple3_loss_w_axis "${SIMPLE3_LOSS_W_AXIS}"
...
echo "============================================================"
echo "JetClass m2-style simple3 reconstructor (unsmear/split/generate)"
...
echo "Simple3: soft_budget=${LOSS_W_BUDGET}, pt/m/e=${LOSS_W_PT_RATIO}/${LOSS_W_M_RATIO}/${LOSS_W_E_RATIO}, axis=${SIMPLE3_LOSS_W_AXIS}, local=${LOSS_W_LOCAL}"
echo "============================================================"
printf ' %q' "${CMD[@]}"
echo
"${CMD[@]}"
