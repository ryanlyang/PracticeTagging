...
RESUME_DIR=${RESUME_DIR:-"$SAVE_DIR/$RUN_NAME/resume"}
...
CMD="python unmerge_distr_model_long.py \
  --save_dir $SAVE_DIR \
  --run_name $RUN_NAME \
  ...
  --max_hours $MAX_HOURS \
  --time_margin_min $TIME_MARGIN_MIN \
  --resume \
...
if [ $EXIT_CODE -eq 99 ]; then
  echo "Time limit reached (checkpoint saved). Chain will continue."
  EXIT_CODE=0
fi
