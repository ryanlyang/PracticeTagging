...
"""
Confidence-gated operation-aware JetClass reconstructor for m2-style training.

Design:
- Shared transformer encoder on full HLT token features.
- Explicit per-parent operation split from epoch 1:
  carry/edit vs split(parent) vs generate-support.
- Per-token carry branch for conservative unsmear-like edits.
- Per-token split branch (2 children) with parent uplift.
- Jet-latent generation branch for truly missing tokens, with a soft generation
  floor so generator candidates are not over-suppressed.
...
class OfflineReconstructorConfidenceHybridOps(nn.Module):
    """
    ...
    def __init__(
        ...
        # Operation gating: carry / split / generate-support.
        self.op_gate_head = nn.Linear(embed_dim, 3)
        self.gen_seed_head = nn.Sequential(
    ...
    def forward(
        ...
        gate_temp = self.gate_temperature.abs().clamp(min=0.35, max=3.0)
        op_logits = self.op_gate_head(x) / gate_temp
        op_logits = op_logits.masked_fill(~mask_hlt.unsqueeze(-1), -1e4)
        op_gate = torch.softmax(op_logits, dim=-1)
        carry_gate = op_gate[..., 0]
        split_gate = op_gate[..., 1]
        gen_gate = op_gate[..., 2]
        ...
        split_parent_w = (tok_exist * (0.50 * split_gate + 0.50 * p_split)).clamp(0.0, 1.0)
        tok_w_raw = (tok_exist * carry_gate * (1.0 - 0.55 * split_parent_w)).clamp(0.0, 1.0)
        ...
        parent_gen_seed = (torch.sigmoid(self.gen_seed_head(x).squeeze(-1)) * gen_gate * mask_hlt.float()).clamp(0.0, 1.0)
        ...
        # Keep a mild generation floor so generator branch is not over-penalized.
        ...
        gen_floor_frac = 0.20
        gen_floor = (budget_added * gen_floor_frac).clamp(min=0.0)
        seed_based_floor = parent_gen_seed.sum(dim=1) * 0.50
        gen_target_added = torch.maximum(gen_target_added, torch.minimum(gen_floor, seed_based_floor + gen_floor))

        sum_gen = gen_exist.sum(dim=1, keepdim=True).clamp(min=eps)
        jet_gen_gate = gen_gate.mean(dim=1, keepdim=True)
        gen_scale = (gen_target_added.unsqueeze(1) / sum_gen).clamp(min=0.35, max=5.0)
        gen_scale = gen_scale * (0.75 + 0.50 * jet_gen_gate)
        ...
        return {
            ...
            "op_gate": op_gate,
            "parent_gen_seed": parent_gen_seed,
        }


def compute_reconstruction_losses_weighted_confgen_ops(
    ...
    # Branch-specific sparsity:
    # keep generation penalty lighter than split/carry so generator is not
    # suppressed when it helps downstream tagging.
    split_w = out.get("child_weight", None)
    gen_w = out.get("gen_weight", None)
    if split_w is not None and gen_w is not None:
        loss_sparse_split_vec = split_w.mean(dim=1)
        loss_sparse_gen_vec = gen_w.mean(dim=1)
        loss_sparse_vec = 0.5 * (loss_sparse_split_vec + loss_sparse_gen_vec)
    elif split_w is not None:
        loss_sparse_split_vec = split_w.mean(dim=1)
        loss_sparse_gen_vec = torch.zeros_like(loss_sparse_split_vec)
        loss_sparse_vec = split_w.mean(dim=1)
    elif gen_w is not None:
        loss_sparse_gen_vec = gen_w.mean(dim=1)
        loss_sparse_split_vec = torch.zeros_like(loss_sparse_gen_vec)
        loss_sparse_vec = gen_w.mean(dim=1)
    else:
        loss_sparse_split_vec = torch.zeros_like(true_added)
        loss_sparse_gen_vec = torch.zeros_like(true_added)
        loss_sparse_vec = torch.zeros_like(true_added)

    w_sparse_split = float(loss_cfg.get("w_sparse_split", loss_cfg.get("w_sparse", 0.0)))
    w_sparse_gen = float(loss_cfg.get("w_sparse_gen", 0.35 * w_sparse_split))
    ...
    # Generator-specific false-positive term with lower weight than global FP.
    n_tok = out.get("tok_weights", torch.zeros_like(mask_hlt.float())).shape[1]
    n_split = out.get("child_weight", torch.zeros((pred.shape[0], 0), device=pred.device)).shape[1]
    gen_slice = slice(int(n_tok + n_split), int(pred.shape[1]))
    if gen_slice.start < gen_slice.stop:
        gen_min_cost = min_pred_cost[:, gen_slice]
        gen_w_eff = w[:, gen_slice]
        gen_unmatched_soft = torch.sigmoid((gen_min_cost - fp_cost_thresh) / fp_cost_tau)
        loss_gen_fp_vec = (gen_w_eff * gen_unmatched_soft).sum(dim=1) / (gen_w_eff.sum(dim=1) + eps)
    else:
        loss_gen_fp_vec = torch.zeros_like(loss_fp_mass_vec)
    ...
    total_vec = (
        ...
        + w_sparse_split * loss_sparse_split_vec
        + w_sparse_gen * loss_sparse_gen_vec
        + float(loss_cfg.get("w_local", 0.0)) * loss_local_vec
        + float(loss_cfg.get("w_fp_mass", 0.0)) * loss_fp_mass_vec
        + float(loss_cfg.get("w_gen_fp", 0.05)) * loss_gen_fp_vec
    )
    ...
    return {
        ...
        "sparse_split": base._weighted_batch_mean(loss_sparse_split_vec, sw),
        "sparse_gen": base._weighted_batch_mean(loss_sparse_gen_vec, sw),
        "local": base._weighted_batch_mean(loss_local_vec, sw),
        "fp_mass": base._weighted_batch_mean(loss_fp_mass_vec, sw),
        "gen_fp": base._weighted_batch_mean(loss_gen_fp_vec, sw),
    }


def build_soft_corrected_view_confgen_ops(
...
def _patch_base_module() -> None:
    base.OfflineReconstructor = OfflineReconstructorConfidenceHybridOps  # type: ignore[assignment]
    base.compute_reconstruction_losses_weighted = compute_reconstruction_losses_weighted_confgen_ops  # type: ignore[assignment]
    base.build_soft_corrected_view = build_soft_corrected_view_confgen_ops  # type: ignore[assignment]
