...
from offline_reconstructor_no_gt_local30kv2 import (
    ...
    OfflineReconstructor as BaseOfflineReconstructor,
...
def _resolve_ckpt_from_run_dir(
    explicit_ckpt: str,
    run_dir: str,
    candidate_names_csv: str,
) -> Optional[Path]:
    if str(explicit_ckpt).strip():
        p = Path(str(explicit_ckpt).strip()).expanduser().resolve()
        if not p.exists():
            raise FileNotFoundError(f"Warm-start checkpoint not found: {p}")
        return p
    if not str(run_dir).strip():
        return None
    rd = Path(str(run_dir).strip()).expanduser().resolve()
    if not rd.exists():
        raise FileNotFoundError(f"Warm-start run_dir not found: {rd}")
    candidates = [x.strip() for x in str(candidate_names_csv).split(",") if x.strip()]
...
def _load_state_dict_forgiving(module: nn.Module, sd: Dict[str, torch.Tensor], label: str) -> str:
    try:
        ...
        filt = {}
        for k, v in sd.items():
            if k in ref and torch.is_tensor(v) and torch.is_tensor(ref[k]) and tuple(v.shape) == tuple(ref[k].shape):
                filt[k] = v
        info = module.load_state_dict(filt, strict=False)
        msg = (
            f"shape_compatible_only(loaded={len(filt)}, "
            f"missing={len(info.missing_keys)}, unexpected={len(info.unexpected_keys)})"
        )
        print(f"[warn] {label}: strict load failed, fallback to {msg} because {e}")
        return msg
...
def enforce_unmerge_only_output(reco_out: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    # In this iterative+eff-split variant, keep efficiency branch enabled.
    return reco_out


def wrap_reconstructor_unmerge_only(model: nn.Module) -> nn.Module:
    # No-op in this variant (efficiency branch stays active).
    return model


class OfflineReconstructorIterative(BaseOfflineReconstructor):
    """
    Base reconstructor + iterative small-step refinement over predicted candidate tokens.
    Refinement is staged via step curriculum (early/mid/late) and can optionally allow
    tiny edits on original HLT token slots.
    """

    def __init__(
        self,
        *args,
        iter_steps_early: int = 1,
        iter_steps_mid: int = 3,
        iter_steps_late: int = 5,
        iter_cap_logpt: float = 0.05,
        iter_cap_eta: float = 0.02,
        iter_cap_phi: float = 0.02,
        iter_cap_loge: float = 0.05,
        iter_hlt_edit_mode: str = "fixed",
        iter_hlt_edit_scale: float = 0.25,
        iter_weight_floor: float = 1e-4,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        hid = max(64, int(self.input_proj[0].out_features // 2))
        self.iter_refiner = nn.Sequential(
            nn.Linear(8, hid),
            nn.GELU(),
            nn.Linear(hid, 4),
        )
        self.iter_steps_early = int(max(iter_steps_early, 0))
        self.iter_steps_mid = int(max(iter_steps_mid, 0))
        self.iter_steps_late = int(max(iter_steps_late, 0))
        self.iter_cap_logpt = float(max(iter_cap_logpt, 0.0))
        self.iter_cap_eta = float(max(iter_cap_eta, 0.0))
        self.iter_cap_phi = float(max(iter_cap_phi, 0.0))
        self.iter_cap_loge = float(max(iter_cap_loge, 0.0))
        self.iter_hlt_edit_mode = str(iter_hlt_edit_mode).strip().lower()
        self.iter_hlt_edit_scale = float(max(min(iter_hlt_edit_scale, 1.0), 0.0))
        self.iter_weight_floor = float(max(iter_weight_floor, 0.0))

    def _num_iter_steps(self, stage_scale: float) -> int:
        ss = float(stage_scale)
        if ss < 0.5:
            return self.iter_steps_early
        if ss < 0.9:
            return self.iter_steps_mid
        return self.iter_steps_late
    ...
    def forward(
        ...
        steps = int(self._num_iter_steps(stage_scale))
        if steps <= 0:
            return out
        ...
        merge_f = out["cand_merge_flags"].clamp(0.0, 1.0)
        eff_f = out["cand_eff_flags"].clamp(0.0, 1.0)
        B, N, _ = cand.shape
        L = int(mask_hlt.shape[1])

        # Start HLT token slots exactly from observed HLT view; iterative mode decides
        # whether they remain fixed or receive tiny capped edits.
        if N >= L:
            cand = cand.clone()
            cand[:, :L, :] = const_hlt[:, :L, :]

        # Build editable mask: all active reco candidates; HLT slots optional tiny-edit mode.
        edit_scale = (cand_w > float(self.iter_weight_floor)).float()
        if self.iter_hlt_edit_mode == "fixed":
            edit_scale[:, :L] = 0.0
        elif self.iter_hlt_edit_mode == "tiny_edit":
            edit_scale[:, :L] = self.iter_hlt_edit_scale * mask_hlt.float()
        else:
            raise ValueError(
                f"Unsupported iter_hlt_edit_mode={self.iter_hlt_edit_mode}. "
                "Use one of: fixed, tiny_edit."
            )

        step_scale = 1.0 / float(max(steps, 1))
        cap_logpt = self.iter_cap_logpt * step_scale
        cap_eta = self.iter_cap_eta * step_scale
        cap_phi = self.iter_cap_phi * step_scale
        cap_loge = self.iter_cap_loge * step_scale

        for _ in range(steps):
            pt = cand[..., 0].clamp(min=eps)
            eta = cand[..., 1].clamp(min=-5.0, max=5.0)
            phi = cand[..., 2]
            E = cand[..., 3].clamp(min=eps)

            ref_feat = torch.stack(
                [
                    torch.log(pt),
                    eta,
                    torch.sin(phi),
            ...
            d = self.iter_refiner(ref_feat)
            d_logpt = cap_logpt * torch.tanh(d[..., 0]) * edit_scale
            d_eta = cap_eta * torch.tanh(d[..., 1]) * edit_scale
            d_phi = cap_phi * torch.tanh(d[..., 2]) * edit_scale
            d_loge = cap_loge * torch.tanh(d[..., 3]) * edit_scale

            new_pt = torch.exp(torch.clamp(torch.log(pt) + d_logpt, min=-9.0, max=9.0))
            new_eta = (eta + d_eta).clamp(min=-5.0, max=5.0)
            new_phi = _wrap_phi_t(phi + d_phi)
            new_E = torch.exp(torch.clamp(torch.log(E) + d_loge, min=-9.0, max=11.0))
            new_E = torch.maximum(new_E, new_pt * torch.cosh(new_eta))
            cand = torch.stack([new_pt, new_eta, new_phi, new_E], dim=-1)

        out["cand_tokens"] = cand
        if "gen_tokens" in out and "child_weight" in out and "gen_weight" in out:
            n_child = int(out["child_weight"].shape[1])
            n_gen = int(out["gen_weight"].shape[1])
            g0 = int(L + n_child)
            if n_gen > 0 and (g0 + n_gen) <= int(N):
                out["gen_tokens"] = cand[:, g0 : g0 + n_gen, :]
        return out


OfflineReconstructor = OfflineReconstructorIterative
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="cold",
        choices=["cold", "warm"],
        help="cold: train from scratch; warm: initialize from previous run checkpoints.",
    )
    parser.add_argument(
        "--warm_start_run_dir",
        type=str,
        default="",
        help="Run dir to search for warm-start checkpoints if explicit ckpt paths are not provided.",
    )
    parser.add_argument(
        "--warm_start_reco_ckpt",
        type=str,
        default="",
        help="Explicit reconstructor checkpoint path for warm start.",
    )
    parser.add_argument(
        "--warm_start_reco_ckpt_names",
        type=str,
        default="offline_reconstructor_stage2.pt,offline_reconstructor.pt",
        help="Comma-separated ckpt names searched under --warm_start_run_dir for reconstructor warm start.",
    )
    parser.add_argument(
        "--warm_start_dual_ckpt",
        type=str,
        default="",
        help="Explicit dual checkpoint path for warm start.",
    )
    parser.add_argument(
        "--warm_start_dual_ckpt_names",
        type=str,
        default="dual_joint_stage2.pt,dual_joint.pt",
        help="Comma-separated ckpt names searched under --warm_start_run_dir for dual warm start.",
    )
    ...
    parser.add_argument(
        "--added_target_merge_frac",
        type=float,
        default=0.85,
        help="Fraction of true_added target assigned to merge branch (eff gets the remainder).",
    )
    parser.add_argument(
        "--iter_steps_early",
        type=int,
        default=1,
        help="Iterative refinement steps during stage_scale~0.35.",
    )
    parser.add_argument(
        "--iter_steps_mid",
        type=int,
        default=3,
        help="Iterative refinement steps during stage_scale~0.70.",
    )
    parser.add_argument(
        "--iter_steps_late",
        type=int,
        default=5,
        help="Iterative refinement steps during stage_scale~1.00 (and eval).",
    )
    parser.add_argument("--iter_cap_logpt", type=float, default=0.05)
    parser.add_argument("--iter_cap_eta", type=float, default=0.02)
    parser.add_argument("--iter_cap_phi", type=float, default=0.02)
    parser.add_argument("--iter_cap_loge", type=float, default=0.05)
    parser.add_argument(
        "--iter_hlt_edit_mode",
        type=str,
        default="fixed",
        choices=["fixed", "tiny_edit"],
        help="fixed: never edit original HLT slots; tiny_edit: allow very small capped edits on HLT slots.",
    )
    parser.add_argument(
        "--iter_hlt_edit_scale",
        type=float,
        default=0.25,
        help="Scale factor for HLT-slot edits when iter_hlt_edit_mode=tiny_edit.",
    )
    parser.add_argument("--iter_weight_floor", type=float, default=1e-4)
    ...
    warm_mode = str(args.warm_start_mode).strip().lower() == "warm"
    warm_reco_ckpt_used: Optional[Path] = None
    warm_dual_ckpt_used: Optional[Path] = None
    warm_reco_load_mode = "none"
    warm_dual_load_mode = "none"
    ...
    # Non-privileged target from offline-vs-HLT count gap, split into merge/eff components.
    # By default, this variant uses a fixed 85/15 split and scale=1.0 (100% added-count recovery).
    ...
    merge_frac = float(np.clip(args.added_target_merge_frac, 0.0, 1.0))
    eff_frac = float(1.0 - merge_frac)
    budget_merge_true_raw = (merge_frac * true_added_raw).astype(np.float32)
    budget_eff_true_raw = (eff_frac * true_added_raw).astype(np.float32)
    ...
    print(
        ...
        f"split merge/eff={merge_frac:.2f}/{eff_frac:.2f}, "
    ...
    data_setup = {
        ...
        "variant": "nopriv_iterrefine_effsplit",
        "added_target_scale": float(added_target_scale),
        "added_target_merge_frac": float(merge_frac),
        "added_target_eff_frac": float(eff_frac),
        "warm_start_mode": "warm" if warm_mode else "cold",
        "warm_start_run_dir": str(args.warm_start_run_dir),
        "warm_start_reco_ckpt": str(args.warm_start_reco_ckpt),
        "warm_start_dual_ckpt": str(args.warm_start_dual_ckpt),
    ...
    reconstructor = OfflineReconstructor(
        input_dim=7,
        iter_steps_early=int(args.iter_steps_early),
        iter_steps_mid=int(args.iter_steps_mid),
        iter_steps_late=int(args.iter_steps_late),
        iter_cap_logpt=float(args.iter_cap_logpt),
        iter_cap_eta=float(args.iter_cap_eta),
        iter_cap_phi=float(args.iter_cap_phi),
        iter_cap_loge=float(args.iter_cap_loge),
        iter_hlt_edit_mode=str(args.iter_hlt_edit_mode),
        iter_hlt_edit_scale=float(args.iter_hlt_edit_scale),
        iter_weight_floor=float(args.iter_weight_floor),
        **cfg["reconstructor_model"],
    ).to(device)
    if warm_mode:
        warm_reco_ckpt_used = _resolve_ckpt_from_run_dir(
            explicit_ckpt=str(args.warm_start_reco_ckpt),
            run_dir=str(args.warm_start_run_dir),
            candidate_names_csv=str(args.warm_start_reco_ckpt_names),
        )
        if warm_reco_ckpt_used is None:
            raise FileNotFoundError(
                "warm_start_mode=warm but no reconstructor checkpoint resolved. "
                "Provide --warm_start_reco_ckpt or --warm_start_run_dir with valid --warm_start_reco_ckpt_names."
            )
        sd = _load_checkpoint_model_state(str(warm_reco_ckpt_used), device)
        warm_reco_load_mode = _load_state_dict_forgiving(
            reconstructor,
            sd,
            label=f"warm-start reco ({warm_reco_ckpt_used})",
        )
        print(
            f"Warm-start reconstructor loaded from: {warm_reco_ckpt_used} "
            f"[{warm_reco_load_mode}]"
        )
    ...
    if warm_mode:
        warm_dual_ckpt_used = _resolve_ckpt_from_run_dir(
            explicit_ckpt=str(args.warm_start_dual_ckpt),
            run_dir=str(args.warm_start_run_dir),
            candidate_names_csv=str(args.warm_start_dual_ckpt_names),
        )
        if warm_dual_ckpt_used is None:
            raise FileNotFoundError(
                "warm_start_mode=warm but no dual checkpoint resolved. "
                "Provide --warm_start_dual_ckpt or --warm_start_run_dir with valid --warm_start_dual_ckpt_names."
            )
        sd_dual = _load_checkpoint_model_state(str(warm_dual_ckpt_used), device)
        warm_dual_load_mode = _load_state_dict_forgiving(
            dual_joint,
            sd_dual,
            label=f"warm-start dual ({warm_dual_ckpt_used})",
        )
        print(
            f"Warm-start dual loaded from: {warm_dual_ckpt_used} "
            f"[{warm_dual_load_mode}]"
        )
    ...
    with open(save_root / "joint_stage_metrics.json", "w", encoding="utf-8") as f:
        json.dump(
            {
                "variant": {
                    "mode": "nopriv_iterrefine_effsplit",
                    "added_target_scale": float(added_target_scale),
                    "added_target_merge_frac": float(merge_frac),
                    "added_target_eff_frac": float(eff_frac),
                    "mean_true_added_raw": float(true_added_raw.mean()),
                    ...
                    "iter_hlt_edit_scale": float(args.iter_hlt_edit_scale),
                    "warm_start_mode": "warm" if warm_mode else "cold",
                    "warm_start_reco_ckpt_used": (str(warm_reco_ckpt_used) if warm_reco_ckpt_used is not None else ""),
                    "warm_start_reco_load_mode": str(warm_reco_load_mode),
                    "warm_start_dual_ckpt_used": (str(warm_dual_ckpt_used) if warm_dual_ckpt_used is not None else ""),
                    "warm_start_dual_load_mode": str(warm_dual_load_mode),
                    "stageB_train_frac": float(stageB_train_frac),
                    "stageB_subset_seed": int(stageB_subset_seed),
                    "stageB_train_count": int(len(stageB_train_idx)),
