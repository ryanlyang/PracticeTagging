#!/usr/bin/env bash
#SBATCH --job-name=jcJv1Str
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --time=2-00:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_150k75k150k_stronger_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_150k75k150k_stronger_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_v1_150k75k150k_stronger}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
NUM_WORKERS="${NUM_WORKERS:-2}"

N_TRAIN_JETS="${N_TRAIN_JETS:-150000}"
N_VAL_JETS="${N_VAL_JETS:-75000}"
...
CMD=(
  python -u train_jetclass_joint_dualview_stage2_unmergeonly.py
  --data_dir "${DATA_DIR}"
  --save_dir "${SAVE_DIR}"
  --run_name "${RUN_NAME}"
  --seed "${SEED}"
...
echo "============================================================"
echo "JetClass Joint Dual-View V1 (stronger HLT profile)"
