#!/usr/bin/env bash
#SBATCH --job-name=jcM2H2C
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=3-00:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_200k50k200k_m2hlt_hybridops_adaptivegen_qcdtbqq_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_v2attr_200k50k200k_m2hlt_hybridops_adaptivegen_qcdtbqq_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_v2attr_200k50k200k_m2hlt_hybridops_adaptivegen_qcdtbqq_base}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
...
N_TEST_JETS="${N_TEST_JETS:-200000}"
...
INCLUDE_CLASSES="${INCLUDE_CLASSES:-QCD,Tbqq}"
TARGET_CLASS="${TARGET_CLASS:-Tbqq}"
...
echo "============================================================"
echo "JetClass m2-style hybrid reconstructor (adaptive split/gen budget, QCD/Tbqq)"
echo "Job ID: ${SLURM_JOB_ID:-N/A}"
echo "Node: ${SLURMD_NODENAME:-N/A}"
echo "Run: ${SAVE_DIR}/${RUN_NAME}"
echo "Split: train=${N_TRAIN_JETS}, val=${N_VAL_JETS}, test=${N_TEST_JETS}"
echo "Class assignment: ${CLASS_ASSIGNMENT} | include_classes=${INCLUDE_CLASSES}"
