#!/usr/bin/env bash
#SBATCH --job-name=jcPG6Resp
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=96G
#SBATCH --time=2:55:00
#SBATCH --output=offline_reconstructor_logs/jetclass_projected_goal6_jet_response_500k250k1m_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_projected_goal6_jet_response_500k250k1m_%j.err
...
OUT_DIR="${OUT_DIR:-${SAVE_ROOT}/response_reports/projected_goal6_500k250k1m_pt_response_fast100k}"
DEVICE="${DEVICE:-cuda}"
...
CMD=(
  python -u analyze_jetclass_twelve_model_jet_response_m2hlt_projected_goal.py
  --model "${MODEL_01_SPEC}"
  --model "${MODEL_02_SPEC}"
  --model "${MODEL_03_SPEC}"
  --model "${MODEL_04_SPEC}"
...
echo "============================================================"
echo "JetClass projected-goal selected6 pT Response/Resolution (500k/250k/1m)"
