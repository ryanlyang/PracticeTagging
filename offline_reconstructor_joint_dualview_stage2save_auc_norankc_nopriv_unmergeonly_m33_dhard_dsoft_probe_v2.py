...
3) Train/evaluate D_soft surrogate v2 (offline->HLT degrader) against deterministic D_hard data.
...
def _make_batch_keys(epoch_idx: int, batch_idx: int, n: int, seed: int, offset: int = 0) -> np.ndarray:
    base_key = int(seed) * 10_000_000 + int(epoch_idx) * 100_000 + int(batch_idx) * 1_000 + int(offset)
    return (base_key + np.arange(int(n), dtype=np.int64)).astype(np.int64)


def _train_degrader_v2(
    model: m33.OfflineToHLTDegrader,
    ...
    rank_temp: float,
    rank_feas_w: float,
    rank_feas_temp: float,
    eps_total: float,
    eps_count: float,
) -> Tuple[m33.OfflineToHLTDegrader, Dict[str, float]]:
    """
    D_soft v2:
    - Recon objective aligned to downstream residual (set+count+pt+mass).
    - Candidate-style augmentation: perturb offline input and train against D_hard(perturbed offline).
    - Ranking regularization: D_soft must rank candidate pools similarly to D_hard.
    """
    ...
    rank_temp = float(max(1e-4, rank_temp))
    rank_feas_temp = float(max(1e-4, rank_feas_temp))
    ...
    for ep in range(int(epochs)):
        ...
        for bi, batch in enumerate(train_loader):
            const_off_base = batch["const_off"].to(device)
            mask_off_base = batch["mask_off"].to(device)
            const_hlt_base = batch["const_hlt"].to(device)
            mask_hlt_base = batch["mask_hlt"].to(device)
            bs = int(const_off_base.shape[0])

            # Candidate-style augmentation: train on D_hard(perturbed offline) targets.
            if float(aug_prob) > 0.0 and rng.rand() < float(aug_prob):
                co_np = const_off_base.detach().cpu().numpy().astype(np.float32)
                mo_np = mask_off_base.detach().cpu().numpy().astype(bool)
                aug_c, aug_m = _build_perturbed_candidates(
                    const_off=co_np,
                    mask_off=mo_np,
                    n_cand=2,
                    rng=rng,
                    pt_sigma=float(aug_pt_sigma),
                    eta_sigma=float(aug_eta_sigma),
                    phi_sigma=float(aug_phi_sigma),
                    e_sigma=float(aug_e_sigma),
                    drop_prob=float(aug_drop_prob),
                )
                aug_c = aug_c[:, 1]
                aug_m = aug_m[:, 1]
                keys_aug = _make_batch_keys(ep, bi, bs, seed=int(seed) + 911, offset=0)
                tgt_c, tgt_m, _ = m33._apply_hlt_effects_deterministic_keyed(
                    const=aug_c,
                    mask=aug_m,
                    cfg=cfg,
                    jet_keys=keys_aug,
                    base_seed=int(dhard_base_seed),
                )
                const_in = torch.tensor(aug_c, dtype=torch.float32, device=device)
                mask_in = torch.tensor(aug_m, dtype=torch.bool, device=device)
                tgt_const = torch.tensor(tgt_c, dtype=torch.float32, device=device)
                tgt_mask = torch.tensor(tgt_m, dtype=torch.bool, device=device)
            else:
                const_in = const_off_base
                mask_in = mask_off_base
                tgt_const = const_hlt_base
                tgt_mask = mask_hlt_base

            pred_hlt, pred_exist_logit = model(const_in, mask_in)
            ...
            loss_rec = soft["total"].mean()
            loss = loss_rec
            loss_rank = torch.zeros((), device=device)

            # Ranking regularization: match D_hard ordering on candidate pools.
            if float(rank_loss_w) > 0.0 and int(rank_k) >= 2 and (bi % max(1, int(rank_every)) == 0):
                co_np = const_off_base.detach().cpu().numpy().astype(np.float32)
                mo_np = mask_off_base.detach().cpu().numpy().astype(bool)
                ch_np = const_hlt_base.detach().cpu().numpy().astype(np.float32)
                mh_np = mask_hlt_base.detach().cpu().numpy().astype(bool)
                ...
                tgt_rep = np.repeat(ch_np, k, axis=0).astype(np.float32)
                tgt_mask_rep = np.repeat(mh_np, k, axis=0).astype(bool)
                ...
                tgt_f = torch.tensor(tgt_rep, dtype=torch.float32, device=device)
                tgt_mf = torch.tensor(tgt_mask_rep, dtype=torch.bool, device=device)

                ph_r, pl_r = model(cand_cf, cand_mf)
                pw_r = torch.sigmoid(pl_r)
                ...
                keys_base = _make_batch_keys(ep, bi, b, seed=int(seed) + 1777, offset=500)
                keys_rep = np.repeat(keys_base, k).astype(np.int64)
                ...
                hard_total = hard_rank["total"].reshape(b, k)
                hard_count = hard_rank["count"].reshape(b, k)
                hard_best = torch.argmin(hard_total, dim=1)

                rank_ce = F.cross_entropy(-soft_rank / rank_temp, hard_best)
                feas_label = ((hard_total <= float(eps_total)) & (hard_count <= float(eps_count))).float()
                feas_logit = (float(eps_total) - soft_rank) / rank_feas_temp
                rank_bce = F.binary_cross_entropy_with_logits(feas_logit, feas_label)
                loss_rank = rank_ce + float(rank_feas_w) * rank_bce
                loss = loss + float(rank_loss_w) * loss_rank
            ...
            tr_rec += float(loss_rec.item()) * bs
            tr_rank += float(loss_rank.item()) * bs
            tr_set += float(soft["set"].mean().item()) * bs
            tr_cnt += float(soft["count"].mean().item()) * bs
            tr_pt += float(soft["pt"].mean().item()) * bs
            tr_mass += float(soft["mass"].mean().item()) * bs
            tr_n += bs

        # Validation on clean pairs only.
        model.eval()
        va_total = 0.0
        va_n = 0
        with torch.no_grad():
            for batch in val_loader:
                ...
                ph, pl = model(const_off, mask_off)
                pw = torch.sigmoid(pl)
                soft = _soft_residual_vec(
                    pred_const=ph,
                    pred_w=pw,
                ...
                va_total += float(lv.item()) * bs
                va_n += bs

        tr = tr_total / max(1, tr_n)
        tr_rec_m = tr_rec / max(1, tr_n)
        tr_rank_m = tr_rank / max(1, tr_n)
        trs = tr_set / max(1, tr_n)
        trc = tr_cnt / max(1, tr_n)
        trp = tr_pt / max(1, tr_n)
        trm = tr_mass / max(1, tr_n)
        va = va_total / max(1, va_n)
        ...
        if (ep + 1) % 2 == 0 or ep == 0:
            print(
                f"DsoftV2 ep {ep+1:03d}: train={tr:.5f} rec={tr_rec_m:.5f} rank={tr_rank_m:.5f} "
                f"set={trs:.5f} cnt={trc:.5f} pt={trp:.5f} mass={trm:.5f} "
                f"val={va:.5f} best={best_val:.5f}@{best_epoch}"
            )

        if no_imp >= int(patience):
            print(f"DsoftV2 early stop at ep {ep+1}")
            break
    ...
    return model, {
        ...
        "train_objective": "aligned_residual + augmented_pairs + rank_regularization",
    }
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="m33 D_hard / D_soft probe v2")
    p.add_argument("--train_path", type=str, default="./data")
    p.add_argument("--save_dir", type=str, default="checkpoints/reco_teacher_joint_fusion_6model_150k75k150k/model33_dhard_dsoft_probe_v2")
    p.add_argument("--run_name", type=str, default="model33_dhard_dsoft_probe_v2_debug_seed0")
    ...
    p.add_argument("--degrader_loss_w_pt", type=float, default=0.12)
    p.add_argument("--degrader_loss_w_mass", type=float, default=0.06)
    p.add_argument("--degrader_aug_prob", type=float, default=0.75)
    p.add_argument("--degrader_aug_pt_sigma", type=float, default=0.08)
    p.add_argument("--degrader_aug_eta_sigma", type=float, default=0.05)
    p.add_argument("--degrader_aug_phi_sigma", type=float, default=0.05)
    p.add_argument("--degrader_aug_e_sigma", type=float, default=0.08)
    p.add_argument("--degrader_aug_drop_prob", type=float, default=0.08)
    p.add_argument("--degrader_rank_k", type=int, default=6)
    p.add_argument("--degrader_rank_every", type=int, default=1)
    p.add_argument("--degrader_rank_loss_w", type=float, default=0.25)
    p.add_argument("--degrader_rank_temp", type=float, default=0.25)
    p.add_argument("--degrader_rank_feas_w", type=float, default=0.10)
    p.add_argument("--degrader_rank_feas_temp", type=float, default=0.08)
...
def main() -> None:
    ...
    print("=" * 72)
    print("m33 D_hard / D_soft probe v2")
    ...
    # ------------------------------------------------------------------
    print("Training D_soft surrogate (degrader v2)...")
    ...
    degrader, degrader_metrics = _train_degrader_v2(
        model=degrader,
        train_loader=dl_tr,
        val_loader=dl_va,
        device=device,
        cfg=cfg,
        dhard_base_seed=int(args.seed + args.dhard_seed_offset),
        seed=int(args.seed),
        ...
        w_chamfer=float(args.res_w_chamfer),
        w_count=float(args.degrader_loss_w_count),
        w_pt=float(args.degrader_loss_w_pt),
        w_mass=float(args.degrader_loss_w_mass),
        unmatched_penalty=float(args.unmatched_penalty),
        aug_prob=float(args.degrader_aug_prob),
        aug_pt_sigma=float(args.degrader_aug_pt_sigma),
        aug_eta_sigma=float(args.degrader_aug_eta_sigma),
        aug_phi_sigma=float(args.degrader_aug_phi_sigma),
        aug_e_sigma=float(args.degrader_aug_e_sigma),
        aug_drop_prob=float(args.degrader_aug_drop_prob),
        rank_k=int(args.degrader_rank_k),
        rank_every=int(args.degrader_rank_every),
        rank_loss_w=float(args.degrader_rank_loss_w),
        rank_temp=float(args.degrader_rank_temp),
        rank_feas_w=float(args.degrader_rank_feas_w),
        rank_feas_temp=float(args.degrader_rank_feas_temp),
        eps_total=float(args.eps_total),
        eps_count=float(args.eps_count),
    )
    ...
    report = {
        "model": "m33_dhard_dsoft_probe_v2",
        "seed": int(args.seed),
    ...
    print("=" * 72)
    print("m33 D_hard / D_soft probe v2 summary")
    print("=" * 72)
