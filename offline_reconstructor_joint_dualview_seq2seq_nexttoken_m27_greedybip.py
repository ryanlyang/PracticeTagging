...
def _greedy_bipartite_assignment(
    ...
    """Greedy one-to-one bipartite assignment by ascending pair cost."""
    n_p = int(cost.shape[0])
    n_t = int(cost.shape[1])
    dev = cost.device
    if n_p <= 0 or n_t <= 0:
    ...
    flat_order = np.argsort(c_np, axis=None)
    used_rows = np.zeros(n_p, dtype=np.bool_)
    used_cols = np.zeros(n_t, dtype=np.bool_)
    rows: List[int] = []
    cols: List[int] = []
    k_max = min(n_p, n_t)
    ...
    for flat_idx in flat_order.tolist():
        ri = int(flat_idx // n_t)
        ci = int(flat_idx % n_t)
        if used_rows[ri] or used_cols[ci]:
            continue
        used_rows[ri] = True
        used_cols[ci] = True
        rows.append(ri)
    ...
    row_t = torch.as_tensor(rows, dtype=torch.long, device=dev)
    col_t = torch.as_tensor(cols, dtype=torch.long, device=dev)
    return row_t, col_t
...
def hungarian_token_loss(
    ...
    for bi in range(bsz):
        ...
        row_t, col_t = _greedy_bipartite_assignment(cost)
...
def hungarian_align_targets(
    ...
    """Align target tokens to prediction slots using greedy bipartite matching.
...
def main() -> None:
    ...
    print("Matching backend (m27 test): greedy bipartite (Hungarian calls replaced)")
