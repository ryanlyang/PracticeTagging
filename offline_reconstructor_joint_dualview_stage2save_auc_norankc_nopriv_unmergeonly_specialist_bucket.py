...
def main() -> None:
    ...
    parser.add_argument("--spec_bucket_enable", action="store_true")
    ...
    if bool(args.disc_weight_enable) and bool(args.spec_bucket_enable):
        raise ValueError("Use either --disc_weight_enable or --spec_bucket_enable, not both together.")
    ...
    # Optional teacher/baseline split scores reused by discrepancy weighting and specialist bucketing.
    need_split_scores = bool(args.disc_weight_enable) or bool(args.spec_bucket_enable)
    p_teacher_train: Optional[np.ndarray] = None
    p_teacher_val: Optional[np.ndarray] = None
    p_baseline_train: Optional[np.ndarray] = None
    p_baseline_val: Optional[np.ndarray] = None
    if need_split_scores:
        ...
        if not np.array_equal(y_teacher_train.astype(np.int64), y_baseline_train.astype(np.int64)):
            raise RuntimeError("Teacher/baseline train label mismatch while building weights.")
        if not np.array_equal(y_teacher_val.astype(np.int64), y_baseline_val.astype(np.int64)):
            raise RuntimeError("Teacher/baseline val label mismatch while building weights.")
    ...
    if bool(args.disc_weight_enable):
        assert p_teacher_train is not None and p_teacher_val is not None
        assert p_baseline_train is not None and p_baseline_val is not None
    ...
    # Specialist-bucket weighting vectors (optional) for a second model path.
    spec_weight_train = np.ones((len(train_idx),), dtype=np.float32)
    spec_weight_val = np.ones((len(val_idx),), dtype=np.float32)
    spec_bucket_mask_train = np.zeros((len(train_idx),), dtype=np.uint8)
    spec_bucket_mask_val = np.zeros((len(val_idx),), dtype=np.uint8)
    spec_bucket_mask_test = np.zeros((len(test_idx),), dtype=np.uint8)
    specialist_summary_train: Dict[str, float] = {"enabled": False}
    specialist_summary_val: Dict[str, float] = {"enabled": False}
    specialist_summary_test: Dict[str, float] = {"enabled": False}
    if bool(args.spec_bucket_enable):
        ...
        p_baseline_test = preds_baseline.astype(np.float32)

        spec_weight_train, spec_bucket_mask_train, specialist_summary_train = build_specialist_bucket_weights(
            labels=y_train_i64,
        ...
        spec_weight_val, spec_bucket_mask_val, specialist_summary_val = build_specialist_bucket_weights(
            labels=y_val_i64,
        ...
        _, spec_bucket_mask_test, specialist_summary_test = build_specialist_bucket_weights(
            labels=y_test_i64,
        ...
        specialist_summary_train["enabled"] = True
        specialist_summary_val["enabled"] = True
        specialist_summary_test["enabled"] = True
        np.savez_compressed(
            save_root / "specialist_bucket_weights.npz",
            train_idx=train_idx.astype(np.int64),
            val_idx=val_idx.astype(np.int64),
            test_idx=test_idx.astype(np.int64),
            weight_train=spec_weight_train.astype(np.float32),
            weight_val=spec_weight_val.astype(np.float32),
            mask_train=spec_bucket_mask_train.astype(np.uint8),
            mask_val=spec_bucket_mask_val.astype(np.uint8),
            mask_test=spec_bucket_mask_test.astype(np.uint8),
        )
        print(
            "Specialist bucket weights (train): "
            f"traffic={specialist_summary_train.get('traffic', float('nan')):.4f}, "
            f"mean={specialist_summary_train.get('mean_weight', float('nan')):.4f}, "
            f"p95={specialist_summary_train.get('p95_weight', float('nan')):.4f}"
        )
    ...
    # Optional specialist branch: full-data training with bucket-focused weights.
    ...
    preds_joint_spec = None
    auc_routed_jointbase = float("nan")
    auc_routed_hltbase = float("nan")
    fpr30_routed_jointbase = float("nan")
    fpr50_routed_jointbase = float("nan")
    fpr30_routed_hltbase = float("nan")
    fpr50_routed_hltbase = float("nan")
    fpr_route_jointbase = np.array([], dtype=np.float64)
    tpr_route_jointbase = np.array([], dtype=np.float64)
    fpr_route_hltbase = np.array([], dtype=np.float64)
    tpr_route_hltbase = np.array([], dtype=np.float64)
    if bool(args.spec_bucket_enable):
        print("\n" + "=" * 70)
        ...
        ds_train_reco_spec = WeightedReconstructionDataset(
            feat_hlt_std[train_idx], hlt_mask[train_idx], hlt_const[train_idx],
            const_off[train_idx], masks_off[train_idx],
            budget_merge_true[train_idx], budget_eff_true[train_idx],
            sample_weight_reco=spec_weight_train,
        )
        ds_val_reco_spec = WeightedReconstructionDataset(
            feat_hlt_std[val_idx], hlt_mask[val_idx], hlt_const[val_idx],
            const_off[val_idx], masks_off[val_idx],
            budget_merge_true[val_idx], budget_eff_true[val_idx],
            sample_weight_reco=spec_weight_val,
        )
        dl_train_reco_spec = DataLoader(
            ds_train_reco_spec,
            batch_size=int(cfg["reconstructor_training"]["batch_size"]),
            shuffle=True,
            drop_last=True,
        ...
        reconstructor_spec = OfflineReconstructor(input_dim=7, **cfg["reconstructor_model"]).to(device)
        reconstructor_spec = wrap_reconstructor_unmerge_only(reconstructor_spec)
        reconstructor_spec, specialist_stageA_metrics = train_reconstructor_weighted(
            reconstructor_spec,
            dl_train_reco_spec,
            dl_val_reco_spec,
            device,
        ...
        specialist_stageA_metrics["enabled"] = True

        stageb_w_spec = np.array([spec_weight_train[train_pos[int(j)]] for j in stageB_train_idx], dtype=np.float32)
        ds_train_joint_spec = JointDualDataset(
            feat_hlt_std[train_idx], feat_hlt_dual[train_idx], hlt_mask[train_idx], hlt_const[train_idx],
            const_off[train_idx], masks_off[train_idx],
            budget_merge_true[train_idx], budget_eff_true[train_idx],
            labels[train_idx],
            sample_weight_cls=spec_weight_train,
            sample_weight_reco=spec_weight_train,
        )
        ds_train_joint_spec_stageb = JointDualDataset(
            feat_hlt_std[stageB_train_idx], feat_hlt_dual[stageB_train_idx], hlt_mask[stageB_train_idx], hlt_const[stageB_train_idx],
            const_off[stageB_train_idx], masks_off[stageB_train_idx],
            budget_merge_true[stageB_train_idx], budget_eff_true[stageB_train_idx],
            labels[stageB_train_idx],
            sample_weight_cls=stageb_w_spec,
            sample_weight_reco=stageb_w_spec,
        )
        ds_val_joint_spec = JointDualDataset(
            feat_hlt_std[val_idx], feat_hlt_dual[val_idx], hlt_mask[val_idx], hlt_const[val_idx],
            const_off[val_idx], masks_off[val_idx],
            budget_merge_true[val_idx], budget_eff_true[val_idx],
            labels[val_idx],
            sample_weight_cls=spec_weight_val,
            sample_weight_reco=spec_weight_val,
        )
        ds_test_joint_spec = JointDualDataset(
            feat_hlt_std[test_idx], feat_hlt_dual[test_idx], hlt_mask[test_idx], hlt_const[test_idx],
            const_off[test_idx], masks_off[test_idx],
            budget_merge_true[test_idx], budget_eff_true[test_idx],
            labels[test_idx],
        ...
        dl_train_joint_spec = DataLoader(
            ds_train_joint_spec, batch_size=BS, shuffle=True, drop_last=True,
            num_workers=args.num_workers, pin_memory=torch.cuda.is_available(),
        )
        dl_train_joint_spec_stageb = DataLoader(
            ds_train_joint_spec_stageb, batch_size=BS, shuffle=True, drop_last=True,
            num_workers=args.num_workers, pin_memory=torch.cuda.is_available(),
        )
        dl_val_joint_spec = DataLoader(
            ds_val_joint_spec, batch_size=BS, shuffle=False,
            num_workers=args.num_workers, pin_memory=torch.cuda.is_available(),
        )
        dl_test_joint_spec = DataLoader(
            ds_test_joint_spec, batch_size=BS, shuffle=False,
            num_workers=args.num_workers, pin_memory=torch.cuda.is_available(),
        )
        ...
        reconstructor_spec, dual_spec, specialist_stageB_metrics, specialist_stageB_states = train_joint_dual(
            ...
            val_loader=dl_val_joint_spec,
            device=device,
            stage_name="StageB-Specialist",
            freeze_reconstructor=True,
            epochs=int(args.stageB_epochs),
            patience=int(args.stageB_patience),
            lr_dual=float(args.stageB_lr_dual),
        ...
        specialist_stageC_metrics["enabled"] = True

        auc_joint_spec, preds_joint_spec, labs_joint_spec, _ = eval_joint_model(
            reconstructor_spec,
            dual_spec,
            dl_test_joint_spec,
            device,
        ...
        assert np.array_equal(labs.astype(np.float32), labs_joint_spec.astype(np.float32))

        route_mask_test = spec_bucket_mask_test.astype(bool)
        preds_routed_jointbase = np.where(route_mask_test, preds_joint_spec, preds_joint).astype(np.float32)
        preds_routed_hltbase = np.where(route_mask_test, preds_joint_spec, preds_baseline).astype(np.float32)
        auc_routed_jointbase = float(roc_auc_score(labs, preds_routed_jointbase))
        auc_routed_hltbase = float(roc_auc_score(labs, preds_routed_hltbase))
        fpr_route_jointbase, tpr_route_jointbase, _ = roc_curve(labs, preds_routed_jointbase)
        fpr_route_hltbase, tpr_route_hltbase, _ = roc_curve(labs, preds_routed_hltbase)
        fpr30_routed_jointbase = fpr_at_target_tpr(fpr_route_jointbase, tpr_route_jointbase, 0.30)
        fpr50_routed_jointbase = fpr_at_target_tpr(fpr_route_jointbase, tpr_route_jointbase, 0.50)
        fpr30_routed_hltbase = fpr_at_target_tpr(fpr_route_hltbase, tpr_route_hltbase, 0.30)
        fpr50_routed_hltbase = fpr_at_target_tpr(fpr_route_hltbase, tpr_route_hltbase, 0.50)

        if not args.skip_save_models:
            torch.save({"model": reconstructor_spec.state_dict(), "val": specialist_stageA_metrics}, save_root / "offline_reconstructor_specialist.pt")
            torch.save({"model": dual_spec.state_dict(), "auc": float(auc_joint_spec)}, save_root / "dual_joint_specialist.pt")
    ...
    if bool(args.spec_bucket_enable) and preds_joint_spec is not None:
        print()
        print(f"Specialist Joint AUC: {auc_joint_spec:.4f}")
        print(
            f"Routed (Base=Joint, Bucket->Specialist) AUC/FPR30/FPR50: "
            f"{auc_routed_jointbase:.4f} / {fpr30_routed_jointbase:.6f} / {fpr50_routed_jointbase:.6f}"
        )
        print(
            f"Routed (Base=HLT, Bucket->Specialist)   AUC/FPR30/FPR50: "
            f"{auc_routed_hltbase:.4f} / {fpr30_routed_hltbase:.6f} / {fpr50_routed_hltbase:.6f}"
        )
        print(
            f"Specialist test traffic: "
            f"{int(spec_bucket_mask_test.sum())} / {len(spec_bucket_mask_test)} "
            f"({float(np.mean(spec_bucket_mask_test)):.4f})"
        )
    if preds_joint_kd is not None:
    ...
    if bool(args.spec_bucket_enable) and preds_joint_spec is not None:
        fpr_spec, tpr_spec, _ = roc_curve(labs, preds_joint_spec)
        plot_lines.append((tpr_spec, fpr_spec, ":", f"Specialist Joint (AUC={auc_joint_spec:.3f})", "teal"))
        plot_lines.append(
            (
                tpr_route_jointbase,
                fpr_route_jointbase,
                "-",
                f"Routed JointBase+Spec (AUC={auc_routed_jointbase:.3f})",
                "darkgreen",
            )
        )
        plot_lines.append(
            (
                ...
                fpr_route_hltbase,
                "--",
                f"Routed HLTBase+Spec (AUC={auc_routed_hltbase:.3f})",
                "sienna",
            )
        )
    if preds_joint_kd is not None:
    ...
    np.savez(
        ...
        auc_joint_specialist=auc_joint_spec,
        auc_routed_jointbase=auc_routed_jointbase,
        auc_routed_hltbase=auc_routed_hltbase,
        fpr_teacher=fpr_t,
        tpr_teacher=tpr_t,
        fpr_baseline=fpr_b,
        tpr_baseline=tpr_b,
        ...
        fpr_routed_jointbase=fpr_route_jointbase,
        tpr_routed_jointbase=tpr_route_jointbase,
        fpr_routed_hltbase=fpr_route_hltbase,
        tpr_routed_hltbase=tpr_route_hltbase,
        ...
        fpr30_routed_hltbase=fpr30_routed_hltbase,
        ...
        fpr50_routed_hltbase=fpr50_routed_hltbase,
        specialist_bucket_mask_test=spec_bucket_mask_test.astype(np.uint8),
        ...
        json.dump(
            {
                "variant": {
                    ...
                    "specialist_bucket_enable": bool(args.spec_bucket_enable),
                ...
                "specialist_bucket": {
                    ...
                    "stageA_specialist": specialist_stageA_metrics,
                    "stageB_specialist": specialist_stageB_metrics,
                    "stageC_specialist": specialist_stageC_metrics,
                    "auc_stage2_specialist": float(auc_stage2_spec) if preds_stage2_spec is not None else None,
                    "auc_joint_specialist": float(auc_joint_spec) if preds_joint_spec is not None else None,
                    "auc_routed_jointbase": float(auc_routed_jointbase) if preds_joint_spec is not None else None,
                    "auc_routed_hltbase": float(auc_routed_hltbase) if preds_joint_spec is not None else None,
                    ...
                    "fpr30_routed_hltbase": float(fpr30_routed_hltbase) if preds_joint_spec is not None else None,
                    "fpr50_routed_hltbase": float(fpr50_routed_hltbase) if preds_joint_spec is not None else None,
                },
                "test_stage2": {
