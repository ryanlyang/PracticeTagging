...
"""
m38 stage-2:
Train one final top tagger from six completed m38 prefix runs.
...
import offline_reconstructor_joint_dualview_stage2save_auc_norankc_nopriv_unmergeonly_m38_seeded_m28_detresid_multicand as m38
...
def _combine_outputs(
    outputs: Sequence[m38.CandidateSplitOutput],
    keep_m: int,
    eps_total: float,
    eps_count: float,
) -> m38.CandidateSplitOutput:
    if len(outputs) < 1:
    ...
    return m38.CandidateSplitOutput(
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="m38 stage2: merge six prefix runs and train one final dualview top tagger")

    p.add_argument("--train_path", type=str, default="./data")
    p.add_argument("--save_dir", type=str, default="checkpoints/reco_teacher_joint_fusion_6model_150k75k150k/model38_prefix6_stage2")
    p.add_argument("--run_name", type=str, default="model38_prefix6_stage2_150k75k300k_seed0")

    p.add_argument("--stage1_save_dir", type=str, default="checkpoints/reco_teacher_joint_fusion_6model_150k75k150k/model38_seeded_m28_detresid_multicand")
    p.add_argument(
        "--stage1_run_names",
        type=str,
        default=(
            "model38_k6_seeded_m28_detresid_multicand_carryboost_150k75k300k_seed0_pfx0,"
            "model38_k6_seeded_m28_detresid_multicand_carryboost_150k75k300k_seed0_pfx3,"
            "model38_k6_seeded_m28_detresid_multicand_carryboost_150k75k300k_seed0_pfx6,"
            "model38_k6_seeded_m28_detresid_multicand_carryboost_150k75k300k_seed0_pfx9,"
            "model38_k6_seeded_m28_detresid_multicand_carryboost_150k75k300k_seed0_pfx12,"
            "model38_k6_seeded_m28_detresid_multicand_carryboost_150k75k300k_seed0_pfx15"
        ),
    )
...
def main() -> None:
    ...
    print("=" * 72)
    print("Model-38 Prefix6 Stage2 (Single final top tagger from six reconstructor runs)")
    ...
    # ---------------------------------------------------------------------
    print("\n" + "=" * 72)
    print("STEP 1: Rebuild deterministic candidates from six stage1 runs")
    print("=" * 72)

    c_tr_runs: List[m38.CandidateSplitOutput] = []
    c_va_runs: List[m38.CandidateSplitOutput] = []
    c_te_runs: List[m38.CandidateSplitOutput] = []
    ...
    for ridx, (rd, pf) in enumerate(zip(run_dirs, prefixes)):
        ...
        carry_model = m38.CarryoverTokenPredictor(
            input_dim=7,
        ...
        c_tr = m38._generate_candidates_split(
        ...
        c_va = m38._generate_candidates_split(
            reco_model=reco_model,
            carry_model=carry_model,
        ...
        c_te = m38._generate_candidates_split(
            reco_model=reco_model,
            carry_model=carry_model,
    ...
    mv_tr = m38._build_m38_multicandidate_arrays(c_tr, max_constits=int(args.max_constits), candidate_k=len(run_dirs))
    mv_va = m38._build_m38_multicandidate_arrays(c_va, max_constits=int(args.max_constits), candidate_k=len(run_dirs))
    mv_te = m38._build_m38_multicandidate_arrays(c_te, max_constits=int(args.max_constits), candidate_k=len(run_dirs))
    ...
    ds_dv_tr = m38.DualViewM38Dataset(
        feat_hlt=feat_hlt_tr,
        mask_hlt=mask_hlt[train_idx],
    ...
    ds_dv_va = m38.DualViewM38Dataset(
        feat_hlt=feat_hlt_va,
        mask_hlt=mask_hlt[val_idx],
    ...
    ds_dv_te = m38.DualViewM38Dataset(
        feat_hlt=feat_hlt_te,
        mask_hlt=mask_hlt[test_idx],
    ...
    model_nog = m38.MultiCandidateM38NoGate(
    ...
    model_nog, met_nog = m38._train_m38_model(
        model=model_nog,
    ...
    model_gat = m38.MultiCandidateM38Gated(
        cand_meta_dim=int(mv_tr["cand_meta"].shape[-1]),
        summary_dim=int(mv_tr["summary_feat"].shape[-1]),
        embed_dim=int(args.embed_dim),
        num_heads=int(args.num_heads),
    ...
    model_gat, met_gat = m38._train_m38_model(
        model=model_gat,
        train_loader=dl_dv_tr,
        val_loader=dl_dv_va,
        device=device,
    ...
    auc_nog, fpr50_nog, pred_nog, lab_final, w_final = m38._eval_m38_model(model_nog, dl_dv_te, device)
    auc_gat, fpr50_gat, pred_gat, _lab2, _w2 = m38._eval_m38_model(model_gat, dl_dv_te, device)

    print("\n" + "=" * 72)
    print("FINAL TEST")
    print("=" * 72)
    print(
        f"m38-stage2 NoGate AUC={auc_nog:.4f} FPR50={fpr50_nog:.6f} | "
        f"m38-stage2 Gated AUC={auc_gat:.4f} FPR50={fpr50_gat:.6f}"
    )

    torch.save({"model": model_nog.state_dict(), "metrics": met_nog}, save_root / "m38_stage2_nogate.pt")
    torch.save({"model": model_gat.state_dict(), "metrics": met_gat}, save_root / "m38_stage2_gated.pt")

    np.savez_compressed(
        save_root / "m38_stage2_test_scores.npz",
        labels_test=lab_final.astype(np.float32),
        preds_m38_stage2_nogate=pred_nog.astype(np.float32),
        preds_m38_stage2_gated=pred_gat.astype(np.float32),
        sample_weight=np.asarray(w_final, dtype=np.float32),
        auc_m38_stage2_nogate=float(auc_nog),
        auc_m38_stage2_gated=float(auc_gat),
        fpr50_m38_stage2_nogate=float(fpr50_nog),
        fpr50_m38_stage2_gated=float(fpr50_gat),
    )
    ...
    if bool(args.save_fusion_scores):
        np.savez_compressed(
            ...
            labels_test=lab_final.astype(np.float32),
            preds_m38_stage2_nogate=np.asarray(pred_nog, dtype=np.float32),
            preds_m38_stage2_gated=np.asarray(pred_gat, dtype=np.float32),
            sample_weight=np.asarray(w_final, dtype=np.float32),
        )

    report = {
        "model": "m38_prefix6_stage2",
        "seed": int(args.seed),
    ...
    with open(save_root / "m38_stage2_report.json", "w", encoding="utf-8") as f:
