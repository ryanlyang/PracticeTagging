...
- Single change: token log-pT upper clamp is raised from 9.0 -> 10.0
"""
...
class OfflineReconstructorPtClamp10(reco_base.OfflineReconstructor):
    def forward(
        self,
        ...
        # Single ablation change: raise max log-pT clamp from 9.0 to 10.0.
        tok_pt = torch.exp(torch.clamp(torch.log(pt) + d_logpt, min=-9.0, max=10.0))
