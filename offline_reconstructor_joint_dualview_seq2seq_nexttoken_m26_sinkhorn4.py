...
LOSS_CFG = {
    ...
    "use_sinkhorn_set": False,
    "use_sinkhorn_ar": False,
    "sinkhorn_tau": 0.08,
    "sinkhorn_iters": 20,
    "huber_delta": 0.12,
...
def sinkhorn_align_targets_and_set(
    ...
    """Differentiable soft alignment (Sinkhorn OT) for AR targets and set cost.

    Returns:
      aligned_tgt_tok: [B,T,D] soft target per prediction slot
      aligned_mask:    [B,T] active prediction slots
      match_cost:      [B,T] row-wise expected matching cost
      set_loss:        scalar mean set loss over batch
    """
...
def compute_reco_losses(
    ...
    set_mode = str(loss_cfg.get("set_loss_mode", "chamfer")).strip().lower()
    use_sinkhorn_set = bool(loss_cfg.get("use_sinkhorn_set", False))
    use_sinkhorn_ar = bool(loss_cfg.get("use_sinkhorn_ar", False))

    sink_ar_tgt_tok = None
    sink_ar_tgt_mask = None
    sink_match_cost = None
    sink_loss_set = None
    if use_sinkhorn_set or use_sinkhorn_ar:
        sink_ar_tgt_tok, sink_ar_tgt_mask, sink_match_cost, sink_loss_set = sinkhorn_align_targets_and_set(
            pred_tok,
            tgt_tok,
            pred_mask_for_set,
            tgt_mask,
    ...
    ar_use_hungarian = bool(loss_cfg.get("ar_use_hungarian_target", True)) and set_mode == "hungarian"
    if use_sinkhorn_ar:
        ar_tgt_tok = sink_ar_tgt_tok
        ar_tgt_mask = sink_ar_tgt_mask
        match_cost = sink_match_cost
    elif ar_use_hungarian:
...
def main() -> None:
    ...
    parser.add_argument("--use_sinkhorn_set", action="store_true")
    parser.add_argument("--use_sinkhorn_ar", action="store_true")
    ...
    loss_cfg = {
        ...
        "use_sinkhorn_set": bool(args.use_sinkhorn_set),
        "use_sinkhorn_ar": bool(args.use_sinkhorn_ar),
