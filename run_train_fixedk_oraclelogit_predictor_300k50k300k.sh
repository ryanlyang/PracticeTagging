#!/usr/bin/env bash
# Fixed-K predictor with frozen-oracle-logit distillation.
#
# Submit:
#   sbatch run_train_fixedk_oraclelogit_predictor_300k50k300k.sh

#SBATCH --job-name=fixkLogitD
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --time=2-00:00:00
#SBATCH --output=offline_reconstructor_logs/fixedk_oraclelogit_300k50k300k_%j.out
#SBATCH --error=offline_reconstructor_logs/fixedk_oraclelogit_300k50k300k_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs

RUN_NAME="${RUN_NAME:-fixedk_oraclelogit_300k50k300k_k8}"
TRAIN_PATH="${TRAIN_PATH:-$SLURM_SUBMIT_DIR/data}"
SAVE_DIR="${SAVE_DIR:-checkpoints/fixedk_oraclelogit_predictor}"
...
# Distillation phase (frozen oracle tagger)
DISTILL_EPOCHS="${DISTILL_EPOCHS:-20}"
DISTILL_PATIENCE="${DISTILL_PATIENCE:-6}"
DISTILL_LR="${DISTILL_LR:-1e-4}"
DISTILL_WEIGHT_DECAY="${DISTILL_WEIGHT_DECAY:-1e-5}"
DISTILL_WARMUP_EPOCHS="${DISTILL_WARMUP_EPOCHS:-2}"
DISTILL_W_LOGIT="${DISTILL_W_LOGIT:-1.0}"
DISTILL_W_HUNGARIAN="${DISTILL_W_HUNGARIAN:-0.15}"

# Optional classifier override for teacher/baseline/oracle-tagger; keep -1 for script defaults.
...
echo "=================================================="
echo "Fixed-K Oracle-Logit Distillation Run"
echo "=================================================="
...
python train_fixedk_oraclelogit_predictor.py \
  ...
  --distill_patience "${DISTILL_PATIENCE}" \
  --distill_lr "${DISTILL_LR}" \
  --distill_weight_decay "${DISTILL_WEIGHT_DECAY}" \
  --distill_warmup_epochs "${DISTILL_WARMUP_EPOCHS}" \
  --distill_w_logit "${DISTILL_W_LOGIT}" \
  --distill_w_hungarian "${DISTILL_W_HUNGARIAN}" \
