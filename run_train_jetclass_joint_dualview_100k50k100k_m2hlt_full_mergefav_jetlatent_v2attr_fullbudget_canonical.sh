#!/usr/bin/env bash
#SBATCH --job-name=jcJLatVC
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --time=7:00:00
#SBATCH --output=offline_reconstructor_logs/jetclass_joint_dualview_100k50k100k_m2hlt_full_mergefav_jetlatent_v2attr_fullbudget_canonical_%j.out
#SBATCH --error=offline_reconstructor_logs/jetclass_joint_dualview_100k50k100k_m2hlt_full_mergefav_jetlatent_v2attr_fullbudget_canonical_%j.err
...
RUN_NAME="${RUN_NAME:-jetclass_joint_v2attr_100k50k100k_m2hlt_full_mergefav_jetlatent_fullbudget_canonical}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
...
# HLT profile knobs (m2hlt merge-favoring profile currently used).
...
# Reconstructor/loss knobs (full-budget style).
...
SCRIPT="${SCRIPT:-$(pwd)/train_jetclass_joint_dualview_stage2_unmergeonly_m2hlt_jetlatent_set2set_v2_attr.py}"
...
echo "============================================================"
echo "JetClass Joint Dual-View V2 Attr (m2hlt mergefav + jetlatent set2set, full in/out)"
