"""
Unsmear utilities (我们的体系版本).

目标：
- 生成监督数据：输入为 smear 后的 HLT view，监督目标为 merge 后 smear 前的 view。
- 这里默认仍在 token-level 上做去噪（不尝试恢复被 merge 丢失的 children 自由度）。

说明：
- 为了让监督更稳定，特征在 jet-axis frame 下计算，并且 axis 统一使用 *post-smear* 的 jet axis
  （推理时可观测），避免“输入/目标 axis 不一致”带来的额外差异。
- 支持 efficiency loss：训练/监督仅在 survive 的 token 上计算（mask=True）。
"""
...
def wrap_dphi_torch(dphi: torch.Tensor) -> torch.Tensor:
    """将角差 wrap 到 (-pi, pi]（torch 版，用于 loss）。"""
    return torch.atan2(torch.sin(dphi), torch.cos(dphi))


@dataclass
class HLTEffectsCfg:
    """HLT effects config（只保留 unsmear 需要的字段）."""
...
def apply_hlt_effects_pair(
    ...
    *,
    ...
    """
    生成 (pre_smear, post_smear) 成对数据，并保证 efficiency loss mask 一致。

    Args:
      const: [N,S,4] (pt,eta,phi,E)
      mask:  [N,S] bool
    Returns:
      pre_smear_const:  [N,S,4] merge 后 smear 前（并应用同一 effloss mask）
      post_smear_const: [N,S,4] merge 后 smear 后（并应用 effloss）
      post_mask:        [N,S] bool survive mask（用于训练/评估）
      origin_counts:    [N,S] int32，每个 token 合并了多少原始 token（singleton=1，merged>1）
    """
    rs = np.random.RandomState(int(seed))
    hlt = const.copy()
    hlt_mask = mask.copy()

    # Effect 0: offline threshold（共同起点）
    pt_thr_off = float(cfg.pt_threshold_offline)
    hlt_mask = hlt_mask & (hlt[:, :, 0] >= pt_thr_off)
    hlt[~hlt_mask] = 0.0

    # Effect 1: HLT threshold（可选增阈值）
    ...
    # pre-smear snapshot (before applying smear); do NOT apply effloss yet
    pre = hlt.copy()
    ...
    # Recompute E (massless approx)
    hlt[:, :, 3] = np.where(valid, hlt[:, :, 0] * np.cosh(np.clip(hlt[:, :, 1], -5.0, 5.0)), 0.0)

    # Effect 4: efficiency loss (apply to post; then apply same mask to pre)
...
def compute_jet_axis(const: np.ndarray, mask: np.ndarray) -> Dict[str, np.ndarray]:
    """计算 jet axis（使用 token 集合的 4-vector 求和）."""
...
def compute_features_with_axis(
    ...
    """计算 engineered features（axis 由外部指定）。
...
def masked_smooth_l1(pred: torch.Tensor, tgt: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """对 [B,S,D] 仅在 mask=True 的 token 上计算 SmoothL1."""
...
def masked_smooth_l1_wrap_dphi(
    ...
    *,
    ...
    """SmoothL1，但对 dPhi 维度使用 wrap-aware residual（避免 -pi/pi 边界不连续）。

    注意：如果 dPhi 在标准化空间训练，应该传入 dphi_scale=std(dPhi)，
    让 wrap 在“角度空间”进行：wrap((pred-tgt)*std)/std。
    """
    diff = pred - tgt
...
def masked_mse(pred: torch.Tensor, tgt: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """对 [B,S,D] 仅在 mask=True 的 token 上计算 MSE."""
    m = mask.to(pred.dtype).unsqueeze(-1)
    diff = (pred - tgt) ** 2
    num = (diff * m).sum()
    den = m.sum().clamp_min(1.0)
...
def masked_gaussian_nll(
    ...
    *,
    ...
    """对角高斯 NLL（heteroscedastic regression）。

    Args:
      mu, log_var, tgt: [B,S,D]
      mask: [B,S] bool
      active_dim_mask: [D] bool/0-1，控制哪些维度启用 uncertainty；
        未启用维度将视为 log_var=0（等价固定方差）。
    """
...
def masked_gaussian_nll_wrap_dphi(
    ...
    *,
    ...
    """对角高斯 NLL，但对 dPhi 维度使用 wrap-aware residual。

    同上：标准化空间下应传入 dphi_scale=std(dPhi)。
    """
    log_var = torch.clamp(log_var, min=-float(log_var_clip), max=float(log_var_clip))
    if active_dim_mask is not None:
        adm = active_dim_mask.to(dtype=mu.dtype, device=mu.device).view(1, 1, -1)
...
def feats_to_7d(
    ...
    *,
    ...
    """将 3D/4D 特征补全为 7D engineered features（在 raw feature 空间）。

    主要用于：unsmear 只预测了 3D/4D，但下游希望统一用 7D。

    Args:
      feat: [N,S,Dk] raw feature values（非标准化空间）
      mask: [N,S] bool
      axis: jet axis dict（通常用 post-smear axis；包含 jet_eta/jet_phi/jet_pt/jet_E）
      kind: '3d'/'4d'/'7d'，表示输入 feat 的语义
    Returns:
    ...
    if k == "4d":
        ...
        # 3D 不含能量：用 massless 近似补一个 E（需要绝对 eta）
        eta = dEta + jet_eta
...
# -----------------------------
# Downstream tagger utilities锛堜粠 tagging/Baseline/tool.py 澶嶅埗锛塡n# -----------------------------
