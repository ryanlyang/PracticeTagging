...
"""
Diagnostic analyzer for multihyp-learned runs.

Reports, on val/test split:
- Learned gated multi-hyp performance
- Each individual hypothesis performance
- Oracle-per-jet hypothesis upper bound
- Gate usage/entropy diagnostics
...
import offline_reconstructor_joint_dualview_stage2save_auc_norankc_nopriv_unmergeonly_multihyp_learned as mh
...
def _infer_h_from_state(state_dual: Dict[str, torch.Tensor]) -> int:
    if "hyp_gate.3.weight" in state_dual:
        return int(state_dual["hyp_gate.3.weight"].shape[0])
    if "hyp_gate.3.bias" in state_dual:
        return int(state_dual["hyp_gate.3.bias"].shape[0])
    raise RuntimeError("Could not infer hypothesis count H from dual checkpoint.")


def _infer_gate_hidden_from_state(state_dual: Dict[str, torch.Tensor]) -> int:
    if "hyp_gate.0.weight" in state_dual:
        return int(state_dual["hyp_gate.0.weight"].shape[0])
    raise RuntimeError("Could not infer gate hidden size from dual checkpoint.")


def _infer_dual_input_dims(state_dual: Dict[str, torch.Tensor]) -> Tuple[int, int]:
    ka = "base_model.input_proj_a.0.weight"
    kb = "base_model.input_proj_b.0.weight"
    if ka not in state_dual or kb not in state_dual:
        raise RuntimeError("Could not infer dual input dims from checkpoint.")
    return int(state_dual[ka].shape[1]), int(state_dual[kb].shape[1])
...
def _collect_predictions(
    ...
    dual_model: mh.DualViewCrossAttnClassifierMultiHypLearned,
    loader: DataLoader,
    device: torch.device,
    weight_floor: float,
    include_flags: bool,
    ...
    all_p_gated: List[np.ndarray] = []
    all_p_h: List[np.ndarray] = []
    all_alpha: List[np.ndarray] = []

    eps = 1e-6
    batch_count = 0
    h = len(dual_model.hyp_temps)
    ...
    for batch in loader:
        ...
        hyp_mass: List[torch.Tensor] = []
        for t in dual_model.hyp_temps:
            if abs(float(t) - 1.0) < 1e-8:
                out_h = reco_out
            else:
                w_h = mh._weights_with_temperature(reco_out["cand_weights"], float(t))
                out_h = mh._reco_out_with_cand_weights(reco_out, w_h)

            feat_b, mask_b = mh._ORIG_BUILD_SOFT_VIEW(
                out_h,
                weight_floor=float(weight_floor),
                scale_features_by_weight=True,
                include_flags=bool(include_flags),
            )
            logits_h = dual_model.base_model(feat_hlt_dual, mask_hlt, feat_b, mask_b).squeeze(1)
            mass_h = out_h["cand_weights"].sum(dim=1) / max(float(out_h["cand_weights"].shape[1]), 1.0)
        ...
        gate_logits = dual_model.hyp_gate(gate_in)
        alpha = torch.softmax(gate_logits, dim=1)
        logits_gated = (alpha * logits_stack).sum(dim=1)

        p_h = torch.sigmoid(logits_stack).detach().cpu().numpy().astype(np.float64)
        p_g = torch.sigmoid(logits_gated).detach().cpu().numpy().astype(np.float64)
        a = alpha.detach().cpu().numpy().astype(np.float64)
        ...
        all_p_h.append(p_h)
        all_p_gated.append(p_g)
        all_alpha.append(a)
        all_y.append(yy)

    y = np.concatenate(all_y) if all_y else np.zeros((0,), dtype=np.float64)
    p_gated = np.concatenate(all_p_gated) if all_p_gated else np.zeros((0,), dtype=np.float64)
...
def main():
    ...
    ap.add_argument("--multihyp_temps", type=str, default="0.75,1.00,1.25")
    ...
    h_ckpt = _infer_h_from_state(state_dual)
    gate_hidden = _infer_gate_hidden_from_state(state_dual)
    in_a, in_b = _infer_dual_input_dims(state_dual)
    ...
    if len(hyp_temps) != h_ckpt:
        raise RuntimeError(
            f"--multihyp_temps has {len(hyp_temps)} temps, but checkpoint gate expects H={h_ckpt}."
        )

    # Configure wrapper globals before model creation.
    mh._HYP_TEMPS = tuple(float(t) for t in hyp_temps)
    mh._GATE_HIDDEN = int(gate_hidden)
    ...
    dual_model.load_state_dict(state_dual, strict=True)
    ...
    h = int(p_h.shape[1]) if p_h.ndim == 2 else int(h_ckpt)

    metrics = {
        "gated": _auc_fpr50(y, p_gated),
    ...
    best_fpr = (float("inf"), -1, float("nan"))  # fpr50, h_idx, auc
    best_auc = (-float("inf"), -1, float("nan"))  # auc, h_idx, fpr50
    for i in range(h):
    ...
    if alpha.size > 0:
        ...
        ent_norm = ent / np.log(float(h))
        gate_argmax = np.argmax(alpha, axis=1)
        gate_counts = np.bincount(gate_argmax, minlength=h).astype(np.int64)
        gate_frac = (gate_counts / max(int(gate_counts.sum()), 1)).astype(np.float64)
        max_alpha = alpha.max(axis=1)
    else:
        ent_norm = np.zeros((0,), dtype=np.float64)
        gate_counts = np.zeros((h,), dtype=np.int64)
        gate_frac = np.zeros((h,), dtype=np.float64)
        max_alpha = np.zeros((0,), dtype=np.float64)
    ...
    report = {
        ...
        "metrics": metrics,
        "gate": {
            "entropy_norm_mean": float(np.mean(ent_norm)) if ent_norm.size else float("nan"),
            "entropy_norm_p95": float(np.quantile(ent_norm, 0.95)) if ent_norm.size else float("nan"),
            "max_alpha_mean": float(np.mean(max_alpha)) if max_alpha.size else float("nan"),
            "max_alpha_p95": float(np.quantile(max_alpha, 0.95)) if max_alpha.size else float("nan"),
            "argmax_counts": gate_counts.astype(int).tolist(),
            "argmax_frac": gate_frac.tolist(),
        },
    ...
    print("=" * 60)
    print("MultiHyp Oracle Diagnostic")
    print("=" * 60)
    print(f"run_dir: {run_dir}")
    print(f"stage/split: {args.stage}/{args.split}")
    print(f"n_jets: {report['n_jets']}")
    print(f"hyp_temps: {report['hyp_temps']}")
    print(
        "gated: "
        f"AUC={report['metrics']['gated']['auc']:.4f}, "
        f"FPR50={report['metrics']['gated']['fpr50']:.6f}"
    )
    print(
        "oracle_per_jet: "
    ...
    print(
        "gate: "
        f"entropy_norm_mean={report['gate']['entropy_norm_mean']:.4f}, "
        f"max_alpha_mean={report['gate']['max_alpha_mean']:.4f}, "
        f"argmax_frac={report['gate']['argmax_frac']}"
    )
    print(
        "hyp_diversity: "
    ...
    if report_json:
        ...
        out_path = out_dir / f"{args.stage}_{args.split}_multihyp_oracle.json"
