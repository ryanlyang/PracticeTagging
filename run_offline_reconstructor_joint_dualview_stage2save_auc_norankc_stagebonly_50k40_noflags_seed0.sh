#!/usr/bin/env bash
# Stage-B-only run (frozen Stage A) with corrected merge/eff flags disabled.
# Seed fixed to 0 for reproducibility.
#
# Submit:
#   sbatch run_offline_reconstructor_joint_dualview_stage2save_auc_norankc_stagebonly_50k40_noflags_seed0.sh
#
#SBATCH --job-name=stgBNF0
#SBATCH --partition=debug
#SBATCH --gres=gpu:1
#SBATCH --mem=32G
#SBATCH --time=01:00:00
#SBATCH --output=offline_reconstructor_logs/offline_reco_stageb_noflags0_%j.out
#SBATCH --error=offline_reconstructor_logs/offline_reco_stageb_noflags0_%j.err

set -euo pipefail

mkdir -p offline_reconstructor_logs

RUN_NAME="${RUN_NAME:-joint_stageb_only_50k_40c_noflags_seed0}"
...
echo "Running Stage-B-only (flags OFF) with seed=${SEED}"
