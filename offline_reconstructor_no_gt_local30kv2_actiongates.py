...
class OfflineReconstructorActionGates(reco_base.OfflineReconstructor):
    """
    Action-gates variant:
    - Replace 4-way action softmax with independent sigmoid gates.
    - Keep the rest of the architecture and reconstruction pathways unchanged.
    - Preserve action_prob output for compatibility by normalizing gates per token.
    """
    ...
    def forward(
        ...
        action_gates = torch.sigmoid(action_logits)
        g_keep = action_gates[..., 0]
        g_unsmear = action_gates[..., 1]
        g_split = action_gates[..., 2]
        g_reassign = action_gates[..., 3]
        # Compatibility tensor used by downstream utilities that expect action_prob.
        action_prob = action_gates / (action_gates.sum(dim=-1, keepdim=True) + eps)
        ...
        d_logpt = stage_scale * (g_unsmear + 0.5 * g_reassign) * unsmear_delta[..., 0]
        d_eta = stage_scale * (g_unsmear * unsmear_delta[..., 1] + g_reassign * reassign_delta[..., 0])
        d_phi = stage_scale * (g_unsmear * unsmear_delta[..., 2] + g_reassign * reassign_delta[..., 1])
        d_logE = stage_scale * (g_unsmear + 0.5 * g_reassign) * unsmear_delta[..., 3]
        ...
        tok_weight = (g_keep + g_unsmear + 0.35 * g_reassign + 0.15 * g_split).clamp(0.0, 1.0)
        ...
        split_exist = split_exist * (g_split.unsqueeze(-1) * stage_scale) * mask_hlt.float().unsqueeze(-1)
