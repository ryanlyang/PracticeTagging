...
RUN_NAME="${RUN_NAME:-jetclass_joint_v2attr_500k250k1m_m2hlt_hybridops_goal_sps001}"
SEED="${SEED:-52}"
DEVICE="${DEVICE:-cuda}"
...
GOAL_APPLY_SCALE="${GOAL_APPLY_SCALE:-0.45}"
GOAL_LAMBDA_HEAD="${GOAL_LAMBDA_HEAD:-0.15}"
GOAL_LAMBDA_RESPONSE="${GOAL_LAMBDA_RESPONSE:-0.30}"
GOAL_LAMBDA_AXIS="${GOAL_LAMBDA_AXIS:-0.12}"
...
SCRIPT="${SCRIPT:-$(pwd)/train_jetclass_joint_dualview_stage2_unmergeonly_m2hlt_hybrid_ops_goal_v2_attr.py}"
...
echo "============================================================"
echo "JetClass m2-style goal-conditioned hybrid reconstructor"
...
echo "Goal: apply_scale=${GOAL_APPLY_SCALE}, lambdas(head/response/axis)=${GOAL_LAMBDA_HEAD}/${GOAL_LAMBDA_RESPONSE}/${GOAL_LAMBDA_AXIS}"
echo "============================================================"
printf ' %q' "${CMD[@]}"
echo
"${CMD[@]}"
