...
but with strict split-mass conservation between parent token and its children.

Per parent token i:
- predict split fraction f_i in [0,1]
- predict child distribution alpha_{i,k} via softmax over K children (sum_k alpha_{i,k}=1)
- children receive: child_pt_{i,k} = parent_pt_i * f_i * alpha_{i,k}
- parent keeps:     parent_pt_i'   = parent_pt_i * (1 - f_i)

This enforces exact pT conservation across parent residual + children (before any downstream
budget reweighting of candidate weights).

Notes:
- Reassign behavior remains original.
- No new learned parameters are added; we reuse existing split heads.
"""
...
class OfflineReconstructorSplitConserve(reco_base.OfflineReconstructor):
    def forward(
        self,
        ...
        tok_pt_full = torch.exp(torch.clamp(torch.log(pt) + d_logpt, min=-9.0, max=9.0))
        tok_eta = (eta + 0.5 * torch.tanh(d_eta)).clamp(min=-5.0, max=5.0)
        tok_phi = reco_base.wrap_phi_t(phi + d_phi)
        tok_E_full = torch.exp(torch.clamp(torch.log(E) + d_logE, min=-9.0, max=11.0))
        tok_E_full = torch.maximum(tok_E_full, tok_pt_full * torch.cosh(tok_eta))

        # Split children from each token with strict parent-residual conservation.
        K = self.max_split_children
        split_logits = self.split_exist_head(x)  # [B, L, K]
        split_alpha = torch.softmax(split_logits, dim=-1)

        # Scalar per-parent split gate in [0,1].
        split_gate = torch.sigmoid(split_logits.mean(dim=-1))
        f_split = (split_gate * p_split * float(stage_scale) * mask_hlt.float()).clamp(0.0, 1.0)

        # Parent residual after split fraction is peeled to children.
        tok_pt = tok_pt_full * (1.0 - f_split)
        tok_E = tok_E_full * (1.0 - f_split)
        tok_E = torch.maximum(tok_E, tok_pt * torch.cosh(tok_eta))

        tok_tokens = torch.stack([tok_pt, tok_eta, tok_phi, tok_E], dim=-1)

        base_tok_weight = (p_keep + p_unsmear + 0.35 * p_reassign + 0.15 * p_split).clamp(0.0, 1.0)
        tok_weight = (base_tok_weight * (1.0 - f_split) * mask_hlt.float()).clamp(0.0, 1.0)
        ...
        # Child mass fractions sum to f_split per parent.
        child_frac = f_split.unsqueeze(-1) * split_alpha
        child_pt = tok_pt_full.unsqueeze(-1) * child_frac
        child_eta = (tok_eta.unsqueeze(-1) + 0.25 * torch.tanh(split_delta[..., 1])).clamp(min=-5.0, max=5.0)
        child_phi = reco_base.wrap_phi_t(tok_phi.unsqueeze(-1) + 0.25 * torch.tanh(split_delta[..., 2]))

        # Mirror pT conservation with E-fraction base, then enforce kinematic floor.
        child_E = tok_E_full.unsqueeze(-1) * child_frac
        ...
        child_weight = child_frac.reshape(B, L * K).clamp(0.0, 1.0)
        ...
        return {
            ...
            "split_parent_frac": f_split,
        }


def main() -> None:
    base_run.OfflineReconstructor = OfflineReconstructorSplitConserve
    base_run.main()
