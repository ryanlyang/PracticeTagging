#!/usr/bin/env python3
"""Run JetClass jet-axis audit with m2 HLT and projected-goal hybrid reconstructor."""

from __future__ import annotations

import numpy as np

import analyze_jetclass_twelve_model_jet_axis as axis
...
def main() -> None:
    # axis imports the response module and both share the same fusion module
    # object, but patch both references to keep this wrapper robust.
    for fusion in (axis.fusion, axis.response.fusion):
        fusion.v2.build_hlt_view = _build_hlt_view_m2style_compat
        fusion.hybrid_ops.OfflineReconstructorHybridOps = (
            projected_goal_hybrid.OfflineReconstructorProjectedGoalHybridOps
        )
        fusion.hybrid_ops.build_soft_corrected_view_hybrid_ops = (
