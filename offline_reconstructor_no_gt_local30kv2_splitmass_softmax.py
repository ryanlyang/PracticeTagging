...
class OfflineReconstructorSplitMassSoftmax(reco_base.OfflineReconstructor):
    """
    Less-restrictive split-mass parameterization:
    - Keep base architecture and nonlinear unsmear/reassign heads unchanged.
    - Keep linear split heads unchanged.
    - Change split child pT allocation from:
        split_frac = sigmoid(delta0) / (K + 1)
      to:
        total_split_mass = sigmoid(delta0_mean)
        alloc = softmax(delta0_child)
        split_frac_k = total_split_mass * alloc_k
    This lets splits carry more mass when needed while staying bounded.
    """
    ...
    def forward(
        ...
        # Split children from each HLT token (reparameterized split mass).
        ...
        split_mass_logits = split_delta[..., 0]
        total_split_mass = torch.sigmoid(split_mass_logits.mean(dim=-1, keepdim=True))
        child_alloc = torch.softmax(split_mass_logits, dim=-1)
        split_frac = (total_split_mass * child_alloc).clamp(min=0.0, max=1.0)
