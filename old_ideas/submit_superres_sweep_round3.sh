...
# SuperRes sweep round 3 (200 runs)
#
# Block A (120 runs): main KD grid around best region
#   SR_SAMPLES: 4, 6, 8, 10
#   KD_TEMP: 3.0, 3.5, 4.0, 4.5, 5.0
#   LOSS_KD: 0.5, 0.7, 0.9
#   LOSS_EMB: 0.1, 0.3
#
# Block B (16 runs): high-KD edge cases
#   SR_SAMPLES: 6, 8
#   KD_TEMP: 3.5, 4.5
#   LOSS_KD: 1.1, 1.3
#   LOSS_EMB: 0.5, 0.7
#
# Block C (16 runs): generator regularization sweep
#   GEN_LAMBDA_MASK: 0.25, 0.5, 0.75, 1.0
#   GEN_LAMBDA_STATS: 0.02, 0.05, 0.10, 0.20
#
# Block D (6 runs): sample count sweep
#   SR_SAMPLES: 2, 4, 6, 8, 10, 12
#
# Block E (36 runs): lower-temp region
#   SR_SAMPLES: 4, 6
#   KD_TEMP: 2.0, 2.5, 3.0
#   LOSS_KD: 0.4, 0.6, 0.8
#   LOSS_EMB: 0.1, 0.2
#
# Block F (6 runs): higher-emb alignment
#   SR_SAMPLES: 8
#   KD_TEMP: 3.5
#   LOSS_KD: 0.6, 0.8, 1.0
#   LOSS_EMB: 0.4, 0.5

SAVE_DIR=${SAVE_DIR:-"checkpoints/superres_sweep_round3"}
...
echo "Block A: main KD grid (120 runs)"
SR_SAMPLES_LIST_A=(4 6 8 10)
KD_TEMP_LIST_A=(3.0 3.5 4.0 4.5 5.0)
LOSS_KD_LIST_A=(0.5 0.7 0.9)
LOSS_EMB_LIST_A=(0.1 0.3)
    ...
    for LK in "${LOSS_KD_LIST_A[@]}"; do
      for LE in "${LOSS_EMB_LIST_A[@]}"; do
        ...
        submit_job "$RUN_NAME" "$K" "$T" "$LK" "$LE" \
          "$GEN_LAMBDA_MASK_BASE" "$GEN_LAMBDA_STATS_BASE"
      done
    done
  done
done

echo "Block B: high-KD edge cases (16 runs)"
SR_SAMPLES_LIST_B=(6 8)
KD_TEMP_LIST_B=(3.5 4.5)
LOSS_KD_LIST_B=(1.1 1.3)
LOSS_EMB_LIST_B=(0.5 0.7)
...
echo "Block C: generator regularization sweep (16 runs)"
...
echo "Block D: sample count sweep (6 runs)"
SR_SAMPLES_LIST_D=(2 4 6 8 10 12)
...
echo "Block E: lower-temp region (36 runs)"
SR_SAMPLES_LIST_E=(4 6)
KD_TEMP_LIST_E=(2.0 2.5 3.0)
LOSS_KD_LIST_E=(0.4 0.6 0.8)
LOSS_EMB_LIST_E=(0.1 0.2)
  ...
  for T in "${KD_TEMP_LIST_E[@]}"; do
    for LK in "${LOSS_KD_LIST_E[@]}"; do
      for LE in "${LOSS_EMB_LIST_E[@]}"; do
        ...
        lk_tag=${LK//./p}
        le_tag=${LE//./p}
        RUN_NAME="E_K${K}_T${t_tag}_lk${lk_tag}_le${le_tag}"
        submit_job "$RUN_NAME" "$K" "$T" "$LK" "$LE" \
          "$GEN_LAMBDA_MASK_BASE" "$GEN_LAMBDA_STATS_BASE"
      done
    done
...
echo "Block F: higher-emb alignment (6 runs)"
K_FIXED_F=8
T_FIXED_F=3.5
LOSS_KD_LIST_F=(0.6 0.8 1.0)
LOSS_EMB_LIST_F=(0.4 0.5)

for LK in "${LOSS_KD_LIST_F[@]}"; do
  for LE in "${LOSS_EMB_LIST_F[@]}"; do
    lk_tag=${LK//./p}
    le_tag=${LE//./p}
    RUN_NAME="F_lk${lk_tag}_le${le_tag}"
    submit_job "$RUN_NAME" "$K_FIXED_F" "$T_FIXED_F" "$LK" "$LE" \
      "$GEN_LAMBDA_MASK_BASE" "$GEN_LAMBDA_STATS_BASE"
  done
done
