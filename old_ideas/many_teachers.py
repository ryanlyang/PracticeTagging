...
def scale_hlt_config(cfg, strength):
    """
    Scale HLT effects between "offline" (strength=0) and nominal HLT (strength=1).
    Values >1.0 exaggerate HLT effects.
    """
    scaled = copy.deepcopy(cfg)
    h = cfg["hlt_effects"]
    s = scaled["hlt_effects"]

    s["pt_resolution"] = h["pt_resolution"] * strength
    s["eta_resolution"] = h["eta_resolution"] * strength
    s["phi_resolution"] = h["phi_resolution"] * strength

    pt_off = h["pt_threshold_offline"]
    pt_hlt = h["pt_threshold_hlt"]
    s["pt_threshold_hlt"] = pt_off + strength * (pt_hlt - pt_off)

    s["merge_radius"] = h["merge_radius"] * strength
    s["merge_enabled"] = bool(h["merge_enabled"] and strength > 0.0)

    s["efficiency_loss"] = min(1.0, h["efficiency_loss"] * strength)

    s["noise_fraction"] = min(1.0, h["noise_fraction"] * strength)
    s["noise_enabled"] = bool(h["noise_enabled"] and strength > 0.0)
    return scaled
...
class JetDatasetMultiTeacher(Dataset):
    """Student HLT + multiple teacher views."""
    def __init__(self, hlt, hlt_mask, teacher_views, teacher_masks, y):
        if isinstance(teacher_views, list):
            teacher_views = np.stack(teacher_views, axis=0)
        if isinstance(teacher_masks, list):
            teacher_masks = np.stack(teacher_masks, axis=0)
        self.hlt = torch.tensor(hlt, dtype=torch.float32)
        self.hlt_mask = torch.tensor(hlt_mask, dtype=torch.bool)
        self.teacher_views = torch.tensor(teacher_views, dtype=torch.float32)
        self.teacher_masks = torch.tensor(teacher_masks, dtype=torch.bool)
        self.y = torch.tensor(y, dtype=torch.float32)
    ...
    def __getitem__(self, i):
        return {
            "hlt": self.hlt[i],
            "mask_hlt": self.hlt_mask[i],
            "teacher": self.teacher_views[:, i],
            "mask_teacher": self.teacher_masks[:, i],
            "label": self.y[i],
        }
...
def train_one_epoch_kd_multi_teachers(student, teachers, loader, opt, device, cfg, temperature, alpha_kd):
    student.train()
    for t in teachers:
        t.eval()
    ...
    for batch in loader:
        x_hlt = batch["hlt"].to(device)
        m_hlt = batch["mask_hlt"].to(device)
        x_teacher = batch["teacher"].to(device)        # (B, T, M, F)
        m_teacher = batch["mask_teacher"].to(device)   # (B, T, M)
        y = batch["label"].to(device)
        ...
        with torch.no_grad():
            ...
            for t_idx, teacher in enumerate(teachers):
                x_t = x_teacher[:, t_idx]
                m_t = m_teacher[:, t_idx]
                t_logits, t_attn, t_z = teacher(x_t, m_t, return_attention=True, return_embedding=True)
                t_logits_list.append(t_logits.squeeze(1))
        ...
        for t_logits, t_attn, t_z, m_t in zip(t_logits_list, t_attn_list, t_z_list, m_teacher.transpose(0, 1)):
            if use_conf:
                loss_kd = loss_kd + kd_loss_conf_weighted(s_logits, t_logits, temperature)
            else:
                loss_kd = loss_kd + kd_loss_basic(s_logits, t_logits, temperature)
            if a_rep > 0:
                loss_rep = loss_rep + rep_loss_cosine(s_z, t_z.detach())
            if a_nce > 0:
                loss_nce = loss_nce + info_nce_loss(s_z, t_z.detach(), tau=tau_nce)
            if a_attn > 0:
                loss_attn = loss_attn + attn_kl_loss(s_attn, t_attn.detach(), m_hlt, m_t)

        n_t = max(1, len(teachers))
        loss_kd = loss_kd / n_t
        loss_rep = loss_rep / n_t
        loss_nce = loss_nce / n_t
        loss_attn = loss_attn / n_t
...
def fit_model_kd_multi_teachers(student, teachers, train_loader, val_loader, device, cfg, desc, save_path=None, skip_save=False,
                                temp_init=7.0, temp_final=None, alpha_init=0.5, alpha_final=None):
    opt = torch.optim.AdamW(student.parameters(), lr=cfg["training"]["lr"], weight_decay=cfg["training"]["weight_decay"])
    sch = get_scheduler(opt, cfg["training"]["warmup_epochs"], cfg["training"]["epochs"])
    ...
    for ep in tqdm(range(cfg["training"]["epochs"]), desc=desc):
        ...
        tr_loss, tr_auc = train_one_epoch_kd_multi_teachers(student, teachers, train_loader, opt, device, cfg, temp, alpha)
        va_auc, _, _ = evaluate_auc(student, val_loader, device)
        sch.step()
...
def run_half_teachers(args):
    device = torch.device(args.device)
    save_dir = Path(args.save_dir) / args.run_name
    save_dir.mkdir(parents=True, exist_ok=True)

    if args.n_teachers < 1:
        raise ValueError("n_teachers must be >= 1")
    if args.single_teacher_idx < 0 or args.single_teacher_idx >= args.n_teachers:
        raise ValueError("single_teacher_idx must be within [0, n_teachers)")

    if args.half_hlt_seeds is not None:
        teacher_seeds = [int(s.strip()) for s in args.half_hlt_seeds.split(",") if s.strip()]
        if len(teacher_seeds) != args.n_teachers:
            raise ValueError("Number of --half_hlt_seeds must match --n_teachers")
    else:
        teacher_seeds = [args.half_hlt_seed_base + i * args.half_hlt_seed_step for i in range(args.n_teachers)]

    print(f"Device:  {device}")
    print(f"Save dir:{save_dir}")
    print(f"Half-HLT strength={args.half_hlt_strength}, student-HLT strength={args.student_hlt_strength}")
    print(f"Teacher seeds: {teacher_seeds}")
    print(f"Student seed:  {args.student_hlt_seed}")
    ...
    cfg_half = scale_hlt_config(CONFIG, args.half_hlt_strength)
    cfg_student = scale_hlt_config(CONFIG, args.student_hlt_strength)

    print("\nGenerating half-HLT teacher views...")
    half_const_list = []
    half_mask_list = []
    for i, seed in enumerate(teacher_seeds, start=1):
        print(f"Half-HLT view #{i} seed={seed}")
        const_hlt, mask_hlt = apply_hlt_effects(constituents_raw, mask_raw, cfg_half, seed=seed, verbose=False)
        half_const_list.append(const_hlt)
        half_mask_list.append(mask_hlt)

    print("\nGenerating student HLT (3/2 strength) view...")
    const_student, mask_student = apply_hlt_effects(
        constituents_raw, mask_raw, cfg_student, seed=args.student_hlt_seed, verbose=True
    )

    print("\nComputing features...")
    features_off = compute_features(constituents_off, masks_off)
    features_half_list = [compute_features(half_const_list[i], half_mask_list[i]) for i in range(args.n_teachers)]
    features_student = compute_features(const_student, mask_student)
    ...
    features_half_std_list = [
        standardize(features_half_list[i], half_mask_list[i], feat_means, feat_stds)
        for i in range(args.n_teachers)
    ]
    features_student_std = standardize(features_student, mask_student, feat_means, feat_stds)

    # Save test artifacts
    test_data_dir = Path().cwd() / "test_split"
    test_data_dir.mkdir(exist_ok=True)
    np.savez(
        test_data_dir / "test_features_and_masks_half_teachers.npz",
        idx_test=test_idx,
        labels=all_labels[test_idx],
        feat_off=features_off_std[test_idx],
        feat_hlt_half_views=np.stack([features_half_std_list[i][test_idx] for i in range(args.n_teachers)], axis=0),
        feat_hlt_student=features_student_std[test_idx],
        mask_off=masks_off[test_idx],
        mask_hlt_half_views=np.stack([half_mask_list[i][test_idx] for i in range(args.n_teachers)], axis=0),
        mask_hlt_student=mask_student[test_idx],
        jet_pt=all_pt[test_idx] if all_pt is not None else None,
        feat_means=feat_means,
        feat_stds=feat_stds,
        n_teachers=args.n_teachers,
        half_hlt_strength=args.half_hlt_strength,
        student_hlt_strength=args.student_hlt_strength,
    )
    print(f"Saved test artifacts to: {test_data_dir / 'test_features_and_masks_half_teachers.npz'}")

    BS = CONFIG["training"]["batch_size"]

    # Student HLT loaders (3/2 strength)
    train_student = JetDatasetSingle(features_student_std[train_idx], mask_student[train_idx], all_labels[train_idx])
    val_student   = JetDatasetSingle(features_student_std[val_idx],   mask_student[val_idx],   all_labels[val_idx])
    test_student  = JetDatasetSingle(features_student_std[test_idx],  mask_student[test_idx],  all_labels[test_idx])

    train_student_loader = DataLoader(train_student, batch_size=BS, shuffle=True, drop_last=True)
    val_student_loader   = DataLoader(val_student,   batch_size=BS, shuffle=False)
    test_student_loader  = DataLoader(test_student,  batch_size=BS, shuffle=False)

    # Teacher loaders (half HLT)
    teacher_models = []
    teacher_aucs = []
    for i in range(args.n_teachers):
        print("\n" + "=" * 80)
        print(f"STEP 1.{i+1}: TEACHER (Half-HLT view {i+1})")
        print("=" * 80)
        train_t = JetDatasetSingle(features_half_std_list[i][train_idx], half_mask_list[i][train_idx], all_labels[train_idx])
        val_t   = JetDatasetSingle(features_half_std_list[i][val_idx],   half_mask_list[i][val_idx],   all_labels[val_idx])
        test_t  = JetDatasetSingle(features_half_std_list[i][test_idx],  half_mask_list[i][test_idx],  all_labels[test_idx])
        train_t_loader = DataLoader(train_t, batch_size=BS, shuffle=True, drop_last=True)
        val_t_loader   = DataLoader(val_t,   batch_size=BS, shuffle=False)
        test_t_loader  = DataLoader(test_t,  batch_size=BS, shuffle=False)

        teacher = ParticleTransformer(input_dim=7, **CONFIG["model"]).to(device)
        best_auc, _ = fit_model_standard(
            teacher, train_t_loader, val_t_loader, device, CONFIG,
            desc=f"Teacher-half{i+1}",
            save_path=save_dir / f"teacher_half_{i+1}.pt",
            skip_save=args.skip_save_models,
        )
        teacher.eval()
        for p in teacher.parameters():
            p.requires_grad = False
        auc_t, _, _ = evaluate_auc(teacher, test_t_loader, device)
        teacher_models.append(teacher)
        teacher_aucs.append(auc_t)
        print(f"Teacher {i+1} test AUC (half-HLT): {auc_t:.4f}")

    print("\n" + "=" * 80)
    print("STEP 2: BASELINE STUDENT (3/2 HLT)")
    print("=" * 80)
    baseline = ParticleTransformer(input_dim=7, **CONFIG["model"]).to(device)
    best_baseline_auc, _ = fit_model_standard(
        baseline, train_student_loader, val_student_loader, device, CONFIG,
        desc="Baseline-3/2-HLT",
        save_path=save_dir / "baseline_hlt15.pt",
        skip_save=args.skip_save_models,
    )

    print("\n" + "=" * 80)
    print(f"STEP 3: STUDENT KD (single teacher idx={args.single_teacher_idx})")
    print("=" * 80)
    train_kd_single = JetDatasetOffHlt(
        features_half_std_list[args.single_teacher_idx][train_idx],
        half_mask_list[args.single_teacher_idx][train_idx],
        features_student_std[train_idx],
        mask_student[train_idx],
        all_labels[train_idx],
    )
    train_kd_single_loader = DataLoader(train_kd_single, batch_size=BS, shuffle=True, drop_last=True)
    student_kd_one = ParticleTransformer(input_dim=7, **CONFIG["model"]).to(device)
    best_kd_one_auc, _ = fit_model_kd(
        student_kd_one, teacher_models[args.single_teacher_idx],
        train_kd_single_loader, val_student_loader, device, CONFIG,
        desc="Student-KD-single",
        save_path=save_dir / "student_kd_single.pt",
        skip_save=args.skip_save_models,
    ...
    print("\n" + "=" * 80)
    print("STEP 4: STUDENT KD (all teachers)")
    print("=" * 80)
    train_kd_multi = JetDatasetMultiTeacher(
        features_student_std[train_idx],
        mask_student[train_idx],
        [features_half_std_list[i][train_idx] for i in range(args.n_teachers)],
        [half_mask_list[i][train_idx] for i in range(args.n_teachers)],
        all_labels[train_idx],
    )
    train_kd_multi_loader = DataLoader(train_kd_multi, batch_size=BS, shuffle=True, drop_last=True)
    student_kd_all = ParticleTransformer(input_dim=7, **CONFIG["model"]).to(device)
    best_kd_all_auc, _ = fit_model_kd_multi_teachers(
        student_kd_all, teacher_models,
        train_kd_multi_loader, val_student_loader, device, CONFIG,
        desc="Student-KD-all",
        save_path=save_dir / "student_kd_all.pt",
        skip_save=args.skip_save_models,
        temp_init=args.temp_init,
        temp_final=args.temp_final,
        alpha_init=args.alpha_init,
    ...
    print("\n" + "=" * 80)
    print("FINAL TEST EVALUATION (3/2 HLT)")
    print("=" * 80)
    auc_baseline, preds_baseline, labs = evaluate_auc(baseline, test_student_loader, device)
    auc_kd_one, preds_kd_one, _ = evaluate_auc(student_kd_one, test_student_loader, device)
    auc_kd_all, preds_kd_all, _ = evaluate_auc(student_kd_all, test_student_loader, device)

    print(f"\n{'Model':<40} {'AUC':>10}")
    print("-" * 55)
    print(f"{'Baseline (3/2 HLT)':<40} {auc_baseline:>10.4f}")
    print(f"{'Student KD (single teacher)':<40} {auc_kd_one:>10.4f}")
    print(f"{'Student KD (all teachers)':<40} {auc_kd_all:>10.4f}")
    print("-" * 55)

    fpr_b, tpr_b, _ = roc_curve(labs, preds_baseline)
    fpr_o, tpr_o, _ = roc_curve(labs, preds_kd_one)
    fpr_a, tpr_a, _ = roc_curve(labs, preds_kd_all)

    wp = 0.5
    idx_b = np.argmax(tpr_b >= wp)
    idx_o = np.argmax(tpr_o >= wp)
    idx_a = np.argmax(tpr_a >= wp)
    br_b = 1.0 / fpr_b[idx_b] if fpr_b[idx_b] > 0 else 0.0
    br_o = 1.0 / fpr_o[idx_o] if fpr_o[idx_o] > 0 else 0.0
    br_a = 1.0 / fpr_a[idx_a] if fpr_a[idx_a] > 0 else 0.0

    print(f"\nBackground Rejection @ {wp*100:.0f}% signal efficiency (3/2 HLT):")
    print(f"  Baseline: {br_b:.2f}")
    print(f"  KD single: {br_o:.2f}")
    print(f"  KD all: {br_a:.2f}")

    plt.figure(figsize=(8, 6))
    plt.plot(tpr_b, fpr_b, "--", label=f"Baseline (AUC={auc_baseline:.3f})", linewidth=2)
    plt.plot(tpr_o, fpr_o, ":", label=f"KD single (AUC={auc_kd_one:.3f})", linewidth=2)
    plt.plot(tpr_a, fpr_a, "-.", label=f"KD all (AUC={auc_kd_all:.3f})", linewidth=2)
    ...
    np.savez(
        ...
        preds_kd_one=preds_kd_one,
        preds_kd_all=preds_kd_all,
        auc_baseline=auc_baseline,
        auc_kd_one=auc_kd_one,
        auc_kd_all=auc_kd_all,
        br_baseline=br_b,
        br_kd_one=br_o,
        br_kd_all=br_a,
        teacher_aucs=np.array(teacher_aucs, dtype=np.float32),
        n_teachers=args.n_teachers,
        half_hlt_strength=args.half_hlt_strength,
        student_hlt_strength=args.student_hlt_strength,
    ...
    with open(summary_file, "a") as f:
        ...
        f.write(f"Half-HLT strength: {args.half_hlt_strength}\n")
        f.write(f"Student HLT strength: {args.student_hlt_strength}\n")
        f.write(f"Teacher seeds: {teacher_seeds}\n")
        f.write(f"Teacher AUCs (half HLT): {teacher_aucs}\n")
        f.write(f"AUC Baseline (3/2 HLT): {auc_baseline:.4f} | BR@50%: {br_b:.2f}\n")
        f.write(f"AUC KD single: {auc_kd_one:.4f} | BR@50%: {br_o:.2f}\n")
        f.write(f"AUC KD all: {auc_kd_all:.4f} | BR@50%: {br_a:.2f}\n")
...
def main():
    ...
    parser.add_argument("--mode", type=str, choices=["half_teachers", "legacy"], default="half_teachers",
                        help="half_teachers = half/one-and-half HLT teacher experiment; legacy = original multi-view script")

    # Half-HLT teacher experiment
    parser.add_argument("--n_teachers", type=int, default=3, help="Number of half-HLT teacher datasets/models")
    parser.add_argument("--half_hlt_strength", type=float, default=0.5, help="Strength for half-HLT generation")
    parser.add_argument("--student_hlt_strength", type=float, default=1.5, help="Strength for student HLT generation")
    parser.add_argument("--half_hlt_seeds", type=str, default=None, help="Comma-separated seeds for half-HLT teachers")
    parser.add_argument("--half_hlt_seed_base", type=int, default=123, help="Base seed for half-HLT teachers")
    parser.add_argument("--half_hlt_seed_step", type=int, default=333, help="Seed step for half-HLT teachers")
    parser.add_argument("--student_hlt_seed", type=int, default=999, help="Seed for 3/2 HLT student dataset")
    parser.add_argument("--single_teacher_idx", type=int, default=0, help="Teacher index to use for single-teacher KD")
    ...
    if args.mode == "half_teachers":
        run_half_teachers(args)
        return
