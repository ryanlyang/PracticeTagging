...
"""
Fusion analysis for six independently trained models:
  ...
  6) Corrected-only tagger from concat-teacher setup

Reports val-selected->test and oracle test references for:
  - individual models
  - HLT+X weighted pairs
  - all-6 weighted fusion (raw / Platt / Isotonic)
...
def build_meta_features(score_dict: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    cc = np.asarray(score_dict["concat_corrected"], dtype=np.float64)

    return np.column_stack([
        h, jd, rs, cs, j5, cc,
        np.abs(h - jd), np.abs(h - rs), np.abs(h - cs), np.abs(h - j5), np.abs(h - cc),
        np.abs(jd - rs), np.abs(jd - cs), np.abs(jd - j5), np.abs(jd - cc),
    ]).astype(np.float64)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Fusion analysis for HLT + five additional models")
    ...
    ap.add_argument("--output_name", type=str, default="fusion_hlt_joint6_analysis.json")
    args = ap.parse_args()
    ...
    dir_concat = Path(args.concat_run_dir)
    ...
    concat_npz = dir_concat / "concat_teacher_stageA_scores.npz"

    for pth in [joint_delta_npz, reco_s09_npz, corr_s01_npz, joint_s01_npz, concat_npz]:
        if not pth.exists():
    ...
    scores_val = {
        ...
        "concat_corrected": np.asarray(z6["preds_corrected_only_val"], dtype=np.float64),
    }
    scores_test = {
        ...
        "concat_corrected": np.asarray(z6["preds_corrected_only_test"], dtype=np.float64),
    }
    ...
    raw_all6_valsel = select_weighted_combo_multi_on_val_eval_test(
        y_val=y_val,
    ...
    raw_all6_oracle = search_best_weighted_combo_multi_at_tpr(
        labels=y_test,
        score_mat=mat_test,
        model_names=MODEL_ORDER,
        target_tpr=target_tpr,
    ...
    platt_all6_valsel = select_weighted_combo_multi_on_val_eval_test(
        y_val=y_val,
    ...
    platt_all6_oracle = search_best_weighted_combo_multi_at_tpr(
        labels=y_test,
    ...
    iso_all6_valsel = select_weighted_combo_multi_on_val_eval_test(
        y_val=y_val,
    ...
    iso_all6_oracle = search_best_weighted_combo_multi_at_tpr(
        labels=y_test,
    ...
    stepwise.extend([
        make_gain_row("all6 raw weighted (val->test)", fpr_hlt_test, float(raw_all6_valsel["test_eval"]["fpr"])),
        make_gain_row("all6 Platt weighted (val->test)", fpr_hlt_test, float(platt_all6_valsel["test_eval"]["fpr"])),
        make_gain_row("all6 Isotonic weighted (val->test)", fpr_hlt_test, float(iso_all6_valsel["test_eval"]["fpr"])),
    ...
    raw_w = np.asarray(raw_all6_valsel["test_eval"]["weights"], dtype=np.float64)
    raw_score = np.sum(mat_test * raw_w[:, None], axis=0)
    raw_thr = float(raw_all6_valsel["test_eval"]["threshold_from_val"])

    iso_w = np.asarray(iso_all6_valsel["test_eval"]["weights"], dtype=np.float64)
    iso_score = np.sum(mat_iso_test * iso_w[:, None], axis=0)
    iso_thr = float(iso_all6_valsel["test_eval"]["threshold_from_val"])
    ...
    out = {
        ...
        "all6_weighted": {
            ...
            "raw_val_selected_eval_test": raw_all6_valsel,
            "raw_test_oracle": raw_all6_oracle,
            "platt_val_selected_eval_test": platt_all6_valsel,
            "platt_test_oracle": platt_all6_oracle,
            "isotonic_val_selected_eval_test": iso_all6_valsel,
            "isotonic_test_oracle": iso_all6_oracle,
        },
        "calibration": {
            "platt": cal_platt,
            "isotonic": cal_iso,
    ...
    print("=" * 70)
    print("HLT + Joint(2x) + RecoTeacher + Corrected + Concat Fusion Analysis")
    print("=" * 70)
