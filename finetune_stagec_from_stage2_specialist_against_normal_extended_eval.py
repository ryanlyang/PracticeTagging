#!/usr/bin/env python3
"""
Stage-C specialist continuation from a saved normal-joint run with extended eval splits.

This variant can keep the original train split fixed, optionally extend val/test,
then train a specialist joint model with discrepancy weighting anchored to a
frozen normal joint model. Checkpoint selection can be done by minimizing
FPR@target-TPR for a deploy-style normal+special weighted combo on validation.

It exports:
  - standalone metrics (teacher / HLT / anchor-normal / specialist)
  - disagreement summaries
  - best weighted normal+special combos at target TPR
  - best OR-union operating-point pairs (normal_tpr, specialist_tpr) targeting
    combined TPR ~= target.
"""
...
def predict_joint_scores(
    ...
    _, preds, labs, _ = joint.eval_joint_model(
...
def search_best_weighted_combo_two_models(
    y: np.ndarray,
    p_normal: np.ndarray,
    ...
    p_special = p_special.astype(np.float32)
    ...
    rows: List[Dict[str, object]] = []
    for inorm in range(inv + 1):
        w_norm = inorm / inv
        w_spec = 1.0 - w_norm
        if min(w_norm, w_spec) < min_w:
            continue

        fused = w_norm * p_normal + w_spec * p_special
        if len(np.unique(y)) <= 1:
            auc = float('nan')
            fpr_t = float('nan')
        else:
        ...
        rows.append(
            {
                'w_normal': float(w_norm),
                'w_special': float(w_spec),
                'auc': float(auc),
                'fpr_at_target_tpr': float(fpr_t),
                'target_tpr': float(tgt),
            }
    ...
    return best, rows_sorted[:max(1, int(top_k))]


def search_best_union_tpr_pairs_two_models(
    y: np.ndarray,
    p_normal: np.ndarray,
    p_special: np.ndarray,
    target_tpr: float,
    ...
    lo = float(max(1e-4, tpr_min))
    hi = float(min(1.0 - 1e-4, tpr_max))
    st = float(max(1e-4, tpr_step))
    tgt = float(np.clip(target_tpr, 1e-4, 1.0 - 1e-4))
    tol = float(max(0.0, tpr_tol))

    grid = np.arange(lo, hi + 0.5 * st, st, dtype=np.float64)

    rows: List[Dict[str, object]] = []
    for tpr_n in grid:
        thr_n = prob_threshold_at_target_tpr(pos_n, float(tpr_n))
        pred_n = p_normal >= float(thr_n)
        for tpr_s in grid:
            thr_s = prob_threshold_at_target_tpr(pos_s, float(tpr_s))
            pred_s = p_special >= float(thr_s)

            pred_u = pred_n | pred_s
            tp = int(np.sum(pred_u & pos))
            fp = int(np.sum(pred_u & neg))
            n_pos = int(np.sum(pos))
            n_neg = int(np.sum(neg))
            tpr_u = _safe_rate(tp, n_pos)
            fpr_u = _safe_rate(fp, n_neg)

            rows.append(
                {
                    'tpr_normal_target': float(tpr_n),
                    'tpr_special_target': float(tpr_s),
                    'thr_normal': float(thr_n),
                    'thr_special': float(thr_s),
                    'tpr_union': float(tpr_u),
                    'fpr_union': float(fpr_u),
                    'tpr_gap': float(abs(float(tpr_u) - tgt)) if np.isfinite(float(tpr_u)) else float('inf'),
                    'target_tpr': float(tgt),
                }
            )
    ...
    rows_sorted = sorted(
        rows,
        key=lambda r: (
            float('inf') if not np.isfinite(float(r['tpr_gap'])) else float(r['tpr_gap']),
            float('inf') if not np.isfinite(float(r['fpr_union'])) else float(r['fpr_union']),
        ),
    )

    within_tol = [r for r in rows if np.isfinite(float(r['tpr_union'])) and abs(float(r['tpr_union']) - tgt) <= tol]
    within_tol_sorted = sorted(
        within_tol,
        key=lambda r: float('inf') if not np.isfinite(float(r['fpr_union'])) else float(r['fpr_union']),
    )

    ge_target = [r for r in rows if np.isfinite(float(r['tpr_union'])) and float(r['tpr_union']) >= tgt]
    ge_target_sorted = sorted(
        ge_target,
        key=lambda r: (
            float('inf') if not np.isfinite(float(r['fpr_union'])) else float(r['fpr_union']),
            float('inf') if not np.isfinite(float(r['tpr_gap'])) else float(r['tpr_gap']),
    ...
    best_within_tol = within_tol_sorted[0] if len(within_tol_sorted) > 0 else {
        'tpr_normal_target': float('nan'),
        'tpr_special_target': float('nan'),
        'thr_normal': float('nan'),
        'thr_special': float('nan'),
        'tpr_union': float('nan'),
        'fpr_union': float('nan'),
        'tpr_gap': float('nan'),
        'target_tpr': float(tgt),
    }
    best_ge_target = ge_target_sorted[0] if len(ge_target_sorted) > 0 else {
        'tpr_normal_target': float('nan'),
        'tpr_special_target': float('nan'),
        'thr_normal': float('nan'),
        'thr_special': float('nan'),
    ...
    return best_within_tol, best_ge_target, rows_sorted[:max(1, int(top_k))]
...
def train_joint_dual_weighted(
    ...
    combo_ref_val_probs: np.ndarray | None = None,
    combo_target_tpr: float = 0.50,
    combo_weight_step: float = 0.02,
    combo_min_weight: float = 0.0,
    ...
    best_val_combo_fpr50 = float("inf")

    select_mode = str(select_metric).lower()
    if select_mode in ("fpr50", "combo_fpr50"):
    ...
    sel_val_combo_fpr50 = float("nan")
    sel_val_combo_w_normal = float("nan")
    no_improve = 0
    ...
    for ep in range(int(epochs)):
        ...
        va_auc, va_preds, va_labs, va_fpr50 = joint.eval_joint_model(
            reconstructor=reconstructor,
            dual_model=dual_model,
            loader=val_loader,
            device=device,
        ...
        va_combo_fpr50 = float("nan")
        va_combo_w_normal = float("nan")
        if select_mode == "combo_fpr50":
            if combo_ref_val_probs is None:
                raise RuntimeError("select_metric=combo_fpr50 requires combo_ref_val_probs.")
            if combo_ref_val_probs.shape[0] != va_preds.shape[0]:
                raise RuntimeError(
                    "combo_ref_val_probs length mismatch: "
                    f"{combo_ref_val_probs.shape[0]} vs {va_preds.shape[0]}"
                )
            combo_best, _ = search_best_weighted_combo_two_models(
                y=va_labs.astype(np.int64),
                p_normal=combo_ref_val_probs.astype(np.float32),
                p_special=va_preds.astype(np.float32),
                target_tpr=float(combo_target_tpr),
                weight_step=float(combo_weight_step),
                min_weight=float(combo_min_weight),
                top_k=1,
            )
            va_combo_fpr50 = float(combo_best.get("fpr_at_target_tpr", float("nan")))
            va_combo_w_normal = float(combo_best.get("w_normal", float("nan")))
            if np.isfinite(va_combo_fpr50) and float(va_combo_fpr50) < best_val_combo_fpr50:
                best_val_combo_fpr50 = float(va_combo_fpr50)
        ...
        elif select_mode == "combo_fpr50":
            improved = np.isfinite(va_combo_fpr50) and (float(va_combo_fpr50) < best_sel_score)
            current_score = float(va_combo_fpr50) if np.isfinite(va_combo_fpr50) else float("inf")
        else:
        ...
        if improved:
            ...
            sel_val_combo_fpr50 = float(va_combo_fpr50)
            sel_val_combo_w_normal = float(va_combo_w_normal)
        ...
        combo_msg = ""
        if select_mode == "combo_fpr50":
            combo_msg = f", val_combo_fpr50={va_combo_fpr50:.6f}, w_normal={va_combo_w_normal:.3f}"
        if (ep + 1) % print_every == 0:
            print(
                f"{stage_name} ep {ep+1}: train_loss={tr_loss:.4f} "
                f"(cls={tr_cls:.4f}, rank={tr_rank:.4f}, reco={tr_reco:.4f}, cons={tr_cons:.4f}) | "
                f"val_auc={va_auc:.4f}, val_fpr50={va_fpr50:.6f}{combo_msg}, "
    ...
    metrics = {
        ...
        "selected_val_combo_fpr50": float(sel_val_combo_fpr50),
        "selected_val_combo_w_normal": float(sel_val_combo_w_normal),
        "best_val_fpr50_seen": float(best_val_fpr50),
        "best_val_auc_seen": float(best_val_auc),
        "best_val_combo_fpr50_seen": float(best_val_combo_fpr50),
    }
    state_pack = {
...
def main() -> None:
    ...
    p.add_argument(
        "--fresh_dual_init",
        action="store_true",
        help="Initialize dual-view classifier from scratch instead of loading init dual checkpoint.",
    )

    # Anchor (normal joint) for discrepancy weighting + combo selection.
    p.add_argument("--anchor_run_dir", type=str, default="", help="If empty, uses --run_dir")
    p.add_argument("--anchor_reco_ckpt", type=str, default="")
    p.add_argument("--anchor_dual_ckpt", type=str, default="")
    ...
    p.add_argument("--selection_metric", type=str, default="combo_fpr50", choices=["auc", "fpr50", "combo_fpr50"])
    p.add_argument("--val_combo_target_tpr", type=float, default=0.50)
    p.add_argument("--val_combo_weight_step", type=float, default=0.02)
    p.add_argument("--val_combo_min_weight", type=float, default=0.00)
    ...
    # 3-model weighted combination search on extended test split (teacher+HLT+special).
    ...
    # 2-model weighted combo search (normal anchor + specialist).
    p.add_argument("--two_model_combo_enable", action="store_true")
    p.add_argument("--two_model_combo_weight_step", type=float, default=0.01)
    p.add_argument("--two_model_combo_min_weight", type=float, default=0.00)
    p.add_argument("--two_model_combo_top_k", type=int, default=50)

    # 2-model OR-union TPR-pair sweep (normal_tpr, specialist_tpr -> combined tpr/fpr).
    p.add_argument("--two_model_union_enable", action="store_true")
    p.add_argument("--two_model_union_tpr_min", type=float, default=0.30)
    p.add_argument("--two_model_union_tpr_max", type=float, default=0.60)
    p.add_argument("--two_model_union_tpr_step", type=float, default=0.01)
    p.add_argument("--two_model_union_tpr_tol", type=float, default=0.003)
    p.add_argument("--two_model_union_top_k", type=int, default=80)
    ...
    anchor_run_dir = Path(args.anchor_run_dir) if str(args.anchor_run_dir).strip() else run_dir
    if not anchor_run_dir.exists():
        raise FileNotFoundError(f"anchor_run_dir not found: {anchor_run_dir}")
    ...
    print(f"Anchor run dir: {anchor_run_dir}")
    ...
    anchor_ckpt_dir = Path(args.anchor_run_dir) if str(args.anchor_run_dir).strip() else run_dir
    anchor_reco_ckpt = Path(args.anchor_reco_ckpt) if str(args.anchor_reco_ckpt).strip() else (
        anchor_ckpt_dir / "offline_reconstructor.pt" if (anchor_ckpt_dir / "offline_reconstructor.pt").exists() else (anchor_ckpt_dir / "offline_reconstructor_stage2.pt")
    )
    anchor_dual_ckpt = Path(args.anchor_dual_ckpt) if str(args.anchor_dual_ckpt).strip() else (
        anchor_ckpt_dir / "dual_joint.pt" if (anchor_ckpt_dir / "dual_joint.pt").exists() else (anchor_ckpt_dir / "dual_joint_stage2.pt")
    )
    if not anchor_reco_ckpt.exists():
        raise FileNotFoundError(f"Missing anchor reco checkpoint: {anchor_reco_ckpt}")
    if not anchor_dual_ckpt.exists():
        raise FileNotFoundError(f"Missing anchor dual checkpoint: {anchor_dual_ckpt}")

    anchor_dual_state = _load_checkpoint_state(anchor_dual_ckpt, device, "anchor_dual")
    key_a_anchor = "input_proj_a.0.weight"
    key_b_anchor = "input_proj_b.0.weight"
    if key_a_anchor not in anchor_dual_state or key_b_anchor not in anchor_dual_state:
        raise RuntimeError("Could not infer anchor dual input dimensions from anchor checkpoint.")
    anchor_dim_a = int(anchor_dual_state[key_a_anchor].shape[1])
    anchor_dim_b = int(anchor_dual_state[key_b_anchor].shape[1])
    if int(feat_hlt_dual.shape[-1]) != int(anchor_dim_a):
        raise RuntimeError(
            "Anchor dual input_dim_a mismatch between features and anchor checkpoint: "
            f"feat_hlt_dual={feat_hlt_dual.shape[-1]}, ckpt={anchor_dim_a}."
        )
    anchor_use_flags = bool(anchor_dim_b == 12)

    anchor_reco_model = OfflineReconstructor(input_dim=7, **cfg["reconstructor_model"]).to(device)
    anchor_dual_model = DualViewCrossAttnClassifier(
        input_dim_a=anchor_dim_a,
        input_dim_b=anchor_dim_b,
        **cfg["model"],
    ).to(device)
    anchor_reco_model.load_state_dict(_load_checkpoint_state(anchor_reco_ckpt, device, "anchor_reco"))
    anchor_dual_model.load_state_dict(anchor_dual_state)
    anchor_reco_model.eval()
    anchor_dual_model.eval()

    ds_train_anchor_eval = WeightedJointDualDataset(
        ...
        labels[train_idx], sample_weight=np.ones((len(train_idx),), dtype=np.float32),
    )
    ds_val_anchor_eval = WeightedJointDualDataset(
    ...
    ds_test_anchor_eval = WeightedJointDualDataset(
    ...
    dl_train_anchor_eval = DataLoader(
        ds_train_anchor_eval, batch_size=bs, shuffle=False,
        num_workers=int(args.num_workers), pin_memory=torch.cuda.is_available(),
    )
    dl_val_anchor_eval = DataLoader(
        ds_val_anchor_eval, batch_size=bs, shuffle=False,
        num_workers=int(args.num_workers), pin_memory=torch.cuda.is_available(),
    )
    dl_test_anchor_eval = DataLoader(
        ds_test_anchor_eval, batch_size=bs, shuffle=False,
        num_workers=int(args.num_workers), pin_memory=torch.cuda.is_available(),
    )
    p_anchor_train, y_anchor_train = predict_joint_scores(
        anchor_reco_model,
        anchor_dual_model,
        dl_train_anchor_eval,
        device=device,
        corrected_weight_floor=float(args.corrected_weight_floor),
        corrected_use_flags=bool(anchor_use_flags),
    )
    p_anchor_val, y_anchor_val = predict_joint_scores(
        anchor_reco_model,
        anchor_dual_model,
        dl_val_anchor_eval,
        device=device,
    ...
    p_anchor_test, y_anchor_test = predict_joint_scores(
        anchor_reco_model,
        anchor_dual_model,
        dl_test_anchor_eval,
        device=device,
    ...
    if bool(args.disc_weight_enable):
        teacher_ckpt = anchor_ckpt_dir / "teacher.pt"
        ...
        if not np.array_equal(y_teacher_train.astype(np.int64), y_anchor_train.astype(np.int64)):
            raise RuntimeError("Teacher/anchor train label mismatch while building discrepancy weights.")
        if not np.array_equal(y_teacher_val.astype(np.int64), y_anchor_val.astype(np.int64)):
            raise RuntimeError("Teacher/anchor val label mismatch while building discrepancy weights.")
        ...
        train_sample_weight, discrepancy_summary = build_discrepancy_weights(
            ...
            p_baseline_train=p_anchor_train,
            p_teacher_val=p_teacher_val,
            p_baseline_val=p_anchor_val,
        ...
        print(
            "Discrepancy weighting enabled (teacher vs anchor-normal): "
            ...
            f"t_anchor={discrepancy_summary.get('t_hlt_val', float('nan')):.6f}, "
    ...
    reco_ckpt = Path(args.reco_ckpt) if str(args.reco_ckpt).strip() else (
        (run_dir / "offline_reconstructor.pt") if (run_dir / "offline_reconstructor.pt").exists() else (run_dir / "offline_reconstructor_stage2.pt")
    )
    dual_ckpt = Path(args.dual_ckpt) if str(args.dual_ckpt).strip() else (
        (run_dir / "dual_joint.pt") if (run_dir / "dual_joint.pt").exists() else (run_dir / "dual_joint_stage2.pt")
    )
    ...
    teacher_metrics = maybe_eval_single_view_checkpoint(
        ckpt_path=anchor_ckpt_dir / "teacher.pt",
        tag="teacher",
    ...
    baseline_metrics = maybe_eval_single_view_checkpoint(
        ckpt_path=anchor_ckpt_dir / "baseline.pt",
        tag="baseline",
    ...
    def _better_selected(new_m: Dict[str, float], cur_m: Dict[str, float] | None) -> bool:
        if cur_m is None:
            return True
        mode = str(args.selection_metric).lower()
        if mode == "auc":
            return float(new_m.get("selected_val_auc", float("-inf"))) > float(cur_m.get("selected_val_auc", float("-inf")))
        if mode == "combo_fpr50":
            return float(new_m.get("selected_val_combo_fpr50", float("inf"))) < float(cur_m.get("selected_val_combo_fpr50", float("inf")))
    ...
    def _run_phase(phase_name: str, freeze_reco: bool, epochs: int, patience: int, min_epochs: int) -> None:
        ...
        reconstructor, dual_joint, ph_metrics, ph_states = train_joint_dual_weighted(
            ...
            combo_ref_val_probs=p_anchor_val.astype(np.float32),
            combo_target_tpr=float(args.val_combo_target_tpr),
            combo_weight_step=float(args.val_combo_weight_step),
            combo_min_weight=float(args.val_combo_min_weight),
        )
    ...
    stageC_metrics = {
        ...
        "selected_val_combo_fpr50": float(selected_metrics.get("selected_val_combo_fpr50", float("nan"))) if selected_metrics else float("nan"),
        "selected_val_combo_w_normal": float(selected_metrics.get("selected_val_combo_w_normal", float("nan"))) if selected_metrics else float("nan"),
        ...
        "best_val_combo_fpr50_seen": float(selected_metrics.get("best_val_combo_fpr50_seen", float("nan"))) if selected_metrics else float("nan"),
    }
    stageC_states = {
    ...
    teacher_pred_model.load_state_dict(_load_checkpoint_state(anchor_ckpt_dir / "teacher.pt", device, "teacher_for_disagreement"))
    baseline_pred_model.load_state_dict(_load_checkpoint_state(anchor_ckpt_dir / "baseline.pt", device, "baseline_for_disagreement"))
    ...
    two_model_combo_out: Dict[str, object] = {"enabled": bool(args.two_model_combo_enable)}
    two_model_combo_best: Dict[str, object] = {}
    two_model_combo_top: List[Dict[str, object]] = []
    if bool(args.two_model_combo_enable):
        two_model_combo_best, two_model_combo_top = search_best_weighted_combo_two_models(
            y=y_teacher_test,
            p_normal=p_anchor_test.astype(np.float32),
            p_special=preds_joint.astype(np.float32),
            target_tpr=dis_target_tpr,
            weight_step=float(args.two_model_combo_weight_step),
            min_weight=float(args.two_model_combo_min_weight),
            top_k=int(args.two_model_combo_top_k),
        )
        two_model_combo_out = {
            "enabled": True,
            "target_tpr": float(dis_target_tpr),
            ...
            "min_weight": float(args.two_model_combo_min_weight),
            "top_k": int(args.two_model_combo_top_k),
            "best": two_model_combo_best,
            "top": two_model_combo_top,
        }

    two_model_union_out: Dict[str, object] = {"enabled": bool(args.two_model_union_enable)}
    two_model_union_best_tol: Dict[str, object] = {}
    two_model_union_best_ge: Dict[str, object] = {}
    two_model_union_top: List[Dict[str, object]] = []
    if bool(args.two_model_union_enable):
        two_model_union_best_tol, two_model_union_best_ge, two_model_union_top = search_best_union_tpr_pairs_two_models(
            y=y_teacher_test,
            p_normal=p_anchor_test.astype(np.float32),
            p_special=preds_joint.astype(np.float32),
            target_tpr=dis_target_tpr,
            tpr_min=float(args.two_model_union_tpr_min),
            tpr_max=float(args.two_model_union_tpr_max),
            tpr_step=float(args.two_model_union_tpr_step),
            tpr_tol=float(args.two_model_union_tpr_tol),
            top_k=int(args.two_model_union_top_k),
        )
        two_model_union_out = {
            "enabled": True,
            "target_tpr": float(dis_target_tpr),
            ...
            "tpr_tol": float(args.two_model_union_tpr_tol),
            "top_k": int(args.two_model_union_top_k),
            "best_within_tol": two_model_union_best_tol,
            "best_ge_target": two_model_union_best_ge,
            "top": two_model_union_top,
        }
    ...
    if bool(args.two_model_combo_enable):
        _write_tsv(
            dis_dir / "combo_weighted_top_normal_special.tsv",
            two_model_combo_top,
            [
                "w_normal",
                "w_special",
    ...
    if bool(args.two_model_union_enable):
        _write_tsv(
            dis_dir / "union_tpr_pairs_top_normal_special.tsv",
            two_model_union_top,
            [
                "tpr_normal_target",
                "tpr_special_target",
    ...
    base_test = {}
    base_path = anchor_ckpt_dir / "joint_stage_metrics.json"
    if base_path.exists():
        try:
    ...
    for fname in [
        ...
        if fname in {"teacher.pt", "baseline.pt", "hlt_stats.json", "joint_stage_metrics.json"}:
            src = anchor_ckpt_dir / fname
        else:
            src = run_dir / fname
        if src.exists():
            try:
    ...
    out_metrics = {
        ...
        "anchor_run_dir": str(anchor_ckpt_dir),
        "source_reco_ckpt": str(reco_ckpt),
        "source_dual_ckpt": str(dual_ckpt),
        "anchor_reco_ckpt": str(anchor_reco_ckpt),
        "anchor_dual_ckpt": str(anchor_dual_ckpt),
        "stageC_args": {
            ...
            "val_combo_target_tpr": float(args.val_combo_target_tpr),
            "val_combo_weight_step": float(args.val_combo_weight_step),
            "val_combo_min_weight": float(args.val_combo_min_weight),
            "corrected_weight_floor": float(args.corrected_weight_floor),
            ...
            "anchor_reco_ckpt": str(anchor_reco_ckpt),
            "anchor_dual_ckpt": str(anchor_dual_ckpt),
            "two_model_combo_enable": bool(args.two_model_combo_enable),
            "two_model_combo_weight_step": float(args.two_model_combo_weight_step),
            "two_model_combo_min_weight": float(args.two_model_combo_min_weight),
            "two_model_combo_top_k": int(args.two_model_combo_top_k),
            "two_model_union_enable": bool(args.two_model_union_enable),
            "two_model_union_tpr_min": float(args.two_model_union_tpr_min),
            "two_model_union_tpr_max": float(args.two_model_union_tpr_max),
            "two_model_union_tpr_step": float(args.two_model_union_tpr_step),
            "two_model_union_tpr_tol": float(args.two_model_union_tpr_tol),
            "two_model_union_top_k": int(args.two_model_union_top_k),
        },
        "data_reload": {
        ...
        "two_model_combo_normal_special": two_model_combo_out,
        "two_model_union_normal_special": two_model_union_out,
    }
    ...
    print(f"Anchor Normal (loaded): AUC={auc_stage2:.4f}, FPR30={fpr30_stage2:.6f}, FPR50={fpr50_stage2:.6f}")
    ...
    if bool(args.two_model_combo_enable):
        print(
            "Best 2-model weighted combo (Normal+Special @ target TPR): "
            f"wN={two_model_combo_best.get('w_normal', float('nan')):.3f}, "
            f"wS={two_model_combo_best.get('w_special', float('nan')):.3f}, "
            f"FPR={two_model_combo_best.get('fpr_at_target_tpr', float('nan')):.6f}"
        )
    if bool(args.two_model_union_enable):
        print(
            "Best 2-model OR-union pair (within tol): "
            f"tprN={two_model_union_best_tol.get('tpr_normal_target', float('nan')):.3f}, "
            f"tprS={two_model_union_best_tol.get('tpr_special_target', float('nan')):.3f}, "
            f"unionTPR={two_model_union_best_tol.get('tpr_union', float('nan')):.6f}, "
            f"unionFPR={two_model_union_best_tol.get('fpr_union', float('nan')):.6f}"
        )
        print(
            "Best 2-model OR-union pair (TPR>=target): "
            f"tprN={two_model_union_best_ge.get('tpr_normal_target', float('nan')):.3f}, "
            f"tprS={two_model_union_best_ge.get('tpr_special_target', float('nan')):.3f}, "
            f"unionTPR={two_model_union_best_ge.get('tpr_union', float('nan')):.6f}, "
            f"unionFPR={two_model_union_best_ge.get('fpr_union', float('nan')):.6f}"
        )
